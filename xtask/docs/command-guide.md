...
- `xtask ra-diagnose`: Diagnose rust-analyzer process footprint and local workspace contract. Use when rust-analyzer is slow, memory-heavy, duplicated, or suspected of ignoring the local workspace contract. Common forms: `xtask ra-diagnose`; `xtask --json ra-diagnose`. Notes: This command reuses the rust-analyzer contract checks surfaced by `xtask doctor --rust-analyzer`.
