...
- `xtask fix`: Apply automatic fixes across workspace packages. `--fmt-only` for fast format-only (~1s); default for fmt + compiler fix; `--clippy` to add clippy lint fixes; `--thorough` for per-package maximal coverage. Common forms: `xtask fix --fmt-only`; `xtask fix`; `xtask fix --clippy`; `xtask fix --thorough`; `xtask fix --smart`.
