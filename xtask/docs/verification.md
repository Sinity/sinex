...
| Phase and closure claims | phase manifest plus Bead `close_reason` evidence parser | `xtask verify plan --check`; `xtask verify closure <bead-id>` | Phase manifests can declare `evidence_manifest` rows. Bead closeouts declare ordinal `Closure Evidence Manifest` rows covering every AC; satisfied non-doc rows require executable commands, deferred rows name follow-up Beads, and source-text or grep-only behavior evidence is rejected. |
...
| --- | --- | --- |
| Focused PR loop | `xtask test` with the owning package/test filter; `xtask impact explain --json` when scope selection matters | During implementation and review of a coherent Bead phase. This is the default agent loop: prove the changed behavior and make the selected test scope explainable. |
| PR publish boundary | `xtask check --changed-strict origin/master`; generated-surface checks touched by the diff, such as `xtask docs command-reference --check` or `xtask docs schema-bundle --check` | Before pushing/opening a PR that changes Rust API, command/docs/schema surfaces, source catalogs, or package metadata. Bind `dev_services` to the workspace whose path equals `git rev-parse --show-toplevel` with `agentctl job start sinex dev_services --workspace "$workspace_id"`, then pass `SINEX_PRE_PUSH_AGENTCTL_LEASE_ID=<job-id>` to the push. The installed pre-push hook computes the pushed checkout's devshell cache, target, database socket, NATS, state, and port env before running `xtask`. It rejects malformed state, project, checkout, lease, or duplicate port identities, performs PostgreSQL database-and-port and NATS protocol readiness checks, and holds the exact lease witness across the selected execution. It revalidates that witness after the command returns, so cancellation or reallocation during the handoff or execution cannot silently produce a green push. The fallback performs its protocol probes inside `nix develop`. Without the variable, the ordinary fallback remains unchanged. It first reports whether the branch changed xtask build inputs. For branches that did not touch those inputs, it can use an existing checkout-local, PATH, or cached xtask binary without a rebuild. For branches that did touch those inputs, the selected binary must be fresh; otherwise the hook prints the changed inputs and the newer source that invalidated each candidate before rebuilding. It uses `nix develop "$REPO_ROOT" --command xtask ...` only when no usable checkout wrapper or binary exists, so ordinary worktree pushes do not inherit another checkout's env or start a surprise bootstrap. Smoke the selector without running the guard via `SINEX_PRE_PUSH_DRY_RUN=1 .githooks/pre-push`; set `BASE_REF=<ref>` to exercise a specific diff. |
...
- None in this cleanup. The remaining items above already have owning Beads.
