...
| PR publish boundary | `xtask check --changed-strict origin/master --allow-contended-host`; generated-surface checks touched by the diff, such as `xtask docs command-reference --check` or `xtask docs schema-bundle --check` | Before pushing/opening a PR that changes Rust API, command/docs/schema surfaces, source catalogs, or package metadata. The installed pre-push hook computes the pushed checkout's devshell cache, target, database socket, NATS, state, and port env before running `xtask`. It prefers that checkout's built `target/debug/xtask`, then a non-`.direnv` PATH xtask with the computed checkout env. It uses `nix develop "$REPO_ROOT" --command xtask ...` only when no usable binary exists, so ordinary worktree pushes do not inherit another checkout's env or start a surprise bootstrap. |
