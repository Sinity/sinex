    ...
    match cmd {
        ...
        TlsCommand::Check {
            ...
            let mut res = CommandResult::success()
                .with_message(if result.valid { "TLS configuration valid" } else { "TLS configuration has issues" })
...
fn setup_env(
    ...
    if !server_cert.exists() {
        anyhow::bail!(
            "Server certificate not found at {}. Run 'cargo xtask stack tls generate-dev-certs' first.",
            server_cert.display()
        );
    }
    if !server_key.exists() {
        anyhow::bail!(
            "Server key not found at {}. Run 'cargo xtask stack tls generate-dev-certs' first.",
            server_key.display()
        );
    }
    ...
    content.push_str(&format!("# Generated by: cargo xtask stack tls setup-env\n\n"));
    ...
    if mtls {
        ...
        if !ca_cert.exists() {
            anyhow::bail!(
                "CA certificate not found at {}. Run 'cargo xtask stack tls generate-dev-certs' first.",
                ca_cert.display()
            );
        }
        content.push_str(&format!(
