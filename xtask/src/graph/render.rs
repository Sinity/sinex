...
/// {
///   "nodes": [
///     { "id": "sinex-core", "label": "sinex-core" },
///     { "id": "sinex-gateway", "label": "sinex-gateway" }
///   ],
///   "edges": [
///     { "source": "sinex-gateway", "target": "sinex-core" }
///   ]
/// }
/// ```
///
...
/// # Example
///
/// In a workspace where `sinex-gateway` imports from `sinex-core`:
/// ```text
/// EdgeJson {
///     source: "sinex-gateway",  // depends on
///     target: "sinex-core"      // this package
/// }
/// ```
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EdgeJson {
        ...
        for pkg in &packages {
            ...
            if let Ok(query) = self.graph.graph().query_forward(vec![pkg_id]) {
                ...
                for dep_id in resolved.package_ids(guppy::graph::DependencyDirection::Forward) {
                    if pkg_id != dep_id {
                        if let Ok(dep_metadata) = self.graph.graph().metadata(dep_id) {
                            edges.push(EdgeJson {
