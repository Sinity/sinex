...
fn generate_dev_source_bindings_manifest_for_home_and_exports(
    ...
    DevSourceBindingsManifest {
        comment: "Generated by `xtask infra dev-bindings`. Point SINEX_SOURCE_BINDINGS_PATH at this file, then run `xtask run all-sources --bg` to start the fast dogfood dev loop with real terminal/git/fs/journald/browser/desktop source bindings when their local materials exist.".to_string(),
        bindings,
    }
}
...
fn execute_dev_bindings(
    ...
    Ok(CommandResult::success()
        ...
        .with_detail(format!("Output: {}", output.display()))
        .with_detail(format!(
            "Run with: SINEX_SOURCE_BINDINGS_PATH={} xtask run all-sources --bg",
            output.display()
        ))
        .with_data(serde_json::to_value(DevBindingsResult {
            output: Some(output.display().to_string()),
