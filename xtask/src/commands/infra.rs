...
fn generate_dev_source_bindings_manifest_for_home_and_exports(
    ...
    DevSourceBindingsManifest {
        comment: "Generated by `xtask infra dev-bindings`. Point SINEX_SOURCE_BINDINGS_PATH at this file before `xtask run core` to run the fast dogfood dev loop with real terminal/git/fs/journald/browser/desktop sources when their local materials exist.".to_string(),
        bindings,
    }
}
