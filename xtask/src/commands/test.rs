...
impl TestCommand {
    ...
    fn resolve_scenario_selection(&self, ctx: &CommandContext) -> Result<ScenarioSelection> {
        ...
        validate_scenario_filter_values(
            "category",
            &self.scenario_categories,
            SCENARIO_CATEGORIES,
        )?;
        validate_scenario_filter_values("lane", &self.scenario_lanes, SCENARIO_LANES)?;

        let workspace_root = crate::sandbox::orchestrator::find_workspace_root()?;
        let catalog = discover_scenario_catalog(&workspace_root)?;
...
fn scenario_packages(entries: &[ScenarioCatalogEntry]) -> Vec<String> {
...
fn scenario_nextest_filter(entries: &[ScenarioCatalogEntry]) -> Option<String> {
    if entries.is_empty() {
        return None;
    }
    let names = entries
