...
pub struct SnapshotCommand {
    ...
    /// U4: Include CLAUDE.md and docs/ + .agent/ (project memory) in the snapshot
    #[arg(long)]
...
impl XtaskCommand for SnapshotCommand {
    ...
    async fn execute(&self, ctx: &CommandContext) -> Result<CommandResult> {
        ...
        // U4: Include project memory (CLAUDE.md + docs/ + .agent/)
        if self.project_memory {
            dynamic_includes.push("CLAUDE.md".to_string());
            dynamic_includes.push("docs/**".to_string());
            dynamic_includes.push(".agent/**".to_string());
            if ctx.is_human() {
                println!("  Project memory: including CLAUDE.md, docs/, and .agent/");
            }
        }
