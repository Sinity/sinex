...
The hook derives the pushed checkout's devshell cache, target, PostgreSQL socket and port, NATS port, and state directories itself, so a push from any worktree runs against that worktree's own stack. Start it first:
...
`agentctl workspace list` must return the registered workspace whose path equals `git rev-parse --show-toplevel`. For a new linked checkout, create it with `agentctl workspace create sinex <name> --branch <branch>`, change into the path returned by that workspace record, and run the same lookup.
