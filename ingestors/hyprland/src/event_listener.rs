...
impl HyprlandEventListener {
    ...
    pub async fn start(self) -> IngestorResult<()> {
        info!("Starting Hyprland event listener with socket2 capture");
        info!("Socket path: {}", self.socket_path.display());
        info!("Configuration: window_augmentation={:?}, workspace_tracking={:?}", 
             self.config.window_augmentation, self.config.workspace_tracking);
