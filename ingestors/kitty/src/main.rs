...
use crate::watcher::KittyIngestor;
...
async fn run_ingestor(config: Config, dry_run: bool, output_file: Option<PathBuf>) -> Result<()> {
    ...
    let event_sink: Arc<dyn EventSink> = if let Some(ref file_path) = output_file {
        ...
        Arc::new(LogSink::new("kitty-ingestor"))
    } else {
        // Initialize database
    ...
    let ingestor = KittyIngestor::new(config.kitty.clone());
    
    // Create runtime config
    let runtime_config = RuntimeConfig {
        heartbeat_interval_secs: config.kitty.heartbeat_interval_secs,
        retry_config: sinex_shared::RetryConfig {
            max_retries: config.kitty.max_retries,
            initial_delay: std::time::Duration::from_secs(1),
            max_delay: std::time::Duration::from_secs(config.kitty.retry_delay_secs),
            exponential_base: 2,
        },
        batch_size: None, // Kitty doesn't batch
...
async fn register_manifest(db: &DatabaseService) -> Result<()> {
    ...
    produces.insert(
        ...
        vec![
            event_type_constants::terminal::COMMAND_EXECUTED.to_string(),
        ],
    );
