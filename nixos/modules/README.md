...
- The module emits `sinexd` as the only long-running Sinex runtime unit.
  ...
  automaton hosts.
- Source runtime defaults are derived from
  `nixos/modules/source-catalog.generated.json`, which is generated by
  `sinexd export-source-catalog` from the linked Rust source inventory. The
  NixOS module reads that artifact with `builtins.fromJSON` for source binding
  metadata and resource defaults; generation lives in `sinexd` because `xtask`
  does not link the source registration inventory. Regenerate with
  `nix develop --command sinexd export-source-catalog` and validate with
  `nix flake check .#checks.x86_64-linux.source-catalog-eval`.
