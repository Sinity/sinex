...
Source binding defaults come from
`nixos/modules/source-catalog.generated.json`, which is generated by
`sinexd export-source-catalog` from the linked Rust source inventory and read by
the NixOS module with `builtins.fromJSON`. The lightweight
`source-catalog-eval` flake check validates the catalog shape and required
source entries at eval time.

### Runtime Resource Profiles

Explicit overrides still use the NixOS module resource shape:
...
- `agentctl job list` — AgentCTL-owned development-service lease summary
