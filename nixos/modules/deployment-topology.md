...
preflight/bootstrap oneshots, and support units generated by enabled source
surfaces:

| Service | Role | Notes |
|---|---|---|
| `sinexd` | Unified runtime | Applies schema at startup, then hosts event engine, API, source contracts, automata, and supervisor. |
...
There are no live split source or per-automaton binaries in the current
...
```
postgresql ─► sinex-preflight ─► sinexd
nats ───────► sinex-nats-bootstrap ─┘
...
- `sinexd` runs declarative schema convergence during startup when
  `SINEX_SCHEMA_APPLY_ON_STARTUP=1`.
