...
CREATE TABLE IF NOT EXISTS sinex_schemas.agent_manifests (
    ...
    config_template_json    JSONB, -- Example JSON/YAML structure of expected config file
    config_schema_id        ULID REFERENCES sinex_schemas.event_payload_schemas(id), 
    ...
    llm_dependencies        JSONB,   -- {"models_used": ["ollama/mistral:7b", "openai/gpt-4-turbo"], "required_capabilities": ["function_calling"]}
    
    repo_url                TEXT,    -- Link to agent's source code
