...
**Architecture Flow:**
1. **Satellite Services** → **sinex-ingestd** (gRPC) → **PostgreSQL** (`raw.events`) + **Redis Streams** (`sinex:events`)
...
*   **Universal Event Log:** All data flows through `raw.events` as immutable, append-only truth
...
## 4. The Canonical Event Substrate: `raw.events`
...
SELECT create_hypertable('raw.events', 'ts_ingest');
```
...
SELECT set_chunk_time_interval('raw.events', INTERVAL '1 week');

-- Enable compression for older chunks
SELECT add_compression_policy('raw.events', INTERVAL '30 days');

-- Retention policy for very old data
SELECT add_retention_policy('raw.events', INTERVAL '5 years');
```
...
```sql
-- Add validation constraint to raw.events
ALTER TABLE raw.events ADD CONSTRAINT valid_payload_schema
CHECK (
...
5. Valid events written to raw.events with schema ID
