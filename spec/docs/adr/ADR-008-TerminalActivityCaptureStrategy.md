...
*   **Decision:**
    ...
    1.  **Atuin (Primary for Structured Command History):** Atuin will be configured for all supported shells (Bash, Zsh, Fish) to capture structured command history (command, timestamp, CWD, exit status, duration) into its local SQLite database. A dedicated Exocortex agent (`ingestor/atuin_db_reader`) will periodically ingest new command entries from this database into `core.events` (e.g., as `shell.command.executed_atuin` events).
