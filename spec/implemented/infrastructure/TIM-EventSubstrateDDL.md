...
## MVP Specification
- Complete core.events table with ULID primary keys
...
*   **Purpose:** Provides the canonical Data Definition Language (DDL) for the `core.events` table and closely related foundational schema objects necessary for the event substrate.
...
COMMENT ON SCHEMA raw IS 'Schema for raw, immutable event data (core.events).';
...
COMMENT ON TABLE core.events IS 'Universal log for all captured raw events before promotion or detailed structuring. Immutable by principle.';
COMMENT ON COLUMN core.events.id IS 'Globally unique, time-sortable ULID for the event (pgx_ulid).';
...
COMMENT ON COLUMN core.events.ts_ingest IS 'Timestamp of ingestion into this table (database server time). Primary TimescaleDB partitioning key.';
...
COMMENT ON COLUMN core.events.ingestor_version IS 'Version of the ingestor code/binary that produced this event.';
COMMENT ON COLUMN core.events.payload_schema_id IS 'ULID Foreign key to sinex_schemas.event_payload_schemas, identifying the schema for the payload. Null if ad-hoc/unknown.';
COMMENT ON COLUMN core.events.payload IS 'Complete raw event data as JSONB. May contain a "_provenance" sub-object for lineage details (e.g., "agent_id_if_generated", "input_event_id_if_derived").';

-- Indexes for core.events
-- Primary Key index is created automatically

-- For common filtering and sorting by time of occurrence
CREATE INDEX IF NOT EXISTS idx_raw_events_ts_orig_desc ON core.events (ts_orig DESC NULLS LAST);

-- For querying by source, type, and then time (common for agent processing)
CREATE INDEX IF NOT EXISTS idx_raw_events_source_type_ts_ingest_desc ON core.events (source, event_type, ts_ingest DESC);

-- For querying by host and time
CREATE INDEX IF NOT EXISTS idx_raw_events_host_ts_ingest_desc ON core.events (host, ts_ingest DESC);

-- For finding events by their payload schema
CREATE INDEX IF NOT EXISTS idx_raw_events_payload_schema_id ON core.events (payload_schema_id) WHERE payload_schema_id IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_raw_events_payload_gin_path_ops ON core.events USING GIN (payload jsonb_path_ops);
-- If full JSONB search is common:
-- CREATE INDEX IF NOT EXISTS idx_raw_events_payload_gin_ops ON core.events USING GIN (payload);
