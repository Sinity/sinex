...
*   **Data Substrate:** The foundational layer of the Exocortex responsible for data storage, event logging, and basic structuring (PostgreSQL, TimescaleDB, `core.events`, etc.).
...
*   **Domain Table:** A PostgreSQL table with a strong schema, holding data promoted and structured from `core.events` for a specific domain (e.g., `domain_hyprland.focus_changes`, `domain_terminal.commands_atuin`).
...
*   **Event (`core.events`):** The atomic unit of data in the Exocortex. An immutable record with `source`, `event_type`, timestamps, `host`, `payload_schema_id`, and a JSONB `payload`.
...
*   **Ingestor:** An Exocortex agent specifically responsible for capturing data from an external source and writing it as events into `core.events`.
...
*   **JSON Schema:** A vocabulary that allows you to annotate and validate JSON documents. Used for defining `core.events.payload` structures.
...
*   **Promotion (Event Promotion):** The process by which raw events from `core.events` are transformed by agents into more structured data in domain tables or knowledge graph entities.
...
*   **`core.events`:** The central, immutable, append-only PostgreSQL table where all captured data is initially logged as events.
...
*   **Schema Registry (`sinex_schemas.event_payload_schemas`):** The PostgreSQL table storing versioned JSON Schema definitions for `core.events.payload` structures.
...
*   **TimescaleDB:** A PostgreSQL extension that turns tables into hypertables for efficient time-series data management. Used for `core.events`.
