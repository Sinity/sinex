...
The schema solves a unique challenge: combining ULID primary keys with TimescaleDB's partitioning requirements:
...
6. **`sinex`** - Legacy compatibility
   - `metrics` - Backward compatibility table for sinex-metrics-lib
...
```sql
-- Events reference schemas and source materials
...
- Suggests future implementation of automatic compression for historical data
...
- **ULID primary key** benefits preserved
...
- **Compression planning** for historical data
