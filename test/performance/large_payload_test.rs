...
async fn test_progressive_payload_sizes(ctx: TestContext) -> TestResult {
    ...
    for (size, label) in test_sizes {
        println!("\n{} Testing {} payload", chrono::Utc::now().format("%H:%M:%S"), label);
        ...
        match insert_result {
            Ok(Ok(_)) => {
                ...
                match retrieve_result {
                    ...
                    Ok(Err(e)) => {
                        ...
                        results.push((label, payload_size, 0, insert_duration, Duration::ZERO, false));
                    }
                    Err(_) => {
            ...
            Ok(Err(e)) => {
                ...
                results.push((label, payload_size, 0, Duration::ZERO, Duration::ZERO, false));
    ...
    for (depth, description) in depths {
        ...
        match insert_event(pool, &event).await {
            Ok(_) => {
                ...
                match query_result {
                    Ok(Some(row)) => {
                        println!("  Type: {:?}, Depth measurement: {:?}", 
    ...
    for (size, description) in array_sizes {
        ...
        match timeout(Duration::from_secs(30), insert_event(pool, &event)).await {
            Ok(Ok(_)) => {
                ...
                match ops_result {
                    Ok(row) => {
                        println!("  Array length: {:?}, Storage size: {} bytes",
                            row.array_length, row.data_size.unwrap_or(0));
                    }
...
async fn test_concurrent_large_payload_memory_pressure(ctx: TestContext) -> TestResult {
    ...
    println!("Spawning {} concurrent tasks with {}MB payloads each",
        concurrent_tasks, payload_size / 1_000_000);
    ...
    println!("  Total data processed: {}MB", 
        (successful * payload_size) / 1_000_000);
    
    // Most tasks should succeed
    assert!(successful > concurrent_tasks / 2, 
...
fn generate_large_json_payload(target_size: usize) -> Value {
    ...
    // Add metadata
    data.insert("generated_at".to_string(), json!(chrono::Utc::now().to_rfc3339()));
