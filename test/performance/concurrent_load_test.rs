...
impl ConcurrentLoadMetrics {
    ...
    async fn record_operation(&self, operation_type: &str, duration: StdDuration, success: bool) {
        if success {
            ...
            *counts.entry(operation_type.to_string()).or_insert(0) += 1;
            ...
            latencies.entry(operation_type.to_string())
    ...
    async fn print_summary(&self) {
        ...
        for operation_type in counts.keys() {
            ...
            println!("  - Success rate: {:.2}%", self.get_success_rate(operation_type).await);
            println!("  - Average latency: {:?}", self.get_average_latency(operation_type).await);
            println!("  - P95 latency: {:?}", self.get_percentile_latency(operation_type, 95.0).await);
            println!("  - P99 latency: {:?}", self.get_percentile_latency(operation_type, 99.0).await);
        }
    }
}
...
async fn test_concurrent_event_ingestion(ctx: TestContext) -> TestResult {
    ...
    let worker_handles = (0..worker_count)
        .map(|worker_id| {
            ...
            tokio::spawn(async move {
                    ...
                    match sinex_db::insert_event_with_validator(&pool_clone, &event, None).await {
                        Ok(_) => {
                            ...
                            }
                            {
                                ...
                                latencies.entry("concurrent_insert".to_string())
                                    .or_insert_with(Vec::new)
                                    .push(duration);
                            }
                        }
                        Err(e) => {
                            ...
                            *errors.entry("concurrent_insert".to_string()).or_insert(0) += 1;
                            ...
                            if worker_errors <= 3 { // Only log first few errors
                ...
                println!("✅ Worker {} completed: {} successes, {} errors", 
    ...
    assert!(total_successes as f64 / total_expected as f64 > 0.95,
        "Success rate should be > 95%");
    assert!(metrics.calculate_throughput().await > 100.0,
        "Concurrent throughput should be > 100 ops/sec");
    assert!(metrics.get_average_latency("concurrent_insert").await < StdDuration::from_millis(100),
...
async fn test_mixed_concurrent_workload(ctx: TestContext) -> TestResult {
                ...
                for op_id in 0..operations_per_worker {
                    ...
                    match operation_type {
                        0..=4 => {
                            ...
                            let factory = EventFactory::new(&format!("mixed-workload-worker-{}", worker_id));
                            let event = factory.create_event(
                                event_types::test::MIXED_WORKLOAD_TEST,
                                json!({
                                    "worker_id": worker_id,
                            ...
                            if result.is_ok() {
                                ...
                                *counts.entry("mixed_insert".to_string()).or_insert(0) += 1;
                                ...
                                latencies.entry("mixed_insert".to_string())
                                    .or_insert_with(Vec::new)
                                    .push(duration);
                            } else {
                        ...
                        5..=7 => {
                            ...
                            if result.is_ok() {
                                ...
                                *counts.entry("mixed_query".to_string()).or_insert(0) += 1;
                                ...
                                let mut latencies = metrics_clone_latencies.lock().await;
                                latencies.entry("mixed_query".to_string())
                                    .or_insert_with(Vec::new)
                                    .push(duration);
                            } else {
                                let mut errors = metrics_clone_errors.lock().await;
                        ...
                        8..=9 => {
                            ...
                            if result.is_ok() {
                                ...
                                *counts.entry("mixed_complex_query".to_string()).or_insert(0) += 1;
                                ...
                                let mut latencies = metrics_clone_latencies.lock().await;
                                latencies.entry("mixed_complex_query".to_string())
                                    .or_insert_with(Vec::new)
                                    .push(duration);
                            } else {
                                let mut errors = metrics_clone_errors.lock().await;
    ...
    // Verify database consistency using centralized query system
    let mixed_workload_count = EventQueries::count_by_source_pattern(&pool, "mixed-workload-worker-%").await?;
    
    println!("🔍 Mixed workload events stored: {}", mixed_workload_count);
    
    // Performance assertions
    assert!(metrics.calculate_throughput().await > 80.0,
        "Mixed workload throughput should be > 80 ops/sec");
    assert!(metrics.get_success_rate("mixed_insert").await > 95.0,
        "Mixed insert success rate should be > 95%");
    assert!(metrics.get_success_rate("mixed_query").await > 95.0,
        "Mixed query success rate should be > 95%");
    assert!(metrics.get_average_latency("mixed_insert").await < StdDuration::from_millis(100),
        "Mixed insert latency should be < 100ms");
    assert!(metrics.get_average_latency("mixed_query").await < StdDuration::from_millis(50),
...
async fn test_rate_limited_concurrent_load(ctx: TestContext) -> TestResult {
                ...
                for op_id in 0..operations_per_worker {
                    ...
                    if result.is_ok() {
                        ...
                        *counts.entry("rate_limited_insert".to_string()).or_insert(0) += 1;
                        ...
                        let mut latencies = metrics_clone_latencies.lock().await;
                        latencies.entry("rate_limited_insert".to_string())
                            .or_insert_with(Vec::new)
                            .push(duration);
                    } else {
                        let mut errors = metrics_clone_errors.lock().await;
    ...
    // Verify database consistency using centralized query system
    let rate_limited_count = EventQueries::count_by_source_pattern(&pool, "rate-limited-worker-%").await?;
    ...
    assert!(metrics.get_average_latency("rate_limited_insert").await < StdDuration::from_millis(150),
        "Rate-limited average latency should be < 150ms");
    assert!(metrics.get_percentile_latency("rate_limited_insert", 95.0).await < StdDuration::from_millis(500),
...
async fn test_burst_load_handling(ctx: TestContext) -> TestResult {
            ...
            .map(|worker_id| {
                ...
                tokio::spawn(async move {
                    for op_id in 0..operations_per_burst {
                        ...
                        let factory = EventFactory::new(&format!("burst-worker-{}-{}", cycle, worker_id));
                        let event = factory.create_event(
                        ...
                        if result.is_ok() {
                            ...
                            *counts.entry("burst_high".to_string()).or_insert(0) += 1;
                            ...
                            let mut latencies = metrics_clone_latencies.lock().await;
                            latencies.entry("burst_high".to_string())
                                .or_insert_with(Vec::new)
                                .push(duration);
                        } else {
                            let mut errors = metrics_clone_errors.lock().await;
            ...
            .map(|worker_id| {
                ...
                tokio::spawn(async move {
                    ...
                    for op_id in 0..(operations_per_burst / 4) {
                        ...
                        let factory = EventFactory::new(&format!("cooldown-worker-{}-{}", cycle, worker_id));
                        let event = factory.create_event(
                        ...
                        if result.is_ok() {
                            ...
                            *counts.entry("burst_low".to_string()).or_insert(0) += 1;
                            ...
                            let mut latencies = metrics_clone_latencies.lock().await;
                            latencies.entry("burst_low".to_string())
                                .or_insert_with(Vec::new)
                                .push(duration);
                        } else {
                            let mut errors = metrics_clone_errors.lock().await;
    ...
    let cooldown_worker_count = EventQueries::count_by_source_pattern(&pool, "cooldown-worker-%").await?;
    let total_burst_events = burst_worker_count + cooldown_worker_count;
    
    println!("🔍 Burst load events stored: {}", total_burst_events);
    
    // Performance assertions
    assert!(metrics.get_success_rate("burst_high").await > 90.0,
        "High activity burst success rate should be > 90%");
    assert!(metrics.get_success_rate("burst_low").await > 95.0,
    ...
    assert!(high_latency < StdDuration::from_millis(200),
