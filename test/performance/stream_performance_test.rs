...
impl StreamMetrics {
    ...
    fn record_throughput(&mut self, operation: &str, count: usize) {
        self.throughput_measurements.push((Instant::now(), count, operation.to_string()));
    }
    ...
    fn print_summary(&self) {
        ...
        println!("Average message size: {:.1} bytes", self.average_message_size());
        ...
        for operation in self.operation_times.keys() {
            ...
            println!("  - Throughput: {:.2} ops/sec", self.calculate_throughput(operation));
            println!("  - Average latency: {:?}", self.average_latency(operation));
            println!("  - P50 latency: {:?}", self.percentile_latency(operation, 50.0));
...
#[sinex_test]
async fn test_stream_write_performance(ctx: TestContext) -> TestResult {
    ...
    println!("✍️  Testing stream write performance with {} messages", message_count);
    ...
    assert!(metrics.calculate_throughput("stream_write") > 500.0,
        "Stream write throughput should be > 500 messages/sec");
    assert!(metrics.average_latency("stream_write") < StdDuration::from_millis(5),
        "Average write latency should be < 5ms");
    assert!(metrics.percentile_latency("stream_write", 95.0) < StdDuration::from_millis(20),
        "P95 write latency should be < 20ms");
    assert!(metrics.success_rate("stream_write") > 99.0,
...
#[sinex_test]
async fn test_stream_read_performance(ctx: TestContext) -> TestResult {
    let redis_client = RedisStreamClient::new("redis://localhost:6379")?;
    let mut metrics = StreamMetrics::new();
        ...
        match cmd("XREADGROUP")
            ...
            Ok(messages) => {
                ...
                for message in &messages {
                    if let Err(e) = redis_client.xack(stream_key, consumer_group, &message.id).await {
    ...
    // Performance assertions
    assert!(metrics.calculate_throughput("stream_read_batch") > 100.0,
        "Stream read throughput should be > 100 batches/sec");
    assert!(metrics.average_latency("stream_read_batch") < StdDuration::from_millis(50),
        "Average read batch latency should be < 50ms");
    assert!(metrics.average_latency("stream_ack_batch") < StdDuration::from_millis(20),
        "Average ACK batch latency should be < 20ms");
    assert!(metrics.success_rate("stream_read_batch") > 95.0,
        "Read batch success rate should be > 95%");
    assert!(messages_read >= message_count * 95 / 100,
...
#[sinex_test]
async fn test_concurrent_stream_performance(ctx: TestContext) -> TestResult {
    ...
    // Create consumer group
    match redis_client.xgroup_create(stream_key, consumer_group, "$", true).await {
                ...
                for msg_id in 0..messages_per_producer {
                    ...
                    match redis_client.xadd(&stream_key, "*", &message_data).await {
                        Ok(_) => {
                            ...
                            metrics_lock.record_operation("concurrent_write", operation_start.elapsed(), true);
                            metrics_lock.record_message_size(serde_json::to_string(&message_data).unwrap_or_default().len());
                        }
                        Err(e) => {
                            let mut metrics_lock = metrics.lock().await;
                            metrics_lock.record_operation("concurrent_write", operation_start.elapsed(), false);
            ...
            .query_async(&mut redis_client).await {
                        Ok(messages) => {
                            ...
                            if !messages.keys.is_empty() {
                                ...
                                let mut metrics_lock = metrics.lock().await;
                                metrics_lock.record_operation("concurrent_read", read_duration, true);
                            }
                        }
                        Err(e) => {
                            let mut metrics_lock = metrics.lock().await;
                            metrics_lock.record_operation("concurrent_read", operation_start.elapsed(), false);
                ...
                println!("  Consumer {} completed: {} messages consumed", consumer_id, messages_consumed);
    ...
    // Performance assertions
    assert!(final_metrics.calculate_throughput("concurrent_write") > 200.0,
        "Concurrent write throughput should be > 200 messages/sec");
    assert!(final_metrics.calculate_throughput("concurrent_read") > 50.0,
        "Concurrent read throughput should be > 50 operations/sec");
    assert!(final_metrics.average_latency("concurrent_write") < StdDuration::from_millis(10),
        "Concurrent write latency should be < 10ms");
    assert!(final_metrics.average_latency("concurrent_read") < StdDuration::from_millis(100),
        "Concurrent read latency should be < 100ms");
    assert!(final_metrics.success_rate("concurrent_write") > 95.0,
        "Concurrent write success rate should be > 95%");
    assert!(final_metrics.success_rate("concurrent_read") > 90.0,
...
#[sinex_test]
async fn test_variable_message_size_performance(ctx: TestContext) -> TestResult {
    let redis_client = RedisStreamClient::new("redis://localhost:6379")?;
    let mut metrics = StreamMetrics::new();
    ...
    for (size_bytes, size_label) in size_tests {
        ...
        println!("    Average latency: {:?}", metrics.average_latency(&format!("write_{}", size_label)));
        println!("    P95 latency: {:?}", metrics.percentile_latency(&format!("write_{}", size_label), 95.0));
    }
    ...
    for (_, size_label) in &size_tests {
        let operation_key = format!("write_{}", size_label);
        assert!(metrics.success_rate(&operation_key) > 95.0,
            "{} success rate should be > 95%", size_label);
        assert!(metrics.average_latency(&operation_key) < StdDuration::from_millis(50),
            "{} average latency should be < 50ms", size_label);
    }
    
    assert!(metrics.success_rate("read_variable") > 95.0,
