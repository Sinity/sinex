...
impl RegressionDetector {
    ...
    pub fn calculate_current_performance(&self, operation: &str) -> Option<PerformanceMeasurement> {
        if let Some(measurements) = self.measurements.get(operation) {
            ...
            let percentile_95_latency = sorted_measurements[p95_index.min(sorted_measurements.len() - 1)];
            let percentile_99_latency = sorted_measurements[p99_index.min(sorted_measurements.len() - 1)];
    ...
    pub fn detect_regression(&self, operation: &str) -> Option<RegressionDetectionResult> {
        ...
        let latency_ratio = current.average_latency.as_secs_f64() / baseline.average_latency.as_secs_f64();
        ...
        let recommendations = self.generate_recommendations(&regression_severity, &affected_metrics, latency_ratio, throughput_ratio);
    ...
    fn calculate_confidence_level(&self, baseline: &PerformanceBaseline, current: &PerformanceMeasurement) -> f64 {
    ...
    fn generate_recommendations(&self, severity: &RegressionSeverity, affected_metrics: &[String], latency_ratio: f64, throughput_ratio: f64) -> Vec<String> {
        ...
        match severity {
            RegressionSeverity::Critical => {
                ...
                recommendations.push("Check for resource exhaustion or system failures".to_string());
            }
            RegressionSeverity::Severe => {
                ...
                recommendations.push("Monitor system resources and database performance".to_string());
            }
        ...
        if affected_metrics.contains(&"latency".to_string()) {
            recommendations.push(format!("Latency increased by {:.1}%", (latency_ratio - 1.0) * 100.0));
        ...
        if affected_metrics.contains(&"throughput".to_string()) {
            recommendations.push(format!("Throughput decreased by {:.1}%", (1.0 - throughput_ratio) * 100.0));
    ...
    pub fn print_regression_report(&self, results: &[RegressionDetectionResult]) {
        ...
        for result in results {
            if result.regression_detected {
                ...
                println!("    Baseline latency: {:?} -> Current: {:?}", 
                         result.baseline_performance.average_latency,
                         result.current_performance.average_latency);
                println!("    Baseline throughput: {:.2} -> Current: {:.2}", 
                         result.baseline_performance.throughput,
                         result.current_performance.throughput);
    ...
    for result in &regression_results {
        match result.operation_name.as_str() {
            "fast_operation" => {
                ...
                assert!(result.regression_detected, "fast_operation should show regression");
                assert!(
            ...
            "medium_operation" => {
                ...
                assert!(!result.regression_detected, "medium_operation should not show regression");
            }
            "slow_operation" => {
                // Should detect severe regression (50ms -> 120ms = 140% increase)
                assert!(result.regression_detected, "slow_operation should show regression");
                assert!(
                    result.regression_severity == RegressionSeverity::Severe ||
                    result.regression_severity == RegressionSeverity::Critical,
...
async fn test_custom_threshold_regression_detection(ctx: TestContext) -> TestResult {
    ...
    if let Some(strict_result) = strict_detector.detect_regression("strict_operation") {
        println!("  Strict threshold result: {:?}", strict_result.regression_severity);
        assert!(strict_result.regression_detected, "Strict thresholds should detect small degradation");
    ...
    if let Some(lenient_result) = lenient_detector.detect_regression("strict_operation") {
        println!("  Lenient threshold result: {:?}", lenient_result.regression_severity);
        assert!(!lenient_result.regression_detected, "Lenient thresholds should not detect small degradation");
