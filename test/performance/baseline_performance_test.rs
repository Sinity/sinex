...
impl BaselineTracker {
    ...
    pub fn calculate_baseline(&mut self, operation: &str, env_info: EnvironmentInfo) -> Option<PerformanceBaseline> {
        if let Some(measurements) = self.measurements.get(operation) {
            ...
            self.baselines.insert(operation.to_string(), baseline.clone());
            Some(baseline)
        } else {
            None
        }
    ...
    pub fn print_baselines(&self) {
        ...
        for (operation, baseline) in &self.baselines {
            ...
            println!("  - Test data size: {}", baseline.environment_info.test_data_size);
            println!("  - Concurrent ops: {}", baseline.environment_info.concurrent_operations);
        }
    }

    pub fn save_baselines_to_database(&self, pool: &sqlx::PgPool) -> AnyhowResult<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for operation in operations {
        if let Some(baseline) = tracker.calculate_baseline(operation, env_info.clone()) {
            ...
            match operation {
                ...
                "primary_key_lookup" => {
                    assert!(baseline.average_latency < StdDuration::from_millis(5),
                        "Primary key lookup baseline should be < 5ms");
                    assert!(baseline.success_rate > 99.0,
                        "Primary key lookup success rate should be > 99%");
                }
                "source_based_query" => {
                    assert!(baseline.average_latency < StdDuration::from_millis(20),
                ...
                "aggregation_query" => {
                    assert!(baseline.average_latency < StdDuration::from_millis(200),
...
async fn test_establish_redis_stream_baselines(ctx: TestContext) -> TestResult {
    ...
    for operation in redis_operations {
        if let Some(baseline) = tracker.calculate_baseline(operation, env_info.clone()) {
            println!("\n✅ Redis baseline established for: {}", baseline.operation_name);
            
            // Store baseline assertions
            match operation {
                "stream_write" => {
                    assert!(baseline.average_latency < StdDuration::from_millis(10),
...
async fn test_establish_concurrent_operation_baselines(ctx: TestContext) -> TestResult {
    ...
    println!("  Configuration: {} workers, {} ops each", concurrent_workers, operations_per_worker);
        ...
        .map(|worker_id| {
            ...
            tokio::spawn(async move {
                ...
                for op_id in 0..operations_per_worker {
                    ...
                    let factory = EventFactory::new(&format!("concurrent-baseline-worker-{}", worker_id));
                    let event = factory.create_event(
    ...
    println!("  Concurrent operations completed: {} successes, {} errors", 
    ...
    if let Some(baseline) = tracker.calculate_baseline("concurrent_insertion", env_info.clone()) {
        println!("\n✅ Concurrent baseline established: {}", baseline.operation_name);
