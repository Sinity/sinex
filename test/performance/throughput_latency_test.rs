...
impl PerformanceMetrics {
    ...
    fn percentile_latency(&self, percentile: f64) -> StdDuration {
        ...
        let index = ((sorted_times.len() as f64 * percentile / 100.0) as usize).min(sorted_times.len() - 1);
        sorted_times[index]
    }
    ...
    fn print_summary(&self, test_name: &str) {
        ...
        println!("- Max latency: {:?}", self.operation_times.iter().max().unwrap_or(&StdDuration::from_millis(0)));
    }
}
...
async fn test_event_ingestion_throughput(ctx: TestContext) -> TestResult {
    ...
    println!("Testing event ingestion throughput with {} events", event_count);
    
    // Generate test events
    let test_events = generators::test_events(event_count);
    
    // Process events in batches for better throughput
    for batch in test_events.chunks(batch_size) {
        ...
        println!("Batch processed: {} success, {} failures, {:.2} ops/sec", 
    ...
    assert!(metrics.calculate_throughput() > 50.0, 
        "Ingestion throughput should be > 50 events/second");
    assert!(metrics.average_latency() < StdDuration::from_millis(100), 
        "Average ingestion latency should be < 100ms");
    assert!(metrics.percentile_latency(95.0) < StdDuration::from_millis(500), 
        "P95 latency should be < 500ms");
    assert!(metrics.error_count < event_count / 20, 
...
async fn test_event_ingestion_latency_scaling(ctx: TestContext) -> TestResult {
    ...
    for load_level in load_levels {
        ...
        let mut metrics = PerformanceMetrics::new();
        let test_events = generators::test_events(load_level);
        ...
        println!("Load level {}: avg latency {:?}, throughput {:.2} ops/sec", 
            load_level, avg_latency, throughput);
    }
    
    println!("\nLatency Scaling Results:");
    for (load, (latency, throughput)) in &results {
        println!("Load {}: {:?} latency, {:.2} ops/sec", load, latency, throughput);
    }
    ...
    assert!(high_load_latency < low_load_latency * 5, 
...
async fn test_database_query_performance(ctx: TestContext) -> TestResult {
    ...
    let query_tests = vec![
        ("Simple ID lookup", "SELECT * FROM core.events WHERE event_id = $1::uuid"),
        ("Source filter", "SELECT * FROM core.events WHERE source = $1"),
        ("Event type filter", "SELECT * FROM core.events WHERE event_type = $1"),
        ("Time range query", "SELECT * FROM core.events WHERE ts_orig >= $1 AND ts_orig <= $2"),
        ("Payload JSON query", "SELECT * FROM core.events WHERE payload @> $1::jsonb"),
    ...
    for (test_name, query) in query_tests {
        ...
        println!("  - Success rate: {:.2}%", (metrics.success_count as f64 / (metrics.success_count + metrics.error_count) as f64) * 100.0);
        
        // Query performance assertions
        assert!(metrics.average_latency() < StdDuration::from_millis(50), 
            "Average query latency should be < 50ms for {}", test_name);
        assert!(metrics.percentile_latency(95.0) < StdDuration::from_millis(200), 
            "P95 query latency should be < 200ms for {}", test_name);
        assert!(metrics.error_count == 0, 
            "All queries should succeed for {}", test_name);
    }
...
async fn test_concurrent_access_performance(ctx: TestContext) -> TestResult {
    ...
    // Performance assertions
    assert!(final_metrics.calculate_throughput() > 100.0, 
        "Concurrent throughput should be > 100 ops/sec");
    assert!(final_metrics.average_latency() < StdDuration::from_millis(200), 
        "Average latency under concurrent load should be < 200ms");
    assert!(final_metrics.percentile_latency(99.0) < StdDuration::from_secs(1), 
        "P99 latency should be < 1 second");
    assert!(final_metrics.error_count < total_operations / 10, 
...
async fn test_stream_processing_performance(ctx: TestContext) -> TestResult {
    ...
    // Performance assertions
    assert!(write_metrics.calculate_throughput() > 200.0, 
        "Stream write throughput should be > 200 messages/sec");
    assert!(read_metrics.calculate_throughput() > 150.0, 
        "Stream read throughput should be > 150 messages/sec");
    assert!(write_metrics.average_latency() < StdDuration::from_millis(10), 
        "Average write latency should be < 10ms");
    assert!(read_metrics.average_latency() < StdDuration::from_millis(100), 
...
async fn test_end_to_end_performance(ctx: TestContext) -> TestResult {
    ...
    println!("Testing end-to-end performance with {} workflows", workflow_count);
    ...
    for workflow_id in 0..workflow_count {
        ...
        if !workflow_success {
            println!("Workflow {} failed: ingestion={}, verification={}", 
                workflow_id, ingestion_result.is_ok(), verification_result.is_ok());
        }
    }
    ...
    // Performance assertions
    assert!(workflow_metrics.calculate_throughput() > 20.0, 
        "End-to-end throughput should be > 20 workflows/sec");
    assert!(workflow_metrics.average_latency() < StdDuration::from_millis(500), 
        "Average end-to-end latency should be < 500ms");
    assert!(workflow_metrics.percentile_latency(95.0) < StdDuration::from_secs(1), 
        "P95 end-to-end latency should be < 1 second");
    assert!(workflow_metrics.error_count < workflow_count / 20, 
        "End-to-end error rate should be < 5%");
    assert_eq!(total_events.count.unwrap_or(0), workflow_metrics.success_count as i64,
