...
async fn test_database_insertion_performance() -> anyhow::Result<()> {
    ...
    assert!(insertion_rate >= 100.0, "Insertion rate too low: {:.2} events/sec", insertion_rate);
    assert!(db_count >= (total_inserted * 95 / 100), "Too many events lost: {} inserted, {} in DB", total_inserted, db_count);
