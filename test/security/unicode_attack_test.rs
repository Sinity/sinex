...
async fn test_unicode_homograph_attacks(ctx: TestContext) -> TestResult {
    ...
    let homograph_pairs = vec![
        ...
        // Other confusables
        ("file.exe", "fiⅼe.exe", "Small Roman numeral 'ⅼ' looks like 'l'"),
        ("google.com", "goog1e.com", "Number '1' looks like letter 'l'"),
        ("microsoft", "rniсrosoft", "'rn' looks like 'm', Cyrillic 'с'"),
...
async fn test_unicode_normalization_attacks(ctx: TestContext) -> TestResult {
    ...
    let normalization_tests = vec![
        // NFD vs NFC
        ("café", "cafe\u{0301}", "NFC vs NFD (combining acute accent)"),
...
async fn test_zero_width_character_attacks(ctx: TestContext) -> TestResult {
    ...
    let zero_width_tests = vec![
        ...
        ("secret", "se\u{FEFF}cret", "Zero-width no-break space (U+FEFF)"),
        
        // Multiple zero-width characters
        ("data", "d\u{200B}a\u{200C}t\u{200D}a", "Multiple zero-width chars"),
    ...
    for (clean, injected, description) in zero_width_tests {
        ...
        println!("  Clean: {:?} (len={}, bytes={})", clean, clean.len(), clean.as_bytes().len());
        println!("  Injected: {:?} (len={}, bytes={})", injected, injected.len(), injected.as_bytes().len());
...
async fn test_direction_override_attacks(ctx: TestContext) -> TestResult {
    ...
    let direction_tests = vec![
        ...
        ("file.txt", "file\u{202E}txt.exe", "RLO hides real extension"),
        ("invoice.pdf", "invoice\u{202E}fdp.exe", "RLO makes exe look like pdf"),
        ...
        ("user@domain.com", "user@\u{202E}moc.niamod", "Email spoofing with RLO"),
    ];
...
async fn test_combined_unicode_attacks(ctx: TestContext) -> TestResult {
    ...
    let combined_attacks = vec![
        ...
        ("PayPal", "Р\u{200B}ay\u{202E}laP\u{200C}al", "Homograph + ZW + RLO + ZWNJ"),
    ];
    ...
    for (legitimate, attack, description) in combined_attacks {
        ...
        println!("  Legitimate: {:?}", legitimate);
        println!("  Attack: {:?} (len={}, bytes={})", attack, attack.len(), attack.as_bytes().len());
...
fn contains_mixed_scripts(s: &str) -> bool {
    ...
    let scripts: std::collections::HashSet<Script> = s.chars()
        .filter(|c| c.is_alphabetic())
        .map(|c| c.script())
        .collect();
    
    // Common script doesn't count as mixing
    let meaningful_scripts = scripts.into_iter()
...
fn contains_confusable_chars(s: &str) -> bool {
    ...
    s.chars().any(|c| confusables.iter().any(|(conf, _)| *conf == c))
}
