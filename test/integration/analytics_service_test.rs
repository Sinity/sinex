...
async fn create_analytics_test_event(
    ...
    let mut builder = RawEventBuilder::new(source, event_type, payload_content)
        .with_host("analytics-test-host");
...
async fn test_get_event_count_by_source_no_time_filter(ctx: TestContext) -> TestResult {
    ...
    assert_eq!(counts.get("shell.kitty"), Some(&19), "Shell events should be 19 (8+5+3+2+1)");
    assert_eq!(counts.get("wm.hyprland"), Some(&3), "Window manager events should be 3");
    assert_eq!(counts.get("clipboard"), Some(&1), "Clipboard events should be 1");
...
async fn test_get_event_count_by_source_with_time_filter(ctx: TestContext) -> TestResult {
    ...
    let counts = service.get_event_count_by_source(Some(one_hour_ago), Some(now)).await?;
    
    // Only recent events should be included
    assert!(counts.get("fs").unwrap_or(&0) >= &2, "Should have some recent filesystem events");
    assert!(counts.get("shell.kitty").unwrap_or(&0) >= &5, "Should have recent shell events");
    assert!(counts.get("wm.hyprland").unwrap_or(&0) >= &1, "Should have recent window events");
    
    // Old system event should not be included
    assert_eq!(counts.get("system"), None, "System events should not be in recent timeframe");
...
async fn test_get_event_count_by_type_no_time_filter(ctx: TestContext) -> TestResult {
    ...
    assert_eq!(counts.get("file.created"), Some(&5), "file.created events should be 5");
    assert_eq!(counts.get("command.executed"), Some(&19), "command.executed events should be 19");
    assert_eq!(counts.get("window.opened"), Some(&3), "window.opened events should be 3");
    assert_eq!(counts.get("copied"), Some(&1), "copied events should be 1");
    assert_eq!(counts.get("boot.completed"), Some(&1), "boot.completed events should be 1");
...
async fn test_get_event_count_by_type_with_time_filter(ctx: TestContext) -> TestResult {
    ...
    let counts = service.get_event_count_by_type(Some(two_hours_ago), Some(now)).await?;
    
    // Should have recent events but not old system events
    assert!(counts.get("file.created").unwrap_or(&0) >= &3, "Should have recent file events");
    assert!(counts.get("command.executed").unwrap_or(&0) >= &10, "Should have recent command events");
    
    // Old boot event should not be included
    assert_eq!(counts.get("boot.completed"), None, "Boot events should not be in recent timeframe");
...
async fn test_get_events_over_time_hourly_intervals(ctx: TestContext) -> TestResult {
    ...
    let time_series = service.get_events_over_time(three_hours_ago, now, 60).await?;
    ...
    for window in time_series.windows(2) {
        ...
        assert!(prev.0 <= curr.0, "Time buckets should be in ascending order");
    }
    
    // Verify total count matches expected recent events
    let total_events: i64 = time_series.iter().map(|(_, count)| count).sum();
    assert!(total_events >= 20, "Should have reasonable number of events in timeframe");
...
async fn test_get_events_over_time_different_intervals(ctx: TestContext) -> TestResult {
    ...
    let two_hour_data = service.get_events_over_time(six_hours_ago, now, 120).await?;
    
    // Smaller intervals should have more buckets
    assert!(thirty_min_data.len() >= two_hour_data.len(), 
    ...
    assert_eq!(thirty_min_total, two_hour_total, "Total event counts should match across different intervals");
...
async fn test_get_top_commands_no_time_filter(ctx: TestContext) -> TestResult {
    ...
    assert_eq!(top_commands[0], ("git status".to_string(), 8), "Most frequent should be git status");
    assert_eq!(top_commands[1], ("cargo build".to_string(), 5), "Second should be cargo build");
    assert_eq!(top_commands[2], ("ls -la".to_string(), 3), "Third should be ls -la");
    assert_eq!(top_commands[3], ("cd /home".to_string(), 2), "Fourth should be cd /home");
    assert_eq!(top_commands[4], ("vim file.rs".to_string(), 1), "Fifth should be vim file.rs");
...
async fn test_get_top_commands_with_time_filter(ctx: TestContext) -> TestResult {
    ...
    let recent_commands = service.get_top_commands(Some(thirty_minutes_ago), Some(now), 10).await?;
    
    // Should have fewer commands due to time filtering
    assert!(!recent_commands.is_empty(), "Should have some recent commands");
    ...
    for (command, count) in &recent_commands {
        ...
        assert!(count >= &1, "Each command should have at least 1 occurrence");
...
async fn test_analytics_with_single_event(ctx: TestContext) -> TestResult {
    ...
    assert_eq!(source_counts.get("test.source"), Some(&1), "Source should have count 1");
    
    let type_counts = service.get_event_count_by_type(None, None).await?;
    assert_eq!(type_counts.len(), 1, "Should have exactly one event type");
    assert_eq!(type_counts.get("test.event"), Some(&1), "Event type should have count 1");
...
async fn test_analytics_time_range_edge_cases(ctx: TestContext) -> TestResult {
    ...
    let source_counts = service.get_event_count_by_source(
        Some(exactly_one_hour_ago), 
        Some(now)
    ).await?;
    
    // Should include the boundary event
    assert_eq!(source_counts.get("boundary.test").unwrap_or(&0), &1, 
    ...
    let source_counts_excluded = service.get_event_count_by_source(
        Some(fifty_minutes_ago), 
        Some(now)
    ).await?;
...
async fn test_get_top_commands_only_command_events(ctx: TestContext) -> TestResult {
    ...
    assert_eq!(top_commands.len(), 1, "Should only find command.executed events");
    assert_eq!(top_commands[0], ("test command".to_string(), 1), "Should find the test command");
...
async fn test_analytics_aggregation_accuracy(ctx: TestContext) -> TestResult {
    ...
    for (i, source) in test_sources.iter().enumerate() {
        for (j, event_type) in test_types.iter().enumerate() {
            ...
            *expected_source_counts.entry(source.to_string()).or_insert(0) += count as i64;
            *expected_type_counts.entry(event_type.to_string()).or_insert(0) += count as i64;
        }
    }
    ...
    for (source, expected_count) in expected_source_counts {
        assert_eq!(source_counts.get(&source), Some(&expected_count), 
            "Source {} should have exactly {} events", source, expected_count);
    }
    
    for (event_type, expected_count) in expected_type_counts {
        assert_eq!(type_counts.get(&event_type), Some(&expected_count), 
            "Event type {} should have exactly {} events", event_type, expected_count);
    }
...
async fn test_activity_heatmap_legacy_method(ctx: TestContext) -> TestResult {
    ...
    for window in heatmap.windows(2) {
        let (prev, curr) = (&window[0], &window[1]);
        assert!(prev.1 >= curr.1, "Heatmap should be ordered by count descending");
    }
    
    // All counts should be positive
    for (timestamp, count) in &heatmap {
        assert!(count > &0, "All activity counts should be positive");
        assert!(timestamp <= &Utc::now(), "All timestamps should be in the past");
    }
...
async fn test_analytics_large_dataset_performance(ctx: TestContext) -> TestResult {
    ...
    assert!(analytics_duration.as_secs() < 1, "Analytics should complete quickly");
