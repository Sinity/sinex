...
async fn test_version_tracking_performance(_ctx: TestContext) -> TestResult {
    ...
    assert!(elapsed.as_secs() < 5, 
        "Version tracking took too long: {:?}", elapsed);
...
async fn test_repeated_version_info_generation(_ctx: TestContext) -> TestResult {
    ...
    assert!(total_duration.as_secs() < 5, 
        "Version info generation too slow: {:?}", total_duration);
...
async fn test_filesystem_monitor_large_directory(_ctx: TestContext) -> TestResult {
    ...
    for i in 0..100 {
        fs::write(temp_path.join(format!("file_{}.txt", i)), format!("content {}", i))?;
    }
    ...
    assert!(memory_growth < 50 * 1024 * 1024, 
        "Memory grew too much during monitor creation: {} bytes", memory_growth);
