...
async fn test_git_annex_hash_deduplication(ctx: TestContext) -> TestResult {
    ...
    let metadata1 = blob_manager.ingest_file(&test_file, Some("first_import")).await?;
    ...
    let metadata2 = blob_manager.ingest_file(&duplicate_file, Some("second_import")).await?;
    ...
    info!(
        "Both files should have the same hash - demonstrating deduplication at git-annex level"
    );
...
async fn test_atuin_import_overlap_detection(ctx: TestContext) -> TestResult {
    ...
    create_test_atuin_db(&db_path, vec![
    ...
    let source_ctx = EventSourceContext::new(config.clone())
        .with_db_pool(ctx.pool().clone());
    ...
    let scanner_args = ScannerArgs {
        paths: vec![db_path.to_string_lossy().to_string()],
        dry_run: false,
    ...
    let source_ctx2 = EventSourceContext::new(config)
        .with_db_pool(ctx.pool().clone());
    ...
    for (command, count) in all_commands {
        ...
        assert!(count <= 2, "Command '{}' appeared {} times - possible duplicate", command, count);
    }
...
"#;
    ...
    let (tx1, mut rx1) = tokio::sync::mpsc::channel(100);
    let scanner_args1 = ScannerArgs {
        paths: vec![history_file.to_string_lossy().to_string()],
        dry_run: false,
        interactive: false,
    ...
    let scanner_args2 = ScannerArgs {
        paths: vec![history_file.to_string_lossy().to_string()],
        dry_run: false,
        interactive: false,
        time_range: Some((start_time2, end_time2)),
    };
    ...
    assert_ne!(events1.len(), events2.len(), "Different time ranges should yield different event counts");
...
async fn test_overlap_analysis_statistics(ctx: TestContext) -> TestResult {
    ...
    for i in 0..100 {
        zsh_content.push_str(&format!(": {}:0;echo 'command {}'\n", 1640995200 + i * 60, i));
    }
    ...
    for i in 50..150 { // 50 overlapping + 50 new
        additional_content.push_str(&format!(": {}:0;echo 'command {}'\n", 1640995200 + i * 60, i));
    }
    fs::write(&history_file, format!("{}{}", zsh_content, additional_content))?;
    ...
    assert_eq!(existing_count, 100, "Should have 100 existing events from first import");
    
    // Count total lines in file for potential import
    let file_content = fs::read_to_string(&history_files[0])?;
    let total_lines = file_content.lines()
...
async fn test_cross_scanner_deduplication(ctx: TestContext) -> TestResult {
    ...
    create_test_atuin_db(&atuin_db, vec![
        create_atuin_entry("cross-test-1", command, timestamp),
    ])?;
    ...
    let atuin_ctx = EventSourceContext::new(atuin_config)
        .with_db_pool(ctx.pool().clone());
    ...
    let atuin_args = ScannerArgs {
        paths: vec![], // Will use smart defaults
        dry_run: false,
        interactive: false,
        time_range: None,
    };
    ...
    let shell_ctx = EventSourceContext::new(shell_config)
        .with_db_pool(ctx.pool().clone());
