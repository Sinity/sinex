...
use sinex_services::{SearchService, SearchQuery};
use sinex_ulid::Ulid;
...
async fn setup_test_data(pool: &DbPool) -> TestResult<Vec<Ulid>> {
...
async fn test_search_sql_injection_prevention(ctx: TestContext) -> TestResult {
    ...
    for pattern in injection_patterns {
        ...
        let count = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM raw.events")
            .fetch_one(pool)
            .await?;
        assert!(count > 0, "SQL injection may have deleted data with pattern: {}", pattern);
    }
...
async fn test_search_sql_injection_in_filters(ctx: TestContext) -> TestResult {
    ...
    let result = service.search_events(query).await;
    assert!(result.is_ok(), "Event type filter injection protection failed");
...
async fn test_search_source_filtering(ctx: TestContext) -> TestResult {
    ...
    assert!(results.iter().all(|r| r.source == "fs" || r.source == "clipboard"));
...
async fn test_search_special_characters_in_text(ctx: TestContext) -> TestResult {
    ...
    assert_eq!(results.len(), 1, "Underscore should be treated literally, not as SQL wildcard");
