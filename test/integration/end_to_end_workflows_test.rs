...
use sinex_core::{RawEventBuilder, CoreError};
...
async fn test_complete_event_ingestion_workflow(ctx: TestContext) -> TestResult {
    ...
    for event_id in &ingested_event_ids {
        let event = sqlx::query!(
            "SELECT event_id::uuid as \"id!\", source, event_type, ts_orig, host, payload FROM core.events WHERE event_id::uuid = $1::uuid",
            event_id.to_uuid()
        )
        .fetch_one(pool)
        .await?;
    ...
    assert_eq!(stored_events.len(), satellite_events.len(), "All events should be stored");
    ...
    for (original, stored) in satellite_events.iter().zip(stored_events.iter()) {
        ...
        assert_eq!(original.event_type, stored.event_type, "Event type should match");
    ...
    let time_range_events = sqlx::query!(
        "SELECT COUNT(*) as count FROM core.events WHERE ts_orig >= $1 AND ts_orig <= $2",
    ...
    assert!(time_range_events.count.unwrap_or(0) >= satellite_events.len() as i64, 
...
async fn test_concurrent_satellite_ingestion_workflow(ctx: TestContext) -> TestResult {
        ...
        let handle = tokio::spawn(async move {
            ...
            for event_id in 0..events_per_satellite {
                let event = RawEventBuilder::new()
                    ...
                    .event_type(&format!("satellite.event.{}", event_id))
                    .host(&format!("host-{}", satellite_id))
                ...
                match sinex_db::events::insert_event_with_validator(&pool_clone, &event, None).await {
                    ...
                    Err(e) => {
                        ...
                        println!("Satellite {} failed to ingest event {}: {}", satellite_id, event_id, e);
                    }
                }
    ...
    assert!(successes.len() + failures.len() == expected_total, 
        "All ingestion attempts should be accounted for");
    
    // Verify database state
    let total_events = sqlx::query!(
        "SELECT COUNT(*) as count FROM core.events WHERE source LIKE 'satellite-%'"
    )
    .fetch_one(pool)
    .await?;
    
    assert_eq!(total_events.count.unwrap_or(0), successes.len() as i64,
        "Database should contain all successful ingestions");
    
    // Verify no data corruption with concurrent writes
    let distinct_event_ids = sqlx::query!(
        "SELECT COUNT(DISTINCT id) as count FROM core.events WHERE source LIKE 'satellite-%'"
    )
    .fetch_one(pool)
    .await?;
    
    assert_eq!(distinct_event_ids.count.unwrap_or(0), successes.len() as i64,
...
async fn test_stream_processing_workflow(ctx: TestContext) -> TestResult {
    ...
    let checkpoint_manager = CheckpointManager::new(pool.clone(), automaton_name.to_string());
    ...
    while processed_count < test_events.len() {
        ...
        match redis_client.xreadgroup(
            ...
            Ok(messages) => {
                ...
                for message in messages {
                    ...
                    // Acknowledge message
                    match redis_client.xack(stream_key, &consumer_group, &message.id).await {
        ...
        let checkpoint_state = CheckpointState {
            last_processed_id: Some(format!("stream-{}", processed_count)),
    ...
    assert!(final_checkpoint.processed_count > 0, "Checkpoint should track progress");
...
async fn test_checkpoint_persistence_recovery_workflow(ctx: TestContext) -> TestResult {
    ...
    let checkpoint_states = vec![
        CheckpointState {
            last_processed_id: Some("event-1".to_string()),
            processed_count: 1,
            data: Some(serde_json::json!({"phase": "initial", "timestamp": Utc::now()})),
        },
        CheckpointState {
            last_processed_id: Some("event-10".to_string()),
            processed_count: 10,
            data: Some(serde_json::json!({"phase": "batch_1", "timestamp": Utc::now()})),
        },
        CheckpointState {
            last_processed_id: Some("event-25".to_string()),
            processed_count: 25,
    ...
    for (i, state) in checkpoint_states.iter().enumerate() {
        match checkpoint_manager.save_checkpoint(state).await {
            Ok(_) => {
                println!("Saved checkpoint {}: {} events processed", i + 1, state.processed_count);
            }
            Err(e) => {
    ...
    assert_eq!(retrieved_checkpoint.processed_count, final_state.processed_count,
        "Retrieved checkpoint should match final state");
    assert_eq!(retrieved_checkpoint.last_processed_id, final_state.last_processed_id,
        "Last processed ID should match");
    
    // Phase 4: Simulate automaton restart and recovery
    let recovery_manager = CheckpointManager::new(pool.clone(), automaton_name.to_string());
    let recovery_checkpoint = recovery_manager.load_checkpoint().await?;
    
    println!("Recovery checkpoint:");
    println!("- Last processed ID: {:?}", recovery_checkpoint.last_processed_id);
    ...
    assert_eq!(recovery_checkpoint.processed_count, 25, "Recovery should start from last checkpoint");
    
    // Phase 5: Continue processing from checkpoint
    let continued_state = CheckpointState {
        last_processed_id: Some("event-50".to_string()),
    ...
    assert_eq!(final_recovery_checkpoint.processed_count, 50,
...
async fn test_multi_component_coordination_workflow(ctx: TestContext) -> TestResult {
        ...
        let handle = tokio::spawn(async move {
            ...
            for i in 0..10 {
                let event = RawEventBuilder::new()
                    ...
                    .event_type(&format!("{}.heartbeat", component_name))
                    .host("coordination-test")
                    .payload(serde_json::json!({
    ...
    for component in &components {
        let heartbeat_count = sqlx::query!(
            "SELECT COUNT(*) as count FROM core.events WHERE source = $1 AND event_type = $2",
            component,
        ...
        assert_eq!(heartbeat_count.count.unwrap_or(0), 10,
...
async fn test_error_recovery_workflow(ctx: TestContext) -> TestResult {
    ...
    for operation_id in 0..operation_count {
        ...
        if should_fail {
            ...
            *error_count_lock += 1;
            ...
            println!("Operation {} encountered error, attempting recovery", operation_id);
            ...
            for retry in 0..3 {
                ...
                let event = RawEventBuilder::new()
                    .source(component_name)
                    .event_type(&format!("recovery.attempt.{}", retry))
                    .host("error-recovery-test")
                    .payload(serde_json::json!({
                ...
                match sinex_db::events::insert_event_with_validator(&pool_clone, &event, None).await {
                    Ok(_) => {
                        ...
                        println!("Recovery attempt {} for operation {} succeeded", retry, operation_id);
                    ...
                    Err(e) => {
                        println!("Recovery attempt {} for operation {} failed: {}", retry, operation_id, e);
                    }
                }
            }
            
            if !recovery_success {
                println!("Operation {} failed after all recovery attempts", operation_id);
            }
        } else {
            // Normal operation
            let event = RawEventBuilder::new()
    ...
    let recovery_events = sqlx::query!(
        "SELECT COUNT(*) as count FROM core.events WHERE source = $1 AND event_type LIKE 'recovery.attempt%'",
        component_name
    )
    .fetch_one(pool)
    .await?;
    
    let normal_events = sqlx::query!(
        "SELECT COUNT(*) as count FROM core.events WHERE source = $1 AND event_type = 'normal.operation'",
        component_name
    )
    .fetch_one(pool)
    .await?;
    
    assert!(recovery_events.count.unwrap_or(0) > 0, "Recovery events should be recorded");
    assert!(normal_events.count.unwrap_or(0) > 0, "Normal operations should be recorded");
    ...
    assert!(total_events.count.unwrap_or(0) > operation_count as i64 / 2,
...
async fn test_performance_under_load_workflow(ctx: TestContext) -> TestResult {
        ...
        .map(|worker_id| {
            ...
            tokio::spawn(async move {
                ...
                for event_id in 0..events_per_worker {
                    ...
                    let event = RawEventBuilder::new()
                        .source(&format!("load-worker-{}", worker_id))
                        .event_type("performance.load.event")
    ...
    assert!(successes >= (total_expected_events * 95) / 100,
        "At least 95% of events should succeed under load");
    assert!(throughput > 10.0, "Throughput should be > 10 events/second");
    assert!(average_latency < StdDuration::from_millis(1000),
        "Average latency should be < 1 second");
    
    // Phase 7: Verify database consistency under load
    let total_db_events = sqlx::query!(
        "SELECT COUNT(*) as count FROM core.events WHERE source LIKE 'load-worker-%'"
    )
    .fetch_one(pool)
    .await?;
    
    assert_eq!(total_db_events.count.unwrap_or(0), successes as i64,
...
async fn test_data_consistency_workflow(ctx: TestContext) -> TestResult {
    ...
    println!("Testing data consistency across {} components", component_count);
    ...
    for chain_id in 0..events_per_component {
        ...
        for component_id in 0..component_count {
            let event = RawEventBuilder::new()
                .source(&format!("consistency-component-{}", component_id))
                .event_type(&format!("consistency.chain.{}", chain_id))
                .host("consistency-test")
                .ts_orig(base_timestamp + Duration::seconds(chain_id as i64 * 10 + component_id as i64))
                .payload(serde_json::json!({
                    "chain_id": chain_id,
    ...
    for (chain_id, chain_events) in event_chains.iter().enumerate() {
        ...
        for event in chain_events {
            match sinex_db::events::insert_event_with_validator(pool, event, None).await {
                Ok(_) => {
                    println!("Inserted event for chain {} from component {}", 
    ...
    for event in temporal_check {
        ...
        chain_groups.entry(chain_id).or_insert_with(Vec::new).push(event);
    }
    ...
    for (chain_id, events) in chain_groups {
        for i in 1..events.len() {
            ...
            if prev.ts_orig >= curr.ts_orig {
                ...
                println!("Temporal violation in chain {}: {} >= {}", 
    ...
    for event in referential_check {
        if let Some(prev_id) = event.previous_event_id {
            let prev_exists = sqlx::query!(
                "SELECT COUNT(*) as count FROM core.events WHERE event_id::uuid = $1::uuid",
                prev_id.parse::<sinex_ulid::Ulid>().unwrap_or_default().to_uuid()
            )
            .fetch_one(pool)
            .await?;
            
            if prev_exists.count.unwrap_or(0) == 0 {
                referential_violations += 1;
                println!("Referential violation: event {} references non-existent {}", 
                    event.id, prev_id);
            }
        }
    }
    ...
    assert!(consistency_violations.len() < events_per_component / 10,
        "Consistency violations should be minimal");
    assert_eq!(temporal_violations, 0, "No temporal violations should occur");
    assert_eq!(referential_violations, 0, "No referential violations should occur");
    assert!(successful_chains >= (events_per_component * 90) / 100,
