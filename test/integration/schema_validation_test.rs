...
use serde_json::json;
use sinex_db::events::insert_event;
...
async fn test_event_validation_with_different_payload_types(ctx: TestContext) -> TestResult {
    let test_cases = vec![
        ("fs", "file.created", json!({"path": "/test/file.txt", "size": 1024})),
        ("shell", "command.executed", json!({"command": "ls -la", "exit_code": 0})),
        ("wm", "window.focused", json!({"window_id": 12345, "title": "Test Window"})),
    ];
...
async fn test_unicode_payload_handling(ctx: TestContext) -> TestResult {
    ...
    assert_eq!(inserted.payload["message"], "Hello 🌍! Test with émojis and spëcial chars");

    Ok(())
}

/// Helper function to check if error contains expected message
fn assert_error_contains(result: &Result<impl std::fmt::Debug, impl std::fmt::Display>, expected: &str) {
    match result {
