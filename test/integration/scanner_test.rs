...
async fn test_filesystem_scanner_basic_functionality(ctx: TestContext) -> TestResult {
    ...
    let source_ctx = EventSourceContext::new(config)
...
"#;
    ...
    assert!(report.source_stats.contains_key("shell_bash_entries") || 
...
async fn test_scanner_dry_run_mode(ctx: TestContext) -> TestResult {
    ...
    assert!(rx.try_recv().is_err(), "No events should be sent in dry run mode");
...
async fn test_scanner_performance_large_directory(ctx: TestContext) -> TestResult {
    ...
    for i in 0..100 {
        fs::write(temp_path.join(format!("file_{:03}.txt", i)), format!("content_{}", i))?;
    }
    ...
    assert!(elapsed.as_secs() < 5, "Scanner should complete within 5 seconds");
    assert!(report.duration.as_millis() > 0);
