...
async fn test_systemd_watchdog_failure_handling(ctx: TestContext) -> TestResult {
    ...
    assert!(
        ...
        "Watchdog should fail after simulated failure"
    );
    ...
    pretty_assertions::assert_eq!(
        events.len(),
        4,
        "Should have 4 recorded events (failed watchdog not recorded)"
    );
...
async fn test_security_hardening_options(ctx: TestContext) -> TestResult {
    ...
    assert!(security_config
        ...
        .contains(&"AF_UNIX".to_string()));
    assert!(security_config
        .restrict_address_families
        .contains(&"AF_INET".to_string()));
