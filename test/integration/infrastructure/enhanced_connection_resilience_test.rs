...
async fn test_connection_pool_under_sustained_pressure(ctx: TestContext) -> TestResult {
    ...
    for (worker_id, pattern) in worker_configs {
        ...
        let handle = tokio::spawn(async move { worker.run(test_duration).await });
...
async fn test_connection_pool_failure_recovery_cycles(_ctx: TestContext) -> TestResult {
    ...
    for cycle in 0..recovery_cycles {
        ...
        let worker_handle =
            tokio::spawn(async move { worker.run(Duration::from_millis(800)).await });
    ...
    println!(
        "  Completed cycles: {}/{}",
        total_recoveries, recovery_cycles
    );
    println!("  {}", metrics.report());
...
async fn test_connection_pool_deadlock_detection_and_resolution(_ctx: TestContext) -> TestResult {
    ...
    for worker_id in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            ...
            for attempt in 0..2 {
                match timeout(Duration::from_millis(400), pool.acquire()).await {
                    Ok(Ok(conn)) => {
                        metrics.add_active_connection();
                        acquired_conns.push(conn);
                        println!(
                            "Worker {} acquired connection {} at {:?}",
                            worker_id,
                            attempt + 1,
                            start.elapsed()
                        );
                    ...
                    Err(_) => {
                        println!(
                            "Worker {} timed out acquiring connection {} - potential deadlock",
                            worker_id,
                            attempt + 1
                        );
    ...
    let monitor_handle = tokio::spawn(async move {
        ...
        while check_count < 20 {
            ...
            println!(
                "Monitor check {}: {} active connections, deadlock: {}, resolved: {}",
    ...
    assert!(
        was_resolved,
        "Should resolve deadlock through timeout and cleanup"
    );
...
async fn test_connection_pool_cascade_failure_recovery(_ctx: TestContext) -> TestResult {
    ...
    let failure_sequence = vec![
        ...
        (
            "Phase 2: Introduce connection timeouts",
            true,
            Duration::from_millis(500),
        ),
        (
            "Phase 3: Amplify failures",
            true,
            Duration::from_millis(500),
        ),
        (
    ...
    for (phase_name, introduce_failures, phase_duration) in failure_sequence {
        ...
        let workers = vec![
            ResilientDbWorker::new(
                "cascade_simple".to_string(),
                pool.clone(),
                metrics.clone(),
                QueryPattern::Simple,
            ),
            ResilientDbWorker::new(
                "cascade_readwrite".to_string(),
                pool.clone(),
                metrics.clone(),
                QueryPattern::ReadWrite,
            ),
        ];
        ...
        for worker in workers {
            let handle = tokio::spawn(async move { worker.run(phase_duration).await });
    ...
    println!(
        "  Connection errors during failure phases: {}",
        metrics.connection_errors.load(Ordering::Relaxed)
    );
    println!(
        "  Timeout errors during failure phases: {}",
    ...
    assert!(
        metrics.connection_errors.load(Ordering::Relaxed) > 0,
        "Should have experienced connection errors during failure phases"
    );
    assert!(
        metrics.successful_queries.load(Ordering::Relaxed) > 0,
...
async fn test_connection_pool_memory_pressure_resilience(ctx: TestContext) -> TestResult {
    ...
    let memory_handle =
        tokio::spawn(async move { memory_test_worker.run(Duration::from_secs(3)).await });
