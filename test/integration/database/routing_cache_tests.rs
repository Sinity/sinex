...
use crate::common::{create_agent_with_subscriptions, insert_event, test_event_with_payload};
...
async fn test_routing_cache_view_exists(ctx: TestContext) -> TestResult {
    ...
    pretty_assertions::assert_eq!(
        view_exists.count.unwrap_or(0),
        1,
        "routing_cache materialized view should exist"
    );
    Ok(())
}
...
async fn test_routing_cache_structure(ctx: TestContext) -> TestResult {
    ...
    pretty_assertions::assert_eq!(
        columns.len(),
        2,
        "routing_cache should have exactly 2 columns"
    );
...
async fn test_routing_cache_auto_refresh_on_agent_change(ctx: TestContext) -> TestResult {
    ...
    pretty_assertions::assert_eq!(
        cache_entries.len(),
        3,
        "Should have 3 routing cache entries"
    );

    // Verify the specific entries
    pretty_assertions::assert_eq!(
        cache_entries[0].event_type.as_ref(),
        Some(&"filesystem:file_created".to_string())
    );
    pretty_assertions::assert_eq!(
        cache_entries[0].agent_id.as_ref(),
        Some(&agent_name.to_string())
    );

    pretty_assertions::assert_eq!(
        cache_entries[1].event_type.as_ref(),
        Some(&"filesystem:file_modified".to_string())
    );
    pretty_assertions::assert_eq!(
        cache_entries[1].agent_id.as_ref(),
        Some(&agent_name.to_string())
    );

    pretty_assertions::assert_eq!(
        cache_entries[2].event_type.as_ref(),
        Some(&"terminal:command_executed".to_string())
    );
    pretty_assertions::assert_eq!(
        cache_entries[2].agent_id.as_ref(),
        Some(&agent_name.to_string())
    );
...
async fn test_batch_router_creates_work_queue_entries(ctx: TestContext) -> TestResult {
    ...
    let event1_id = insert_event(
        pool,
        &test_event_with_payload("test_source", "test_event", json!({"data": "test data 1"})),
    )
    .await?;
    let event2_id = insert_event(
        pool,
        &test_event_with_payload("test_source", "test_event", json!({"data": "test data 2"})),
    )
    .await?;
    ...
    let event_ids: Vec<uuid::Uuid> = work_items
        .into_iter()
...
async fn test_batch_router_avoids_duplicate_routing(ctx: TestContext) -> TestResult {
    ...
    let event_id = insert_event(
        pool,
        &test_event_with_payload("test_source", "test_event", json!({"data": "dedup test"})),
    )
    .await?;
    ...
    pretty_assertions::assert_eq!(
        work_items.count.unwrap_or(0),
        1,
        "Should have exactly 1 work queue item"
    );
...
async fn test_routing_cache_performance_over_triggers(ctx: TestContext) -> TestResult {
    ...
    assert!(
        ...
        "Batch routing should complete quickly"
    );
