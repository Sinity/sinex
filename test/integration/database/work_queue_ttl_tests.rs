...
async fn test_ttl_policy_purges_old_succeeded_items(ctx: TestContext) -> Result<(), anyhow::Error> {
    ...
    pretty_assertions::assert_eq!(
        old_item_exists.count.unwrap(),
        0,
        "Old item should be purged"
    );
    ...
    pretty_assertions::assert_eq!(
        recent_item_exists.count.unwrap(),
        1,
...
async fn test_ttl_policy_purges_old_failed_items(ctx: TestContext) -> Result<(), anyhow::Error> {
    ...
    // Create test event
    let event =
        RawEventBuilder::new("test_source", "test_event", json!({"test": "old_failed"})).build();
...
async fn test_ttl_policy_keeps_pending_items(ctx: TestContext) -> Result<(), anyhow::Error> {
    ...
    // Create test event
    let event =
        RawEventBuilder::new("test_source", "test_event", json!({"test": "old_pending"})).build();
    let event_id = insert_event(ctx.pool(), &event).await?;
...
#[sinex_test]
async fn test_ttl_policy_keeps_items_without_processed_at(
    ctx: TestContext,
) -> Result<(), anyhow::Error> {
    // Create test agent first
    crate::common::create_test_agent(ctx.pool(), "test-agent").await?;
...
async fn test_ttl_policy_respects_90_day_threshold(ctx: TestContext) -> Result<(), anyhow::Error> {
    ...
    let just_old_event =
        RawEventBuilder::new("test_source", "test_event", json!({"test": "just_old"})).build();
    let just_old_event_id = insert_event(ctx.pool(), &just_old_event).await?;

    let just_new_event =
        RawEventBuilder::new("test_source", "test_event", json!({"test": "just_new"})).build();
    ...
    pretty_assertions::assert_eq!(
        purged_count,
        1,
        "Should purge exactly 1 item at 90-day threshold"
    );
