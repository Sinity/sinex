...
async fn test_queue_depth_metric_calculation(ctx: TestContext) -> TestResult {
    ...
    pretty_assertions::assert_eq!(
        queue_metrics.len(),
        2,
        "Should have metrics for 2 agents (even if one has 0 pending)"
    );
    ...
    pretty_assertions::assert_eq!(
        sorted_metrics[0].queue_depth,
        2,
        "Agent1 should have 2 pending items"
    );

    // Check agent2 (metrics-agent-2)
    pretty_assertions::assert_eq!(sorted_metrics[1].agent_name, agent2);
    pretty_assertions::assert_eq!(
        sorted_metrics[1].queue_depth,
        0,
        "Agent2 should have 0 pending items (1 processing)"
    );
...
async fn test_dequeue_latency_metric_calculation(ctx: TestContext) -> TestResult {
    ...
    let agent_metric = latency_metrics
        .iter()
        .find(|m| m.agent_name == agent_name)
        .unwrap();
    assert!(
        agent_metric.avg_dequeue_latency_ms > 90.0,
        "Should measure at least 90ms latency"
    );
    assert!(
        agent_metric.avg_dequeue_latency_ms < 200.0,
        "Should be under 200ms latency"
    );
...
async fn test_per_agent_lag_metric_calculation(ctx: TestContext) -> TestResult {
    ...
    let slow_agent_metric = lag_metrics
        .iter()
        .find(|m| m.agent_name == slow_agent)
        .unwrap();
    let fast_agent_metric = lag_metrics
        .iter()
        .find(|m| m.agent_name == fast_agent)
        .unwrap();

    assert!(
        slow_agent_metric.max_lag_seconds > 500.0,
        "Slow agent should have significant lag"
    );
    assert!(
        fast_agent_metric.max_lag_seconds < 60.0,
        "Fast agent should have minimal lag"
    );
...
async fn test_prometheus_metrics_exposition(ctx: TestContext) -> TestResult {
    ...
    assert!(
        prometheus_output.contains("sinex_queue_depth"),
        "Should contain queue depth metric"
    );
    assert!(
        prometheus_output.contains("sinex_dequeue_latency_ms"),
        "Should contain latency metric"
    );
    assert!(
        prometheus_output.contains("sinex_agent_lag_seconds"),
        "Should contain lag metric"
    );

    // Verify agent labels
    assert!(
        prometheus_output.contains(&format!("agent_name=\"{}\"", agent_name)),
...
async fn generate_prometheus_metrics(pool: &DbPool) -> Result<String> {
    ...
    // Agent lag metrics
    output
        .push_str("# HELP sinex_agent_lag_seconds Maximum age of oldest pending event per agent\n");
