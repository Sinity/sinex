...
async fn test_satellite_event_pipeline_integration(ctx: TestContext) -> TestResult {
    ...
    // Create a mock context for the satellite
    let test_ctx = EventSourceContext {
        service_name: "test-fs".to_string(),
        host: "test-host".to_string(),
        work_dir: ctx.work_dir(),
    ...
    // Create a runner for the satellite
    let mut satellite = satellite;
    satellite.initialize(test_ctx).await?;
...
async fn test_multi_satellite_coordination(ctx: TestContext) -> TestResult {
    // Create a shared channel to collect events from both satellites
    let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel();
    ...
    // Create contexts for each satellite
    let fs_ctx = EventSourceContext {
        service_name: "test-fs".to_string(),
        host: "test-host".to_string(),
        work_dir: ctx.work_dir(),
        dry_run: false,
    ...
    let cmd_ctx = EventSourceContext {
        service_name: "test-cmd".to_string(),
        host: "test-host".to_string(),
        work_dir: ctx.work_dir(),
        dry_run: false,
        config: std::collections::HashMap::new(),
    ...
    fs_satellite.initialize(fs_ctx).await?;
    cmd_satellite.initialize(cmd_ctx).await?;
    
    // Start both satellites concurrently
    let fs_handle = tokio::spawn(async move {
        fs_satellite.start_streaming().await
    });
    
    let cmd_handle = tokio::spawn(async move {
        cmd_satellite.start_streaming().await
    });
    
    // Drop the sender to avoid blocking
    drop(tx);
    ...
    assert!(!events.is_empty(), "No events received");
    assert_eq!(events.len(), 5, "Expected 5 events total, got {}", events.len());
...
async fn test_satellite_operational_modes(ctx: TestContext) -> TestResult {
    ...
    let (scanner_tx, mut scanner_rx) = tokio::sync::mpsc::unbounded_channel();
    
    let scanner_ctx = EventSourceContext {
        service_name: "test-scanner".to_string(),
        host: "test-host".to_string(),
        work_dir: ctx.work_dir(),
        dry_run: false,
        config: std::collections::HashMap::new(),
        event_sender: scanner_tx,
    };
    
    let mut scanner_satellite = TestScannerSatellite::new(3);
    scanner_satellite.initialize(scanner_ctx).await?;
    
    // Run scanner mode (should complete naturally)
    let scanner_start = std::time::Instant::now();
    let scanner_result = scanner_satellite.start_streaming().await;
    let scanner_duration = scanner_start.elapsed();
    
    // Should complete successfully
    assert!(scanner_result.is_ok(), "Scanner mode failed: {:?}", scanner_result);
    // Should complete quickly as it's a finite operation
    assert!(scanner_duration.as_secs() < 5, "Scanner mode took too long: {:?}", scanner_duration);
    ...
    let (sensor_tx, mut sensor_rx) = tokio::sync::mpsc::unbounded_channel();
    
    let sensor_ctx = EventSourceContext {
        service_name: "test-sensor".to_string(),
        host: "test-host".to_string(),
        work_dir: ctx.work_dir(),
        dry_run: false,
        config: std::collections::HashMap::new(),
        event_sender: sensor_tx,
    };
    
    let mut sensor_satellite = TestFilesystemSatellite::new(100); // Large number to ensure continuous operation
    sensor_satellite.initialize(sensor_ctx).await?;
    
    // Start sensor mode in background
    let sensor_handle = tokio::spawn(async move {
        sensor_satellite.start_streaming().await
    });
...
async fn test_satellite_fault_tolerance(ctx: TestContext) -> TestResult {
    ...
    let satellite_ctx = EventSourceContext {
        service_name: "test-resilient".to_string(),
        host: "test-host".to_string(),
        work_dir: ctx.work_dir(),
        dry_run: false,
        config: std::collections::HashMap::new(),
    ...
    assert!(events.len() <= 10, "Received more events than expected: {}", events.len());
    assert!(events.len() >= 5, "Received too few events despite retries: {}", events.len());
