...
async fn test_connection_pool_exhaustion() {
    ...
    assert!(max_active <= MAX_CONNECTIONS as u64, 
...
async fn test_connection_leak_detection() {
            ...
            for conn in conns.iter() {
                if !conn.released.load(Ordering::Relaxed) &&
                   conn.acquired_at.elapsed() > leak_timeout {
                    println!("LEAK DETECTED: Connection {} held by {} for {:?}",
                        conn.id, conn.acquired_by, conn.acquired_at.elapsed());
    ...
    for conn in conns.iter() {
        if !conn.released.load(Ordering::Relaxed) {
            println!("  Leaked: Connection {} by {} (held for {:?})",
                conn.id, conn.acquired_by, conn.acquired_at.elapsed());
        }
    }
