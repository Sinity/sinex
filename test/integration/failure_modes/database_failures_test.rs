...
async fn test_transaction_rollback_behavior(ctx: TestContext) -> Result<(), anyhow::Error> {
    ...
    println!(
        "  Partial writes before rollback: {}",
        partial_writes.load(Ordering::Relaxed)
    );
    println!(
        "  Concurrent conflict detected: {}",
        conflict_detected.load(Ordering::Relaxed)
    );
    ...
    if total_rollbacks >= 2 {
        println!(
            "Successfully detected and handled {} rollbacks",
            total_rollbacks
        );
    } else {
        println!(
            "WARNING: Only {} rollbacks detected (expected >= 2)",
            total_rollbacks
        );
    }
...
async fn test_migration_failure_handling(ctx: TestContext) -> TestResult {
    ...
    let migrations = vec![
        (
            "001_initial",
            "CREATE TABLE test_migration_1 (id SERIAL PRIMARY KEY)",
            true,
        ),
        (
            "002_add_column",
            "ALTER TABLE test_migration_1 ADD COLUMN data TEXT",
            true,
        ),
        (
            "003_bad_migration",
            "ALTER TABLE nonexistent_table ADD COLUMN fail TEXT",
            false,
        ),
        (
            "004_dependent",
            "CREATE TABLE test_migration_2 (id SERIAL, ref_id INT REFERENCES test_migration_1(id))",
            true,
        ),
    ];
...
async fn test_database_restart_resilience(ctx: TestContext) -> TestResult {
    ...
    println!(
        "  Queries during outage: {}",
        queries_during_outage.load(Ordering::Relaxed)
    );
    println!(
...
async fn test_large_result_set_handling(ctx: TestContext) -> Result<(), anyhow::Error> {
    ...
    let fetch_all_count = all_at_once_result
        .map(|r| r.map(|rows| rows.len()))
        .unwrap_or(Ok(0))
        .unwrap_or(0) as i64;
    ...
    println!(
        "  Fetch all: {} rows in {:?}",
        fetch_all_count, fetch_all_time
    );
    println!("  Stream: {} rows in {:?}", stream_count, stream_time);
    println!(
        "  Memory efficiency: Streaming is {}x faster",
