...
async fn test_orphaned_work_recovery() {
    ...
    for item in queue.iter() {
        println!(
            "  Item {}: status={:?}, assigned_to={:?}, attempts={}",
...
async fn test_zombie_worker_prevention() {
    ...
    println!(
        "  Good worker: {} iterations (stopped at shutdown)",
        good_count_at_shutdown
    );
    println!(
        "  Zombie worker: {} iterations (force killed)",
        zombie_count_at_shutdown
    );
    println!(
        "  Good worker after shutdown: {} (should be same)",
        good_count_final
    );
    println!(
        "  Zombie after abort: {} (should be same)",
        zombie_count_final
    );
    ...
    assert!(
        good_count_at_shutdown > 0 && zombie_count_at_shutdown > 0,
