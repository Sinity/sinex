    ...
    impl TimeoutStats {
        ...
        fn summary(&self) -> String {
            ...
            format!(
                ...
                timeouts,
                (timeouts as f64 / attempts as f64) * 100.0,
                errors,
        ...
        let handle = tokio::spawn(async move {
            for iteration in 0..5 {
                ...
                if current < POOL_SIZE as u64 {
                    ...
                    while new_count > max {
                        match max_used.compare_exchange(
                            max,
                            new_count,
                            Ordering::Relaxed,
                            Ordering::Relaxed,
                    ...
                    eprintln!(
                        "Worker {} couldn't acquire connection (pool exhausted)",
                        worker_id
                    );
                }
...
async fn test_retry_with_backoff(_ctx: TestContext) -> TestResult {
    ...
    async fn flaky_operation(attempts: &Arc<AtomicU64>, fail_count: u64) -> Result<String, String> {
    ...
    assert!(
        successes >= 2,
        "Expected at least 2 operations to succeed after retry"
    );
