...
async fn test_memory_leak_detection() {
    ...
    if let (Some(early), Some(mid)) = (samples.get(2), samples.get(25)) {
        ...
        assert!(
            growth_rate > 1000.0,
            "Memory should grow during leak phase at >1KB/sample, got {:.1} bytes/sample",
            growth_rate
        );
    }
    ...
    if let (Some(mid), Some(late)) = (samples.get(52), samples.get(75)) {
        ...
        assert!(
            stable_growth < 10000.0,
            "Memory growth should slow after leak stops, got {:.1} bytes/sample",
            stable_growth
        );
    }
...
async fn test_cpu_throttling_detection() {
    ...
    impl PerformanceMonitor {
        ...
        async fn record_processing_time(&self, duration: Duration) {
            ...
            if times.len() >= 20 {
                ...
                let recent_avg =
                    recent.iter().map(|d| d.as_millis()).sum::<u128>() / recent.len() as u128;
                let older_avg =
                    older.iter().map(|d| d.as_millis()).sum::<u128>() / older.len() as u128;

                // If recent processing is 2x slower than older, likely throttled
                if recent_avg > older_avg * 2 {
                    self.throttle_detected.store(true, Ordering::Relaxed);
                    println!(
                        "CPU throttling detected! Recent avg: {}ms, Older avg: {}ms",
    ...
    assert!(
        times.len() >= 50,
        "Should have collected at least 50 timing samples"
    );
    ...
    if times.len() >= 10 {
        ...
        let avg_early =
            early_times.iter().map(|d| d.as_nanos()).sum::<u128>() / early_times.len() as u128;
        assert!(
            avg_early < 100_000_000, // 100ms
            "Early processing times should be <100ms, got {} ns",
            avg_early
        );
    }
    ...
    if times.len() >= 40 {
        ...
        assert!(
            degradation_ratio < 50.0, // 50x degradation should be detectable
            "Extreme performance degradation should be detectable, ratio: {:.2}",
            degradation_ratio
        );
    }
...
async fn test_io_saturation_handling() -> TestResult {
    ...
    let baseline_avg = baseline_latencies
        ...
        .map(|d| d.as_micros())
        .sum::<u128>()
        / baseline_latencies.len() as u128;
                ...
                match result {
                    Ok(Ok(())) => {
                        ...
                        if latency.as_micros() > baseline * 10 {
                            ...
                            eprintln!(
                                "Slow write detected: {:?} ({}x baseline)",
                                latency,
                                latency.as_micros() / baseline
                            );
                        }
                    }
                    Ok(Err(e)) => {
    ...
    let mut sorted_latencies: Vec<_> = all_latencies.iter().map(|d| d.as_micros()).collect();
    ...
    println!(
        "    p50: {} μs ({:.1}x baseline)",
        p50,
        p50 as f64 / baseline_avg as f64
    );
    println!(
        "    p95: {} μs ({:.1}x baseline)",
        p95,
        p95 as f64 / baseline_avg as f64
    );
    println!(
        "    p99: {} μs ({:.1}x baseline)",
        p99,
        p99 as f64 / baseline_avg as f64
    );
    ...
    assert!(
        slow_percentage < 95.0,
        "Should handle most I/O operations without timeout, got {:.1}% slow",
        slow_percentage
    );

    // Verify latency measurements are reasonable
    assert!(
        p50 < 1_000_000, // 1 second p50 should be achievable
        "p50 latency should be <1s, got {} μs",
        p50
    );
...
async fn test_resource_usage_patterns() {
    ...
    assert!(
        detected_pattern.is_some(),
        "Should detect a resource usage pattern"
    );
}
