...
async fn test_disk_full_handling(ctx: TestContext) -> TestResult {
    ...
    let result =
        try_write_event_data(&test_path, recovery_data, &write_attempts, &write_failures).await;
...
async fn test_filesystem_availability(_ctx: TestContext) -> TestResult {
    ...
    async fn watch_filesystem(path: &PathBuf, phase: &str, counter: &Arc<AtomicU64>) -> TestResult {
    ...
    let result = watch_filesystem(
        &mount_point,
        "during_unavailable",
        &events_during_unavailable,
    )
    .await;
    assert!(result.is_err());
    ...
    println!(
        "  Events before unmount: {}",
        events_before_unmount.load(Ordering::Relaxed)
    );
    println!(
        "  Events during unavailable: {}",
        events_during_unavailable.load(Ordering::Relaxed)
    );
    println!(
        "  Events after remount: {}",
        events_after_remount.load(Ordering::Relaxed)
    );
