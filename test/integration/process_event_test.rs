...
use sinex_core::{ProcessHeartbeatEmitter, MetricsProvider};
...
impl MetricsProvider for MockMetricsProvider {
    fn get_events_processed_last_minute(&self) -> u32 {
        (*self.events_processed.lock().unwrap() % 60) as u32
    }
...
async fn test_process_lifecycle_events(ctx: TestContext) -> TestResult {
    ...
    emitter.emit_process_shutdown("Graceful shutdown requested").await?;
...
async fn test_process_heartbeat_continuous_emission(ctx: TestContext) -> TestResult {
    ...
    let heartbeat_handle = tokio::spawn(async move {
        emitter_clone.start_heartbeat_loop().await
    });
    ...
    assert!(heartbeat_count >= 3, "Should have at least 3 heartbeat events, got {}", heartbeat_count);
    assert!(heartbeat_count <= 8, "Should not have excessive heartbeats, got {}", heartbeat_count);
...
async fn test_health_aggregator_process_discovery(ctx: TestContext) -> TestResult {
    ...
    for row in health_data {
        ...
        found_processes.insert(name.clone(), (
    ...
    assert_eq!(found_processes["web_server"], ("healthy".to_string(), 3600, 512, 1000));
    assert_eq!(found_processes["worker_1"], ("healthy".to_string(), 1800, 256, 500));
    assert_eq!(found_processes["worker_2"], ("degraded".to_string(), 900, 128, 250));
    assert_eq!(found_processes["monitor"], ("healthy".to_string(), 7200, 64, 100));
...
async fn test_process_failure_detection(ctx: TestContext) -> TestResult {
    ...
    metrics_provider.add_custom_metric("last_error", serde_json::json!("Database connection failed"));
    emitter.emit_heartbeat().await?;
    
    // Simulate shutdown due to errors
    emitter.emit_process_shutdown("Process terminated due to critical errors").await?;
    ...
    assert_eq!(payload1.get("health_status").unwrap_or(&serde_json::json!("healthy")), "healthy");
    ...
    let shutdown_event = events.iter().find(|e| e.event_type == "process.shutdown").unwrap();
    let shutdown_payload: serde_json::Value = shutdown_event.payload.clone();
    assert_eq!(shutdown_payload["reason"], "Process terminated due to critical errors");
...
async fn test_process_restart_detection(ctx: TestContext) -> TestResult {
    ...
    let start_events: Vec<_> = all_events.iter()
        .filter(|e| e.event_type == "process.started")
        .collect();
    
    assert_eq!(start_events.len(), 2, "Should have two process.started events");
...
async fn test_high_frequency_heartbeats(ctx: TestContext) -> TestResult {
    ...
    assert!(duration.as_millis() < 5000, "High frequency heartbeats should complete within 5 seconds");
    ...
    for i in 1..timestamps.len() {
        assert!(timestamps[i].ts_orig >= timestamps[i-1].ts_orig, "Heartbeats should be chronologically ordered");
    }
