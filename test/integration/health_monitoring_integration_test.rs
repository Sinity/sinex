...
async fn test_component_health_checks(
    ...
    monitor
        .update_component_health("terminal_source", terminal_health, None)
        .await;

    // Test clipboard source health
    let clipboard_health = check_clipboard_source_health().await?;
    monitor
        .update_component_health("clipboard_source", clipboard_health, None)
        .await;

    // Test window manager source health
    let wm_health = check_window_manager_source_health().await?;
    monitor
        .update_component_health("hyprland_source", wm_health, None)
        .await;

    // Test collector health
    let collector_health = check_collector_health().await?;
    monitor
        .update_component_health("unified_collector", collector_health, None)
        .await;

    // Test worker health
    let worker_health = check_worker_health(pool).await?;
    monitor
        .update_component_health("promotion_worker", worker_health, None)
        .await;

    // Test git-annex health
    let annex_health = check_git_annex_health().await?;
    monitor
        .update_component_health("git_annex", annex_health, None)
        .await;
...
async fn check_worker_health(pool: &DbPool) -> Result<HealthStatus> {
    ...
    match sqlx::query("SELECT COUNT(*) FROM sinex_schemas.work_queue")
        .fetch_one(pool)
        .await
    {
        Ok(_) => {
            ...
            if output.status.success() {
                    ...
                    .output()
                {
                    Ok(git_output) if git_output.status.success() => {
                        match Command::new("git")
                            .args(["annex", "init", "health-check"])
                            .current_dir(&temp_dir)
                            .output()
                        {
                            Ok(annex_output) if annex_output.status.success() => {
                                Ok(HealthStatus::Healthy)
                            }
...
async fn test_failure_detection_and_recovery(
    ...
    for i in 1..=5 {
        monitor
            .update_component_health(
                "terminal_source",
                HealthStatus::Unhealthy,
                Some(format!("Terminal failure #{}", i)),
            )
            .await;
    }
    ...
    assert!(
        terminal_health.failure_count >= 5,
    ...
    monitor
        ...
        .await;
    monitor
        .update_component_health("terminal_source", HealthStatus::Healthy, None)
        .await;

    let recovered_health = monitor.get_system_health().await;
    pretty_assertions::assert_eq!(
        recovered_health["filesystem_source"].status,
        HealthStatus::Healthy
    );
    pretty_assertions::assert_eq!(
        recovered_health["terminal_source"].status,
        HealthStatus::Healthy
    );
    pretty_assertions::assert_eq!(
        recovered_health["terminal_source"].failure_count,
        0,
        "Failure count should reset on recovery"
    );
...
async fn test_system_health_aggregation(
    ...
    monitor
        .update_component_health(
            ...
            Some("Clipboard tools not available".to_string()),
        )
        .await;
    ...
    monitor
        .update_component_health(
            "filesystem_source",
            HealthStatus::Unhealthy,
            Some("Filesystem watcher crashed".to_string()),
        )
        .await;

    let multiple_unhealthy = monitor.get_unhealthy_components().await;
    assert!(
        multiple_unhealthy.len() >= 2,
        "Should detect multiple unhealthy components"
    );
...
async fn test_health_monitoring_performance_impact(_ctx: TestContext) -> TestResult {
    ...
    for i in 0..iterations {
        ...
        monitor
            .update_component_health(&component_name, status, None)
            .await;
    }
    ...
    assert!(
        avg_update_time < Duration::from_millis(1),
        "Health updates should be fast, average: {:?}",
        avg_update_time
    );
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            for j in 0..100 {
                ...
                monitor
                    .update_component_health(&component_name, HealthStatus::Healthy, None)
                    .await;
            }
        });
        handles.push(handle);
    ...
    assert!(
        ...
        "Concurrent health updates should complete quickly: {:?}",
        concurrent_duration
    );
...
async fn test_health_monitoring_with_real_workload(ctx: TestContext) -> TestResult {
    ...
    let workload_task = tokio::spawn(async move {
        ...
        for i in 0..100 {
            ...
            match queries::insert_event(&workload_pool, &event).await {
                Ok(_) => {
                    workload_monitor
                        .update_component_health("database", HealthStatus::Healthy, None)
                        .await;
                    workload_monitor
                        .update_component_health("event_processing", HealthStatus::Healthy, None)
                        .await;
                }
                Err(e) => {
                    workload_monitor
            ...
            if i % 10 == 0 {
                ...
                let _ =
                    queries::add_to_work_queue(&workload_pool, event.id, "health-test-agent", 3)
                        .await;
                ...
                .await
                {
                    Ok(items) => {
                        workload_monitor
                            .update_component_health("worker_system", HealthStatus::Healthy, None)
                            .await;
    ...
    assert!(
        health_ratio > 0.7,
        "System should be healthy most of the time during normal workload: {:.2}%",
        health_ratio * 100.0
    );
    ...
    pretty_assertions::assert_eq!(
        final_count,
        100,
        "All events should be processed despite health monitoring"
    );
