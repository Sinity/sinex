...
#[sinex_test]
async fn test_database_connectivity_timeout(_ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    match result {
        Ok(Ok((status, _details, messages))) => {
            ...
            assert!(messages.iter().any(|m| m.contains("timeout")
            ...
            assert!(
                elapsed.as_secs() <= 7,
                "Should timeout within ~5s internal timeout, took: {:?}",
                elapsed
            );
        }
        Ok(Err(e)) => {
...
#[sinex_test]
async fn test_extensions_timeout_handling(_ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    match result {
        Ok(Err(_)) => {
            ...
            assert!(
                elapsed.as_secs() <= 8,
                "Should fail quickly on connection timeout"
            );
        }
...
#[sinex_test]
async fn test_resource_verification_performance(_ctx: TestContext) -> color_eyre::eyre::Result<()> {
    let start_time = Instant::now();
    ...
    for i in 0..5 {
        ...
        let (status, details, _messages) = sinex_preflight::resources::verify_system_resources()
            .await
        ...
        assert!(
            iteration_duration.as_secs() < 5,
            "Resource check iteration {} took too long: {:?}",
            i,
            iteration_duration
        );
    }
    ...
    assert!(
        max_duration.as_millis() - min_duration.as_millis() < 2000,
...
#[sinex_test]
async fn test_configuration_verification_timeout(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    match result {
        ...
        Err(_) => {
            panic!(
                "Configuration verification should not timeout, took: {:?}",
                elapsed
            );
        }
    }
...
async fn test_service_verification_timeout(_ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    match result {
        Ok(Ok((status, details, _messages))) => {
            ...
            assert!(
                elapsed.as_secs() < 10,
                "Service verification should complete reasonably quickly, took: {:?}",
                elapsed
            );
        }
        Ok(Err(e)) => {
        ...
        Err(_) => {
            panic!(
                "Service verification should not timeout, took: {:?}",
                elapsed
            );
        }
    }
    ...
    println!(
        "✓ Service verification timeout test completed in {:?}",
        elapsed
    );
    Ok(())
}
...
async fn test_benchmark_all_phases(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    // Benchmark extensions (might warn in test environment)
    let start = Instant::now();
    let (status, _details, _messages) = sinex_preflight::database::verify_postgresql_extensions()
        .await
        .map_err(|e| format!("Extensions benchmark failed: {}", e))?;
    let ext_duration = start.elapsed();
    assert!(matches!(
    ...
    // Benchmark migration readiness
    let start = Instant::now();
    let (status, _details, _messages) = sinex_preflight::database::verify_migration_readiness()
        .await
        .map_err(|e| format!("Migration benchmark failed: {}", e))?;
    let migration_duration = start.elapsed();
    assert!(matches!(
    ...
    // Benchmark resources
    let start = Instant::now();
    let (status, _details, _messages) = sinex_preflight::resources::verify_system_resources()
        .await
        .map_err(|e| format!("Resources benchmark failed: {}", e))?;
    let resources_duration = start.elapsed();
    assert!(matches!(
    ...
    // Benchmark services
    let start = Instant::now();
    let (status, _details, _messages) = sinex_preflight::services::verify_service_dependencies()
        .await
        .map_err(|e| format!("Services benchmark failed: {}", e))?;
    let services_duration = start.elapsed();
    assert!(matches!(
    ...
    assert!(
        ...
        "Total verification should complete within 30 seconds"
    );

    // Individual phase targets
    assert!(
        benchmarks["database_connectivity"].as_secs() < 5,
        "Database connectivity should be < 5s"
    );
    assert!(
        benchmarks["configuration"].as_secs() < 2,
        "Configuration should be < 2s"
    );
    assert!(
        benchmarks["resources"].as_secs() < 3,
...
async fn test_concurrent_performance(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    for handle in handles {
        let (task_id, task_duration, result) = handle
            .await
            .map_err(|e| format!("Task join failed: {}", e))?;
        ...
        match result {
            Ok((status, _details, _messages)) => {
                assert!(
                    ...
                    "Concurrent task {} should pass/warn",
                    task_id
                );
    ...
    // Analyze concurrent performance
    let avg_task_duration: Duration = task_results
        .iter()
        .map(|(_, duration)| *duration)
        .sum::<Duration>()
        / task_results.len() as u32;
    ...
    assert!(
        avg_task_duration.as_secs() < 5,
        "Average task duration should be reasonable"
    );
...
async fn test_graceful_shutdown_handling(_ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let verification_task = tokio::spawn(async move {
        ...
        while !shutdown_flag_clone.load(Ordering::Relaxed) {
            ...
            if iteration % 5 == 0 && shutdown_flag_clone.load(Ordering::Relaxed) {
                println!(
                    "Verification gracefully shutting down after {} iterations",
                    iteration
                );
                break;
            }
    ...
    // Wait for graceful shutdown
    let final_iteration = verification_task
        .await
        .map_err(|e| format!("Verification task failed: {}", e))?;
    ...
    assert!(
        final_iteration >= 3,
        "Should have run for at least a few iterations"
    );
    assert!(
        final_iteration <= 50,
        "Should have shut down before safety break"
    );
...
async fn test_memory_usage_during_verification(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    // Service verification
    let _ = sinex_preflight::services::verify_service_dependencies()
        .await
        .map_err(|e| format!("Service verification failed: {}", e))?;
    memory_measurements.push(("services", get_current_memory_usage()));
    ...
    for (phase, memory) in &memory_measurements {
        ...
        println!(
            "  After {}: {:.2} MB (+{:.2} MB)",
            phase, memory_mb, diff_mb
        );
    }

    // Verify memory usage is reasonable (shouldn't grow excessively)
    let max_memory = memory_measurements
        .iter()
        .map(|(_, mem)| *mem)
        .fold(0.0, f64::max);
    let memory_growth = max_memory - initial_memory;
    ...
    assert!(
        memory_growth / 1024.0 / 1024.0 < 100.0,
        "Memory growth should be reasonable (< 100MB), grew: {:.2} MB",
        memory_growth / 1024.0 / 1024.0
    );
