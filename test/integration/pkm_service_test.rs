...
async fn test_create_note_annotation(ctx: TestContext) -> TestResult {
    ...
    let inserted_event = sinex_db::events::insert_event_with_validator(ctx.pool(), &event, None).await?;
    ...
    let annotation_id = service.create_note(
        inserted_event.id,
        content,
        tags.clone(),
        created_by
...
async fn test_multiple_note_annotations(ctx: TestContext) -> TestResult {
    ...
    let note1_id = service.create_note(
        inserted_event.id,
        "First note",
        vec!["first".to_string()],
        "user1"
    ...
    let note2_id = service.create_note(
        inserted_event.id,
        "Second note",
...
async fn test_create_entities_from_list(ctx: TestContext) -> TestResult {
    ...
    // Create entities
    let entity_ids = service.create_entities_from_list(inserted_event.id, entities.clone()).await?;
    ...
    for (i, entity_id) in entity_ids.iter().enumerate() {
        ...
        assert_eq!(entity.metadata["source_event_id"], json!(inserted_event.id.to_string()));
    }
...
async fn test_entity_type_constraints_valid(ctx: TestContext) -> TestResult {
    ...
    let valid_types = vec![
        ...
        "location", "concept", "tool", "event"
    ];
    
    for entity_type in valid_types {
        let entities = vec![(format!("Test {}", entity_type), entity_type.to_string())];
        let result = service.create_entities_from_list(inserted_event.id, entities).await;
        assert!(result.is_ok(), "Failed to create entity with type: {}", entity_type);
    }
...
async fn test_bidirectional_relationships(ctx: TestContext) -> TestResult {
    ...
    // Create entities
    let entity1 = knowledge_graph::create_entity(ctx.pool(), CreateEntityInput {
        entity_type: "person".to_string(),
    ...
    let entity2 = knowledge_graph::create_entity(ctx.pool(), CreateEntityInput {
        entity_type: "person".to_string(),
        name: "Bob".to_string(),
        canonical_name: None,
        aliases: None,
    ...
    let _relation_id = service.link_entities(
        entity1.entity_id,
        entity2.entity_id,
        "collaborates_with",
        HashMap::new()
...
async fn test_get_relationship_by_id(ctx: TestContext) -> TestResult {
    ...
    let relation = knowledge_graph::get_relation_by_id(ctx.pool(), relation_id).await?
...
async fn test_create_and_get_artifact(ctx: TestContext) -> TestResult {
    ...
    assert_eq!(artifact.original_path, Some("/home/user/documents/doc.pdf".to_string()));
    ...
    let retrieved = artifacts::get_artifact_by_id(ctx.pool(), artifact.artifact_id).await?
...
async fn test_artifact_type_constraints_valid(ctx: TestContext) -> TestResult {
    ...
    for artifact_type in valid_types {
        ...
        assert!(result.is_ok(), "Failed to create artifact with type: {}", artifact_type);
    }
...
async fn test_complete_pkm_workflow(ctx: TestContext) -> TestResult {
    ...
    let note_id = service.create_note(
        ...
        vec!["meeting".to_string(), "project".to_string(), "collaboration".to_string()],
        "alice"
    ).await?;
    ...
    let entity_ids = service.create_entities_from_list(inserted_event.id, entities).await?;
    assert_eq!(entity_ids.len(), 4);
    ...
    let relation1_id = service.link_entities(alice_id, project_id, "leads", alice_project_props).await?;
    let relation2_id = service.link_entities(bob_id, project_id, "collaborates_on", bob_project_props).await?;
    let relation3_id = service.link_entities(project_id, stanford_id, "hosted_by", project_org_props).await?;
    ...
    let retrieved_artifact = artifacts::get_artifact_by_id(ctx.pool(), artifact.artifact_id).await?;
    assert!(retrieved_artifact.is_some());
    assert_eq!(retrieved_artifact.unwrap().created_from_event_id, Some(inserted_event.id));
...
async fn test_transaction_rollback_on_entity_failure(ctx: TestContext) -> TestResult {
    ...
    assert!(!valid_entity_exists, "Transaction should have rolled back, no entities should exist");
...
async fn test_relationship_with_nonexistent_entities(ctx: TestContext) -> TestResult {
    ...
    let entity = knowledge_graph::create_entity(ctx.pool(), CreateEntityInput {
        entity_type: "person".to_string(),
    ...
    let result = service.link_entities(
        entity.entity_id,
        fake_id,
        "knows",
        HashMap::new()
    ...
    assert!(result.is_err(), "Should fail when linking to non-existent entity");
...
async fn test_empty_entity_list(ctx: TestContext) -> TestResult {
    ...
    let entity_ids = service.create_entities_from_list(inserted_event.id, vec![]).await?;
...
async fn test_annotation_with_empty_content(ctx: TestContext) -> TestResult {
    ...
    let annotation = annotations::get_annotation_by_id(ctx.pool(), annotation_id).await?
