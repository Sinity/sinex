...
async fn test_atuin_watermarking(ctx: TestContext) -> TestResult {
    ...
    let ctx_with_db =
        event_sources::test_context(serde_json::to_value(&config).unwrap()).with_db_pool(pg_pool);
    ...
    pretty_assertions::assert_eq!(
        second_events.len(),
        3,
        "Should get all 3 entries including the new one"
    );

    // Verify the new entry is included
    let commands: Vec<String> = second_events
        .iter()
        .map(|e| {
    ...
    assert!(
        commands.contains(&"echo third".to_string()),
        "Should include the newly added command"
    );
    Ok(())
}
...
async fn test_atuin_watermarking_resume_behavior(ctx: TestContext) -> TestResult {
    ...
    crate::common::create_test_agent(&pg_pool, "test-agent")
        .await
        .unwrap();
    ...
    pretty_assertions::assert_eq!(
        ...
        2,
        "First run should process both initial entries"
    );
    ...
    pretty_assertions::assert_eq!(
        ...
        2,
        "Second run should only process the 2 new entries"
    );

    let second_run_commands: Vec<String> = second_run_events
        .iter()
        .map(|e| {
            let payload: CommandExecutedAtuinPayload =
                serde_json::from_value(e.payload.clone()).unwrap();
    ...
    println!(
        "DEBUG: Second run captured commands: {:?}",
        second_run_commands
    );
    ...
    println!(
        "✅ Watermarking test passed! First run: {} events, Second run: {} events",
        first_run_events.len(),
        second_run_events.len()
    );
    Ok(())
...
async fn test_atuin_performance_with_many_entries(ctx: TestContext) -> TestResult {
    ...
    assert!(
        db_creation_time.as_millis() < 2000,
        "Database creation should be fast ({}ms)",
        db_creation_time.as_millis()
    );
    assert!(
    ...
    pretty_assertions::assert_eq!(
        command_count,
        num_entries as usize,
        "All commands should be captured correctly"
    );

    println!(
        "Performance test: {} entries processed in {}ms (DB creation: {}ms)",
        num_entries,
        processing_time.as_millis(),
        db_creation_time.as_millis()
    );
...
async fn test_real_atuin_integration(ctx: TestContext) -> TestResult {
    ...
    if !atuin_db_path.exists() {
        eprintln!(
            "Skipping real Atuin test - database not found at: {:?}",
            atuin_db_path
        );
    ...
    for (i, event) in events.iter().enumerate() {
        ...
        assert!(
            !payload.command_string.is_empty(),
            "Real command should not be empty"
        );
        assert!(!payload.cwd.is_empty(), "Real CWD should not be empty");
        assert!(
            !payload.atuin_history_id.is_empty(),
            "Real history ID should not be empty"
        );
        assert!(payload.duration_ns >= 0, "Duration should be non-negative");

        println!(
            "Real entry {}: '{}' (exit: {}, duration: {}ms)",
            i + 1,
            payload.command_string,
            payload.exit_code,
...
async fn test_live_atuin_monitoring(ctx: TestContext) -> TestResult {
    ...
    println!(
        "\n🎉 Live monitoring complete! Captured {} new commands",
        total_events
    );
