...
fn execute_command_via_atuin(cmd: &str) -> anyhow::Result<()> {
    if std::process::Command::new("atuin")
        .arg("--version")
        .output()
        .is_ok()
    {
...
async fn test_real_atuin_integration(ctx: TestContext) -> TestResult {
    ...
    assert!(
        !events.is_empty(),
        "Should capture at least some Atuin entries (got real: {})",
        db_path.to_string_lossy().contains(".local/share/atuin")
    );

    println!(
        "Successfully processed {} Atuin entries from {:?}",
        events.len(),
        db_path
    );
...
async fn test_live_atuin_monitoring(ctx: TestContext) -> TestResult {
    ...
    println!(
        "\n🎉 Live monitoring test complete! Captured {} events",
        total_events
    );
    assert!(
        total_events > 0,
        "Should capture at least some events from generated commands"
    );
...
async fn test_atuin_production_scale(ctx: TestContext) -> TestResult {
    ...
    if !events.is_empty() {
        println!(
            "Production scale test: {} entries processed in {}ms",
            events.len(),
        ...
        assert!(
            processing_time.as_millis() < 10000,
