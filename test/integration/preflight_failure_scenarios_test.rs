...
async fn test_database_connection_failures(_ctx: TestContext) -> TestResult {
    ...
    let (status, _details, _messages) = sinex_preflight::database::verify_database_connectivity().await
        .unwrap_or_else(|_| (sinex_preflight::VerificationStatus::Fail, json!({}), vec!["Connection error".to_string()]));
...
async fn test_database_extension_failures(_ctx: TestContext) -> TestResult {
    ...
    assert!(error_msg.contains("connect") || error_msg.contains("database") || error_msg.contains("URL"));
...
async fn test_resource_constraint_failures(_ctx: TestContext) -> TestResult {
    ...
    let warning_messages: Vec<_> = messages.iter()
        .filter(|m| m.contains("⚠") || m.contains("warning"))
        .collect();
    
    println!("✓ Resource constraint scenarios tested - {} warnings", warning_messages.len());
...
async fn test_configuration_failures(_ctx: TestContext) -> TestResult {
    ...
    let (status, details, messages) = sinex_preflight::configuration::verify_configuration_generation().await
...
async fn test_service_dependency_failures(_ctx: TestContext) -> TestResult {
    ...
    let binaries = details.get("binaries").expect("Should have binaries section");
    assert!(binaries.is_object());
    
    // Count missing required binaries
    let missing_required = binaries.as_object().unwrap().values()
    ...
    if missing_required > 0 {
        ...
        assert_eq!(status, sinex_preflight::VerificationStatus::Fail);
        assert!(messages.iter().any(|m| m.contains("Missing required binaries")));
    }
...
async fn test_binary_availability_failures(_ctx: TestContext) -> TestResult {
    ...
    assert!(!output.status.success(), "Nonexistent binary should not be found");
    
    // Test error handling when 'which' command itself fails
    let output = Command::new("nonexistent_which_command")
        .arg("ls")
        .output();
...
async fn test_error_message_formatting(_ctx: TestContext) -> TestResult {
    ...
    let (status, _details, messages) = sinex_preflight::database::verify_database_connectivity().await
        .unwrap_or_else(|_| (sinex_preflight::VerificationStatus::Fail, json!({}), vec!["Connection failed".to_string()]));
    ...
    if status == sinex_preflight::VerificationStatus::Fail {
        ...
        assert!(!error_messages.is_empty(), "Should have at least one error message");
        
        // Error messages should contain helpful information
        for error_msg in error_messages {
            assert!(error_msg.len() > 10, "Error messages should be descriptive: {}", error_msg);
        }
    }
...
async fn test_error_aggregation(_ctx: TestContext) -> TestResult {
    ...
    if let Ok((status, _details, messages)) = sinex_preflight::configuration::verify_configuration_generation().await {
        for message in messages {
    ...
    if let Ok((_status, _details, messages)) = sinex_preflight::resources::verify_system_resources().await {
        for message in messages {
    ...
    println!("✓ Error aggregation tested - {} errors, {} warnings", 
...
async fn test_timeout_conditions(_ctx: TestContext) -> TestResult {
    ...
    match result {
        Ok(Ok((status, _details, messages))) => {
            assert_eq!(status, sinex_preflight::VerificationStatus::Fail);
            assert!(messages.iter().any(|m| m.contains("timeout") || m.contains("connection failed")));
...
async fn test_graceful_degradation(_ctx: TestContext) -> TestResult {
    ...
    let (config_status, _config_details, _config_messages) = sinex_preflight::configuration::verify_configuration_generation().await
...
async fn test_recovery_mechanisms(ctx: TestContext) -> TestResult {
    ...
    let (status2, _details2, _messages2) = sinex_preflight::database::verify_database_connectivity().await
