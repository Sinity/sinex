...
#[sinex_test]
async fn test_basic_pel_recovery(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let pel_info: redis::streams::StreamPendingReply =
        redis_client.xpending(stream_key, group_name).await?;
    ...
    let final_pel: redis::streams::StreamPendingReply =
        redis_client.xpending(stream_key, group_name).await?;
...
#[sinex_test]
async fn test_partial_pel_recovery(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    let mut redis_client = ctx.redis().await?;
...
#[sinex_test]
async fn test_multi_consumer_pel_recovery(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    let mut redis_client = ctx.redis().await?;
    ...
    for (consumer, id) in &all_message_ids {
        if consumer != "consumer-2" {
            let ack_result: i64 = redis_client.xack(stream_key, group_name, &[id]).await?;
            assert_eq!(ack_result, 1);
        }
    }
...
#[sinex_test]
async fn test_idle_timeout_pel_recovery(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    let mut redis_client = ctx.redis().await?;
    ...
    assert_eq!(
        claimed.len(),
        3,
        "Should claim all messages after idle time"
    );
...
#[sinex_test]
async fn test_forced_pel_takeover(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    let mut redis_client = ctx.redis().await?;
    ...
    assert_eq!(
        final_pel.count(),
        0,
        "PEL should be empty after forced takeover"
    );
...
#[sinex_test]
async fn test_pel_redelivery_count(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    let mut redis_client = ctx.redis().await?;
    ...
    let claimed: Vec<redis::streams::StreamClaimReply> = redis_client
        .xclaim(stream_key, group_name, "final-consumer", 1, &[&message_id])
        .await?;
...
#[sinex_test]
async fn test_concurrent_pel_claims(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    let mut redis_client = ctx.redis().await?;
...
#[sinex_test]
async fn test_production_pel_recovery_patterns(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    let mut redis_client = ctx.redis().await?;
