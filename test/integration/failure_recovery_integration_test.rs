...
async fn test_event_source_crash_recovery(_ctx: TestContext) -> TestResult {
    ...
    assert!(
        monitoring_recovery,
        "Source monitoring should detect and handle crashes"
    );
...
async fn test_source_crash_and_restart() -> Result<bool> {
    ...
    assert!(
        ...
        "Crash should be detected"
    );
    assert!(
        restart_successful.load(Ordering::SeqCst),
...
async fn test_source_state_recovery() -> Result<bool> {
    ...
    pretty_assertions::assert_eq!(
        recovered_state["last_processed_id"].as_i64().unwrap(),
        12345
    );
...
async fn test_source_monitoring_recovery() -> Result<bool> {
    ...
    pretty_assertions::assert_eq!(
        failed_sources.load(Ordering::SeqCst),
        0,
        "No sources should be failed after recovery"
    );
    pretty_assertions::assert_eq!(
        recovered_sources.load(Ordering::SeqCst),
        1,
        "Should have detected one recovery"
    );
    pretty_assertions::assert_eq!(
        healthy_sources.load(Ordering::SeqCst),
        3,
        "All sources should be healthy after recovery"
    );
...
async fn test_worker_failure_and_retry(ctx: TestContext) -> TestResult {
    ...
    assert!(
        retry_logic,
        "Worker retry logic should handle failures correctly"
    );
    ...
    assert!(
        concurrent_failure,
        "System should handle multiple concurrent worker failures"
    );
...
async fn test_dead_letter_queue_handling(pool: &DbPool) -> Result<bool> {
    ...
    if !final_claim.is_empty() {
        assert!(
            final_claim[0].attempts >= 2,
            "Should have high attempt count if still claimable"
        );
    }
...
async fn test_concurrent_worker_failures(pool: &DbPool) -> Result<bool> {
    ...
    assert!(
        ...
        "Should have some worker activity"
    );
    ...
    assert!(
        health_check.is_ok(),
        "System should remain healthy after concurrent failures"
    );
...
async fn test_memory_pressure_recovery() -> Result<bool> {
    ...
    assert!(
        processed_count > 0,
        "Should process some events despite memory pressure"
    );
...
async fn test_channel_overflow_recovery() -> Result<bool> {
    ...
    let producer = tokio::spawn(async move {
        for i in 0..50 {
            let event =
                RawEventBuilder::new("overflow_test", "channel.overflow", json!({"sequence": i}))
                    .build();
    ...
    assert!(
        overflow_handled.load(Ordering::SeqCst),
        "Should detect channel overflow"
    );
