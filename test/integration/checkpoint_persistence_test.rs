...
impl HotlogAutomaton for TestCheckpointAutomaton {
    async fn initialize(&mut self, ctx: HotlogAutomatonContext) -> sinex_satellite_sdk::SatelliteResult<()> {
        self.context = Some(ctx);
        Ok(())
    }
    
    async fn process_event(&mut self, event: HotlogAutomatonEvent) -> sinex_satellite_sdk::SatelliteResult<ProcessingResult> {
        // Record that we processed this event
        self.processed_events.lock().await.push(event.event.id.to_string());
...
async fn test_checkpoint_persistence_and_restart_recovery(ctx: crate::TestContext) -> crate::TestResult {
    ...
    let _ = redis_client.delete_consumer_group("sinex:streams:hotlog", &consumer_group).await;
    ...
    for event in &test_events {
        ...
        redis_client.add_to_stream(
            "sinex:streams:hotlog",
            &[("data".to_string(), serialized)],
        ).await?;
    }
    
    info!("Injected {} test events into hotlog stream", test_events.len());
    
    // Step 2: Run automaton for a short time to process some events
    info!("Step 2: Running automaton to process events");
    
    let runner_handle = {
        let mut runner_clone = HotlogAutomatonRunner::new(TestCheckpointAutomaton::new(service_name.clone()));
        runner_clone.initialize(
            service_name.clone(),
            consumer_group.clone(),
            consumer_name.clone(),
            vec![EventFilter::new(Some("test".to_string()), None)],
    ...
    info!("Processed {} events before restart", processed_count_before_restart);
    
    // Verify checkpoint was saved to database
    let checkpoint_before = checkpoint_manager.load_checkpoint().await?;
    assert!(checkpoint_before.is_some(), "Checkpoint should be saved to database");
    let checkpoint_before = checkpoint_before.unwrap();
    assert!(checkpoint_before.processed_count > 0, "Processed count should be > 0 in checkpoint");
    
    info!(
        "Checkpoint before restart: processed_count={}, last_id={:?}",
        checkpoint_before.processed_count,
        checkpoint_before.last_processed_id
    );
    ...
    assert!(checkpoint_after.is_some(), "Checkpoint should still exist after restart");
    ...
    let checkpoint_after = checkpoint_after.unwrap();
    assert!(
    ...
    info!(
        ...
        checkpoint_after.processed_count,
        checkpoint_after.last_processed_id
    );
    ...
    let pending_messages = redis_client.pending_messages(
        "sinex:streams:hotlog",
        &consumer_group,
        None,
        None,
