...
use crate::common::resources::{temp_dir, create_test_file};
use sinex_annex::{BlobManager, AnnexConfig, GitAnnex};
use std::path::Path;
use std::sync::Arc;
...
async fn test_blake3_content_deduplication(ctx: TestContext) -> TestResult {
    ...
    let metadata1 = fixture.manager
        .ingest_from_bytes(content, "file1.txt", "text/plain")
        .await?;
    
    let metadata2 = fixture.manager
    ...
    assert_eq!(metadata1.checksum_blake3.as_ref().unwrap(), &expected_blake3);
    
    // Both should reference the same blob in database
    let blob1 = fixture.manager.get_blob_metadata(&metadata1.blob_id).await?;
    let blob2 = fixture.manager.get_blob_metadata(&metadata2.blob_id).await?;
...
async fn test_file_vs_bytes_deduplication(ctx: TestContext) -> TestResult {
    ...
    let retrieved1 = fixture.manager.retrieve_content(&metadata1.annex_key).await?;
    let retrieved2 = fixture.manager.retrieve_content(&metadata2.annex_key).await?;
...
async fn test_git_annex_get_content(ctx: TestContext) -> TestResult {
    ...
    let retrieved_content = fixture.manager
...
async fn test_git_annex_verification(ctx: TestContext) -> TestResult {
    ...
    assert_eq!(updated_metadata.verification_status.as_deref(), Some("verified"));
...
async fn test_annex_key_parsing(ctx: TestContext) -> TestResult {
    ...
    let annex_key = sinex_annex::AnnexKey::parse(&metadata.annex_key)?;
...
async fn test_large_blob_handling(ctx: TestContext) -> TestResult {
    ...
    eprintln!("Large blob ({} MB) - Ingest: {:?}, Retrieve: {:?}", 
              large_size / (1024 * 1024), ingest_duration, retrieve_duration);
...
async fn test_multiple_blob_sizes(ctx: TestContext) -> TestResult {
    ...
    for size in test_sizes {
        ...
        let retrieved = fixture.manager.retrieve_content(&metadata.annex_key).await?;
...
async fn test_retrieve_nonexistent_content(ctx: TestContext) -> TestResult {
    ...
    let result = fixture.manager.retrieve_content("SHA256E-s100--nonexistent.dat").await;
    assert!(result.is_err());
...
async fn test_binary_content_integrity(ctx: TestContext) -> TestResult {
    ...
    let metadata = fixture.manager
        .ingest_from_bytes(&binary_content, "binary_test.bin", "application/octet-stream")
        .await?;
...
async fn test_storage_statistics_emission(ctx: TestContext) -> TestResult {
    ...
    let stats_events: Vec<_> = recent_events.iter()
        .filter(|e| e.source == "blob_storage" && e.event_type == "metrics.blob_storage.statistics")
        .collect();
    
    assert!(!stats_events.is_empty(), "Storage statistics event should be created");
...
async fn test_metrics_emission_during_operations(ctx: TestContext) -> TestResult {
    ...
    let metrics_before = events_before.iter()
    ...
    let _retrieved = fixture.manager.retrieve_content(&metadata.annex_key).await?;
    
    // Check that operation metrics were emitted
    let events_after = crate::common::get_recent_events(ctx.pool(), 100).await?;
    let metrics_after = events_after.iter()
        .filter(|e| e.source == "blob_storage" && e.event_type == "metrics.blob_storage.operation")
        .count();
    ...
    let operation_events: Vec<_> = events_after.iter()
        .filter(|e| e.source == "blob_storage" && e.event_type == "metrics.blob_storage.operation")
        .collect();
    
    for event in operation_events.iter().take(2) { // Check recent metrics
