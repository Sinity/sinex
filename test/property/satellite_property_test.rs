        ...
        rt.block_on(async {
            ...
            match config_result {
                Ok(config) => {
                    ...
                    assert_eq!(config.checkpoint_interval_secs, checkpoint_interval);
    ...
    fn satellite_event_processing_preserves_order(
        ...
        rt.block_on(async {
            ...
            for i in 1..db_events.len() {
                assert!(db_events[i-1].id.timestamp_ms() <= db_events[i].id.timestamp_ms());
            }
        });
    }
}
    ...
    fn satellite_survives_network_partitions(
        ...
        rt.block_on(async {
            ...
            for i in 0..events_before_partition {
                let event = ctx.create_test_event("partition_test", &format!("before.{}", i));
                ctx.insert_event(&event).await.unwrap();
            }
            ...
            for i in 0..events_during_partition {
                let event = ctx.create_test_event("partition_test", &format!("during.{}", i));
            ...
            for i in 0..events_after_partition {
                let event = ctx.create_test_event("partition_test", &format!("after.{}", i));
                ctx.insert_event(&event).await.unwrap();
            }
...
proptest! {
    ...
    fn satellite_automaton_coordination_is_correct(
        ...
        rt.block_on(async {
            ...
            ctx.wait_for_checkpoint_progress(automaton_type, events.len() as u64).await.unwrap();
