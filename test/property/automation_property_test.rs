    ...
    fn automaton_processing_is_deterministic(
        ...
        rt.block_on(async {
            ...
            for run in 0..2 {
                ...
                let automaton = ctx.start_test_automaton(&test_automaton).await.unwrap();
                ...
                // Wait for processing
                ctx.wait_for_checkpoint_progress(&test_automaton, events.len() as u64).await.unwrap();
...
proptest! {
    ...
    fn automaton_state_consistency_under_concurrency(
        ...
        rt.block_on(async {
            ...
            let automaton = ctx.start_test_automaton(&automaton_name).await.unwrap();
...
proptest! {
    ...
    fn automaton_recovery_from_checkpoint_is_correct(
        ...
        rt.block_on(async {
            ...
            let automaton1 = ctx.start_test_automaton(&automaton_name).await.unwrap();
            ...
            if !initial_events.is_empty() {
                ctx.wait_for_checkpoint_progress(&automaton_name, initial_events.len() as u64).await.unwrap();
            }
            ...
            drop(automaton1);
            let automaton2 = ctx.start_test_automaton(&automaton_name).await.unwrap();
            ...
            if !recovery_events.is_empty() {
                ...
                ctx.wait_for_checkpoint_progress(&automaton_name, expected_total as u64).await.unwrap();
            }
    ...
    fn automaton_batch_processing_is_efficient(
        ...
        rt.block_on(async {
            ...
            for chunk in events.chunks(batch_size) {
                ...
                ctx.wait_for_checkpoint_progress(&automaton_name, total_processed as u64).await.unwrap();
    ...
    fn automaton_error_handling_is_robust(
        ...
        rt.block_on(async {
            ...
            if !valid_events.is_empty() {
                ctx.wait_for_checkpoint_progress(&automaton_name, valid_events.len() as u64).await.unwrap();
            }
            ...
            ctx.wait_for_checkpoint_progress(&automaton_name, expected_minimum as u64).await.unwrap();
...
proptest! {
    ...
    fn automaton_memory_usage_is_bounded(
        ...
        rt.block_on(async {
            ...
            // Final verification
            ctx.wait_for_checkpoint_progress(&automaton_name, processed_count as u64).await.unwrap();
...
proptest! {
    ...
    fn multiple_automata_coordination_is_correct(
        ...
        rt.block_on(async {
            ...
            for automaton_name in automaton_names.iter() {
                ctx.wait_for_checkpoint_progress(automaton_name, events.len() as u64).await.unwrap();
            }
