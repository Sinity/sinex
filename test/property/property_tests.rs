    ...
    match (a, b) {
        (Value::Number(n1), Value::Number(n2)) => {
            if let (Some(f1), Some(f2)) = (n1.as_f64(), n2.as_f64()) {
                ...
                assert!(
                    (f1 - f2).abs() <= tolerance,
                    "Numbers not equal within tolerance: {} vs {}",
                    f1,
                    f2
                );
            } else {
    ...
    leaf.prop_recursive(
        ...
        |inner| {
            prop_oneof![
                ...
                prop::collection::hash_map("[a-zA-Z_][a-zA-Z0-9_]{0,30}", inner, 0..10,)
...
fn arb_timestamp() -> impl Strategy<Value = DateTime<Utc>> {
    ...
    (start_millis..=end_millis).prop_map(|millis| DateTime::from_timestamp_millis(millis).unwrap())
}
