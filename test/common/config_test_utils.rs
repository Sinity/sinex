...
pub mod validation {
    ...
    pub fn validate_observability_config() -> impl Fn(&ConfigValue) -> Result<()> {
        ConfigValidator::new()
            ...
            .validate_regex("observability.log_level", r"^(trace|debug|info|warn|error)$")
            .validate_custom(|config| {
        ...
        move |config: &ConfigValue| {
            ...
            if config.optional_str("observability.metrics_port").is_some() {
                if let Err(e) = validate_observability_config()(config) {
                    return Err(CoreError::configuration("Observability configuration invalid")
                        .with_source(e)
                        .build());
                }
            }
...
pub mod extraction {
    ...
    pub fn extract_sources_config(config: &ConfigValue) -> Result<SourcesTestConfig> {
        ...
        let filesystem_watch_paths = if filesystem_enabled {
            config.optional_str("sources.filesystem.watch_paths")
...
impl TestConfigFactory {
    ...
    pub fn create_config_invalid_value(field_path: &str, invalid_value: ConfigValue) -> ConfigValue {
        let mut config = test_configs::valid_database_config();
    ...
    pub fn create_randomized_config(seed: u64) -> ConfigValue {
        ...
        toml::from_str(&format!(r#"
            ...
            batch_size = {}
            flush_interval = "{}s"
        "#, seed, pool_size, buffer_size, (seed % 10) + 1, (seed % 30) + 1))
...
mod tests {
    ...
    fn test_config_validation_scenarios() {
        for scenario in scenarios::all_validation_scenarios() {
            ...
            match (result.is_ok(), scenario.should_validate) {
                ...
                (true, false) => {
                    panic!("Expected {} to fail validation, but it passed", scenario.name);
                }
                (false, true) => {
                    panic!("Expected {} to pass validation, but it failed: {:?}", scenario.name, result.unwrap_err());
                }
            }
        }
    }
    ...
    fn test_validation_chains_in_config() {
        ...
        if let Some(url) = config.optional_str("database.url") {
            let chain = ValidationChain::validate(url.to_string(), "database.url")
                .is_valid_url();
            
            assert!(!chain.is_valid());
            assert!(chain.errors().iter().any(|e| e.to_string().contains("invalid URL")));
        }
    }
}