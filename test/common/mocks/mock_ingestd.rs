...
impl MockIngestd {
    ...
    pub fn with_redis(mut self, redis: sinex_satellite_sdk::redis_client::RedisStreamClient) -> Self {
    ...
    pub async fn wait_for_events(&self, expected: usize, timeout_secs: u64) -> Result<()> {
...
impl IngestServiceTrait for MockIngestService {
        ...
        *count += 1;
        ...
        if self.config.store_in_database {
            if let Some(pool) = &self.pool {
                let _ = sinex_db::events::insert_event_with_validator(pool, &event, None).await;
            }
        }
    ...
    async fn send_event_batch(
        ...
        Ok(Response::new(BatchResponse {
            success_count,
            error_count,
            message: format!("Processed {}/{} events", success_count, success_count + error_count),
        }))
    }
}
...
impl MockIngestdBuilder {
    ...
    pub fn with_redis_publishing(mut self, redis: sinex_satellite_sdk::redis_client::RedisStreamClient, stream_key: &str) -> Self {
