...
pub type ProcessorFunction = Arc<dyn Fn(&RawEvent) -> Result<ProcessingResult> + Send + Sync>;
...
impl MockAutomaton {
                        ...
                        if batch_count > 0 {
                            ...
                            if config.save_checkpoints {
                                let state = CheckpointState {
                                    ...
                                    last_processed_id: None, // Would be set from last event
    ...
    pub async fn wait_for_processing(&self, expected: usize, timeout_secs: u64) -> Result<()> {
    ...
    pub async fn wait_for_checkpoint(&self, expected_count: u64, timeout_secs: u64) -> Result<CheckpointState> {
        let timeout = std::time::Duration::from_secs(timeout_secs);
        let start = std::time::Instant::now();
    ...
    for (_stream, messages) in result {
        for (message_id, fields) in messages {
            ...
            if config.processing_delay_ms > 0 {
                tokio::time::sleep(std::time::Duration::from_millis(config.processing_delay_ms)).await;
            }
            ...
            processing_results.lock().await.push(processing_result.clone());
            
            if processing_result.success {
                // Acknowledge the message
                let _: i64 = redis.xack(&config.stream_key, &config.consumer_group, &[&message_id]).await?;
                
                // Record as processed
                events_processed.lock().await.push(message_id);
                processed_count += 1;
            }
        }
    }
...
fn extract_event_from_fields(fields: &[(String, String)]) -> Result<RawEvent> {
...
impl MockAutomatonBuilder {
    ...
    pub fn build(self, pool: sqlx::PgPool, redis: sinex_satellite_sdk::redis_client::RedisStreamClient) -> MockAutomaton {
