...
impl MockSatellite {
    ...
    pub async fn wait_for_generation(&self, expected: usize, timeout_secs: u64) -> Result<()> {
    ...
    pub async fn wait_for_sending(&self, expected: usize, timeout_secs: u64) -> Result<()> {
        let timeout = std::time::Duration::from_secs(timeout_secs);
        let start = std::time::Instant::now();
...
fn generate_event(template: &EventTemplate, satellite_id: &str, sequence: usize) -> RawEvent {
    ...
    if let serde_json::Value::Object(ref mut map) = payload {
        map.insert("satellite_id".to_string(), serde_json::Value::String(satellite_id.to_string()));
        map.insert("sequence".to_string(), serde_json::Value::Number(serde_json::Number::from(sequence)));
        map.insert("timestamp".to_string(), serde_json::Value::String(chrono::Utc::now().to_rfc3339()));
    }
    
    sinex_events::RawEventBuilder::new(
        &template.source,
        &template.event_type,
        payload,
    ).build()
...
impl MockSatelliteBuilder {
    ...
    /// Set event template
    pub fn with_event_template(mut self, source: &str, event_type: &str, payload: serde_json::Value) -> Self {
