...
impl GenericEventBuilder {
    ...
    pub fn build(self) -> RawEvent {
        let mut builder = RawEventBuilder::new(self.source, self.event_type, self.payload);
...
impl ClipboardEventBuilder {
    ...
    pub fn build(self) -> RawEvent {
        let content = self
            .content
            .unwrap_or_else(|| "test clipboard content".to_string());
        ...
        let mut builder = RawEventBuilder::new(sources::CLIPBOARD, "content.changed", payload);
...
impl AgentEventBuilder {
    ...
    pub fn build(self) -> RawEvent {
        ...
        let mut builder = RawEventBuilder::new(sources::SINEX, &event_type, payload);
...
pub mod quick {
    ...
    pub fn fs_modified(path: &str) -> RawEvent {
        EventBuilder::filesystem().path(path).modified().build()
    }

    /// Quick filesystem deleted event
    pub fn fs_deleted(path: &str) -> RawEvent {
        EventBuilder::filesystem().path(path).deleted().build()
    }

    /// Quick terminal command event
    pub fn terminal_cmd(cmd: &str) -> RawEvent {
        EventBuilder::terminal().command(cmd).success().build()
    }
    ...
    pub fn agent_heartbeat(name: &str) -> RawEvent {
        EventBuilder::agent().name(name).heartbeat().build()
    }

    /// Quick agent error event
    pub fn agent_error(name: &str, error: &str) -> RawEvent {
        EventBuilder::agent().name(name).error(error).build()
    }
}
...
pub mod batch {
    ...
    pub fn fs_events(paths: &[&str]) -> Vec<RawEvent> {
        paths.iter().map(|path| quick::fs_created(path)).collect()
    }
