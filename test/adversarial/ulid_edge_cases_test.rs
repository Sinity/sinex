...
async fn test_ulid_max_timestamp_representation(ctx: TestContext) -> TestResult {
    ...
    println!("Max timestamp date: ~{} years from epoch", max_timestamp_ms / (365 * 24 * 60 * 60 * 1000));
...
async fn test_ulid_timestamp_wraparound_behavior(ctx: TestContext) -> TestResult {
    ...
    println!("Expected wrapped: {}", overflow_timestamp & ((1u64 << 48) - 1));
...
async fn test_ulid_concurrent_generation_safety(ctx: TestContext) -> TestResult {
    ...
    println!("Generated {} ULIDs in {:?}", total_ulids, elapsed);
    println!("Rate: {:.0} ULIDs/second", total_ulids as f64 / elapsed.as_secs_f64());
    ...
    assert_eq!(db_count as usize, expected_total - errors.len(), 
...
async fn test_ulid_random_component_distribution(ctx: TestContext) -> TestResult {
    ...
    for _ in 0..sample_size {
        ...
        *random_bytes_distribution.entry(random_component[0]).or_insert(0) += 1;
    }
    ...
    assert!(cv < 0.1, "Random component should have uniform distribution");
