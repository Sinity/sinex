...
async fn test_network_partition_during_processing(ctx: TestContext) -> TestResult {
            ...
            for attempt in 0..10 {
                ...
                if worker_id == 1 && attempt >= 5 {
                    ...
                    match fake_result {
                        Ok(_) => {
                            println!(
                                "Worker {}: Impossible - partition resolved instantly",
                                worker_id
                            );
                        }
                        Err(_) => {
    ...
    println!(
        "- Partition events: {}",
        partition_events.load(Ordering::SeqCst)
    );
    println!(
        "- Successful operations: {}",
        successful_operations.load(Ordering::SeqCst)
    );
    println!(
        "- Failed operations: {}",
        failed_operations.load(Ordering::SeqCst)
    );
...
async fn test_split_brain_scenario(ctx: TestContext) -> TestResult {
    ...
    println!(
        "- Brain A operations: {}",
        brain_a_operations.load(Ordering::SeqCst)
    );
    println!(
        "- Brain B operations: {}",
        brain_b_operations.load(Ordering::SeqCst)
    );
    println!(
        "- Conflicts detected: {}",
    ...
    if brain_a_operations.load(Ordering::SeqCst) > 0
        && brain_b_operations.load(Ordering::SeqCst) > 0
    {
        ...
        match TcpStream::connect(server_addr) {
            ...
            Err(e) => {
                ...
                if connection_count < 100 {
                    println!(
                        "  UNEXPECTED: Socket exhaustion at low count: {}",
                        connection_count
                    );
                }
