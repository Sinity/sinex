...
async fn test_event_causality_violation(ctx: TestContext) -> TestResult {
    ...
    for test_round in 0..10 {
        ...
        for child in child_events {
            ...
            let handle = tokio::spawn(async move {
                ...
                let parent_check = sqlx::query!(
                    "SELECT payload->>'processed' as processed FROM raw.events WHERE id::uuid = $1::uuid",
                    parent_id.to_uuid()
                )
        ...
        sqlx::query!(
            "UPDATE raw.events SET payload = payload || '{\"processed\": \"true\"}'::jsonb WHERE id::uuid = $1::uuid",
                ...
                match insert_event(&pool_clone, &event).await {
                    ...
                    Err(e) => {
                        ...
                        if e.to_string().contains("duplicate key") {
                            ...
                            println!("Worker {} insertion {} failed: {}", worker_id, insertion_id, e);
                        }
                    }
                }
            }
...
async fn test_data_consistency_under_concurrent_updates(ctx: TestContext) -> TestResult {
    ...
    let final_state = sqlx::query!(
        "SELECT payload->>'counter' as counter FROM raw.events WHERE id::uuid = $1::uuid",
        event_id.to_uuid()
    )
    .fetch_one(pool)
    .await?;
    
    let final_counter: i32 = final_state.counter.unwrap_or("0".to_string()).parse().unwrap_or(0);
    ...
    assert_eq!(final_counter as u64, successful, "Counter value should match successful updates");
...
async fn test_worker_coordination_microsecond_sync(ctx: TestContext) -> TestResult {
    ...
    for _i in 0..10 {
        ...
        db_events::insert_event_with_validator(pool, &event, None).await.unwrap();
    ...
    for worker_id in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            let claim1_result = sqlx::query!(
                "UPDATE raw.events SET payload = payload || jsonb_build_object('claimed_by', $2::text) WHERE id::uuid = $1::uuid",
                first_id.to_uuid(),
                worker_id.to_string()
            )
            .execute(&mut *tx)
...
async fn test_worker_load_balancing_concurrent(ctx: TestContext) -> TestResult {
        ...
        let handle = tokio::spawn(async move {
            ...
            loop {
                ...
                let claim_result = sqlx::query!(
                    ...
                    WHERE id::uuid IN (
                        SELECT id::uuid FROM raw.events 
    ...
    println!("  Load balance ratio: {:.2} (lower is better)", load_balance_ratio);
    ...
    assert!(load_balance_ratio < 3.0, "Load should be reasonably balanced");
...
async fn test_database_transaction_isolation(ctx: TestContext) -> TestResult {
    ...
    for tx_id in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            ...
            let initial_read = sqlx::query!(
                "SELECT payload->>'value' as value FROM raw.events WHERE id::uuid = $1::uuid",
                event_id.to_uuid()
            )
            .fetch_one(&mut *tx)
            .await
            ...
            let initial_value: i32 = initial_read.value.unwrap_or("0".to_string()).parse().unwrap_or(0);
            ...
            sqlx::query!(
                "UPDATE raw.events SET payload = jsonb_set(payload, '{value}', $2::text::jsonb) WHERE id::uuid = $1::uuid",
                event_id.to_uuid(),
                new_value.to_string()
            )
            .execute(&mut *tx)
            ...
            let final_value: i32 = final_read.value.unwrap_or("0".to_string()).parse().unwrap_or(0);
            
            if final_value != new_value {
                violations.fetch_add(1, Ordering::SeqCst);
                println!("Transaction {} isolation violation: expected {}, got {}", tx_id, new_value, final_value);
            }
    ...
    println!("  Final value: {}", final_state.value.unwrap_or("N/A".to_string()));
...
async fn test_database_lock_contention(ctx: TestContext) -> TestResult {
        ...
        let handle = tokio::spawn(async move {
            for _attempt in 0..5 {
                ...
                let lock_result = sqlx::query!(
                    "SELECT payload FROM raw.events WHERE id::uuid = $1::uuid FOR UPDATE",
                    event_id.to_uuid()
                )
                .fetch_one(&pool_clone)
                .await;
                ...
                match lock_result {
                    Ok(_) => {
                        ...
                        if lock_time > Duration::from_millis(50) {
                            ...
                            println!("Worker {} experienced lock contention: {:?}", worker_id, lock_time);
                        }
                        ...
                        sqlx::query!(
                            "UPDATE raw.events SET payload = jsonb_set(payload, '{lock_count}', ((payload->>'lock_count')::int + 1)::text::jsonb) WHERE id::uuid = $1::uuid",
                            event_id.to_uuid()
                        )
                        .execute(&pool_clone)
                        .await
    ...
    let final_state = sqlx::query!(
        "SELECT payload->>'lock_count' as lock_count FROM raw.events WHERE id::uuid = $1::uuid",
        event_id.to_uuid()
    )
    .fetch_one(pool)
    .await?;
    
    let final_lock_count: i32 = final_state.lock_count.unwrap_or("0".to_string()).parse().unwrap_or(0);
    ...
    assert_eq!(final_lock_count as u64, successes, "Lock count should match successful operations");
