...
async fn test_multiple_concurrent_shutdown_signals(_ctx: TestContext) -> TestResult {
                ...
                if shutdown_counter.load(Ordering::SeqCst) > 0 {
                    ...
                    for resource in resources_held {
                        if shutdown_counter.load(Ordering::SeqCst) > 2 {
                            ...
                            println!(
                                "Task {} LEAKED resource {} due to concurrent shutdown",
                                task_id, resource
                            );
                        } else {
                            // Successful cleanup
    ...
    println!(
        "- Resource leaks: {}",
        resource_leaks.load(Ordering::SeqCst)
    );
...
#[sinex_test]
async fn test_event_router_state_corruption(ctx: TestContext) -> TestResult {
    let pool = ctx.pool();
    ...
    let state_events = vec![
        (
            "file.created",
            json!({"path": "/test.txt", "state": "created"}),
        ),
        (
            "file.modified",
            json!({"path": "/test.txt", "state": "modified"}),
        ),
        (
            "file.deleted",
            json!({"path": "/test.txt", "state": "deleted"}),
        ),
        (
            "file.modified",
            json!({"path": "/test.txt", "state": "modified_after_delete"}),
        ), // Invalid!
    ];
                ...
                Ok(_) => {
                    ...
                    if event_type == "file.modified"
                        && event.payload.get("state")
                            == Some(&serde_json::json!("modified_after_delete"))
                    {
    ...
    if corruption_detected.load(Ordering::SeqCst) > 0 {
        println!(
            "STATE MACHINE VIOLATION: Detected {} impossible transitions",
...
#[sinex_test]
async fn test_worker_state_machine_corruption(ctx: TestContext) -> TestResult {
    let pool = ctx.pool();

    // Create a job that will be processed by workers
    let test_event = crate::common::events::generic_adversarial_event(
        "test",
        "worker.test",
        json!({"test": true}),
        None,
                ...
                match complete_result {
                    Ok(result) => {
                        if result.rows_affected() == 0 {
                            ...
                            println!(
                                "Worker {} FAILED to complete - state changed by another worker!",
                                worker_id
                            );
                        } else {
    ...
    println!(
        "- Successful claims: {}",
        successful_claims.load(Ordering::SeqCst)
    );
    println!(
        "- State violations: {}",
        state_violations.load(Ordering::SeqCst)
    );
