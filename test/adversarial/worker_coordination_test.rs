...
async fn test_worker_claim_exact_same_microsecond(ctx: TestContext) -> TestResult {
    ...
    println!(
        "  Total claims: {} (should be {})",
        total_claimed,
        event_ids.len()
    );
    println!(
        "  Double claims detected: {}",
        double_claims.load(Ordering::SeqCst)
    );
...
async fn test_dead_worker_holding_locks(ctx: TestContext) -> TestResult {
    ...
    // Insert work item
    let work_event = crate::common::events::generic_adversarial_event(
        "test",
        "critical.work",
            ...
            match claim_result {
                Ok(Ok(result)) => {
                    if result.rows_affected() > 0 {
                        ...
                        format!(
                            "Worker {} found event already claimed after {:?}",
                            worker_id, elapsed
                        )
                    }
                }
                Ok(Err(e)) => {
                    format!(
                        "Worker {} query error after {:?}: {}",
                        worker_id, elapsed, e
                    )
                }
                Err(_) => {
                    format!(
                        "Worker {} TIMEOUT after 5s - zombie holding lock!",
                        worker_id
                    )
                }
            }
    ...
    match recovery_result {
        Ok(result) => {
            println!(
                "  Recovery successful: {} rows updated",
...
async fn test_mass_worker_wakeup_thundering_herd(ctx: TestContext) -> TestResult {
    ...
    let _coordinator = Arc::new(
        crate::common::timing_optimization::replacements::WorkerReadinessCoordinator::new(100),
    );
    ...
    println!(
        "  Connection errors: {}",
        connection_errors.load(Ordering::SeqCst)
    );
