...
async fn test_unbounded_file_descriptor_explosion(ctx: TestContext) -> TestResult {
    ...
    for path in paths.iter().take(100) {
        // Limit to avoid actual system issues
        let path = path.clone();
    ...
    if watcher_count < 100 {
        println!(
            "RESOURCE LIMIT: System restricted file watchers to {}",
            watcher_count
        );
    }
    Ok(())
...
"#,
        ...
        if let Ok(Some(config)) =
            tokio::time::timeout(tokio::time::Duration::from_millis(500), update_rx.recv()).await
        {
    ...
    assert!(
        ...
        "Should handle at least some config reloads"
    );
    assert!(
        accumulated_configs.len() <= 100,
        "Should not accumulate unlimited configs in memory"
    );
    ...
    for config in &accumulated_configs {
        assert!(
            ...
            "Config should maintain enabled events"
        );
    }
    Ok(())
}
...
async fn test_string_concatenation_memory_bomb(ctx: TestContext) -> TestResult {
    ...
    for i in 0..20 {
        ...
        match serde_json::to_string(&event) {
            Ok(_) => println!(
                "Iteration {}: String size {} - OK",
                i,
                expanding_string.len()
            ),
            Err(e) => {
                println!(
                    "Iteration {}: String size {} - FAILED: {}",
                    i,
                    expanding_string.len(),
                    e
                );
...
async fn test_collector_event_queue_overflow() {
    ...
    println!(
        "- Drop rate: {:.2}%",
        (total_dropped as f64 / 10000.0) * 100.0
    );
