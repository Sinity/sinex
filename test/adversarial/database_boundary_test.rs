...
async fn test_concurrent_btree_index_splits(ctx: TestContext) -> TestResult {
    ...
    let query_handle = {
        ...
        tokio::spawn(async move {
            ...
            for _ in 0..100 {
                ...
                struct FakeCountRecord {
                    count: Option<i64>,
                }
                let count_result: Result<FakeCountRecord, sqlx::Error> =
                    Ok(FakeCountRecord { count: Some(count) });
    ...
    println!(
        "  Query inconsistencies during splits: {}",
...
async fn test_events_spanning_chunk_boundary(ctx: TestContext) -> TestResult {
    ...
    let boundary_events = vec![
        (
            chunk_boundary - chrono::Duration::milliseconds(1),
            "before_boundary",
        ),
    ...
    match agg_result {
        Ok(record) => {
            ...
            if record.count.unwrap_or(0) != 3 {
                println!(
                    "    CHUNK ISSUE: Expected 3 events, got {}",
    ...
    let query_tasks = vec![
        ...
        tokio::spawn({
            ...
            async move {
                ...
                struct FakeRangeRecord {
                    count: Option<i64>,
                }
                let result: Result<FakeRangeRecord, anyhow::Error> =
                    Ok(FakeRangeRecord { count: Some(count) });
