...
async fn test_watching_special_files() {
    ...
    for special_file in special_files {
        ...
        match fs::metadata(special_file) {
            Ok(metadata) => {
                println!("  Metadata: size={}, is_file={}, is_dir={}", 
                         metadata.len(), metadata.is_file(), metadata.is_dir());
...
async fn test_fifo_pipe_watching() {
    ...
    // Create FIFO pipe
    let mkfifo_result = Command::new("mkfifo")
        .arg(&fifo_path)
        .output();
    ...
    match mkfifo_result {
        Ok(output) => {
            if output.status.success() {
                ...
                println!("Failed to create FIFO: {}", String::from_utf8_lossy(&output.stderr));
            }
        }
        Err(e) => {
    ...
    for worker_id in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            for i in 0..100 {
                ...
                // Create file
                match fs::write(&file_path, format!("content from worker {} iteration {}", worker_id, i)) {
                    Ok(_) => {
...
async fn test_watching_symlink_cycles() {
    ...
    for (name, link_path) in [("link_a", &link_a), ("link_b", &link_b), ("link_c", &link_c)] {
        match fs::metadata(link_path) {
            Ok(_) => {
                println!("ISSUE: {} metadata resolved despite circular reference!", name);
            }
            Err(e) => {
