...
async fn test_config_file_replaced_with_symlink() {
    ...
    fs::write(&sensitive_file, "secret_key=very_secret_value\npassword=admin123").unwrap();
    ...
    match timeout(Duration::from_secs(5), update_rx.recv()).await {
        Ok(Some(new_config)) => {
            ...
            if config_debug.contains("secret") || config_debug.contains("password") {
                println!("CRITICAL: Sensitive data detected in config: {}", config_debug);
            }
        }
        Ok(None) => {
...
"#;
    ...
    for (i, &byte) in config_bytes.iter().enumerate() {
        ...
        println!("Wrote byte {} of {}: '{}'", i + 1, config_bytes.len(), byte as char);
        
        // Check if config watcher triggered on partial file
        match timeout(Duration::from_millis(100), update_rx.recv()).await {
            Ok(Some(_new_config)) => {
                println!("WARNING: Config reloaded from partial file at byte {}", i + 1);
    ...
    let collector_handle = tokio::spawn(async move {
        ...
        while let Ok(Some(config)) = timeout(Duration::from_millis(100), update_rx.recv()).await {
            ...
            if configs_received.len() > 50 {
                println!("WARNING: {} configs accumulated in memory", configs_received.len());
            }
        }
