...
async fn test_event_registry_creation(_ctx: TestContext) -> TestResult {
    ...
    assert!(!registry.event_types.is_empty(), "Registry should contain event types");
    
    // Verify some expected event types are present
    assert!(registry.event_types.contains(&"file.created"), "Registry should contain file.created event type");
    assert!(registry.event_types.contains(&"command.executed"), "Registry should contain command.executed event type");
    assert!(registry.event_types.contains(&"window.focused"), "Registry should contain window.focused event type");
    Ok(())
}
...
async fn test_event_registry_get_event_type(_ctx: TestContext) -> TestResult {
    ...
    assert!(source.is_some(), "Should find source for file.created event");
    pretty_assertions::assert_eq!(source.unwrap(), "filesystem", "file.created should map to filesystem source");
    
    // Test getting non-existent event type
    let unknown = registry.source_for_event("nonexistent.event");
    assert!(unknown.is_none(), "Should not find source for nonexistent event type");
    Ok(())
}
...
async fn test_event_registry_get_source_events(_ctx: TestContext) -> TestResult {
    ...
    assert!(!fs_events.is_empty(), "Filesystem source should have events");
    assert!(fs_events.contains(&"file.created"), "Filesystem should contain file.created event");
    assert!(fs_events.contains(&"file.modified"), "Filesystem should contain file.modified event");
    assert!(fs_events.contains(&"file.deleted"), "Filesystem should contain file.deleted event");
    
    // Test terminal source
    let terminal_events = registry.events_for_source("terminal.kitty");
    assert!(!terminal_events.is_empty(), "Terminal source should have events");
    assert!(terminal_events.contains(&"command.executed"), "Terminal should contain command.executed event");
    
    // Test non-existent source
    let unknown_events = registry.events_for_source("unknown_source");
    assert!(unknown_events.is_empty(), "Unknown source should have no events");
    Ok(())
}
...
async fn test_event_registry_all_sources(_ctx: TestContext) -> TestResult {
    ...
    assert!(sources.contains(&"filesystem"), "Should contain filesystem source");
    assert!(sources.contains(&"terminal.kitty"), "Should contain terminal.kitty source");
    assert!(sources.contains(&"window_manager.hyprland"), "Should contain window_manager.hyprland source");
    assert!(sources.contains(&"clipboard"), "Should contain clipboard source");
    Ok(())
}
...
async fn test_event_registry_event_type_properties(_ctx: TestContext) -> TestResult {
    ...
    pretty_assertions::assert_eq!(source.unwrap(), "filesystem", "file.created should map to filesystem");
    ...
    assert!(cmd_source.is_some(), "Should find source for command.executed");
    pretty_assertions::assert_eq!(cmd_source.unwrap(), "terminal.kitty", "command.executed should map to terminal.kitty");
    Ok(())
}
...
async fn test_event_registry_validate_payload(_ctx: TestContext) -> TestResult {
    ...
    assert!(registry.schema_for_event("file.modified").is_some(), "Should find schema for file.modified");
    assert!(registry.schema_for_event("file.deleted").is_some(), "Should find schema for file.deleted");
    assert!(registry.schema_for_event("command.executed").is_some(), "Should find schema for command.executed");
    Ok(())
}
...
async fn test_event_registry_immutability(_ctx: TestContext) -> TestResult {
    ...
    pretty_assertions::assert_eq!(source1, source2, "Registry instances should return same source mappings");
    Ok(())
}
...
async fn test_event_registry_schema_lookup(_ctx: TestContext) -> TestResult {
    ...
    assert!(registry.schema_generators.contains_key("file.created"), "Should have schema generator for file.created");
    Ok(())
}
...
async fn test_event_registry_event_source_mapping(_ctx: TestContext) -> TestResult {
    ...
    for event in &fs_events {
        ...
        pretty_assertions::assert_eq!(source, Some("filesystem"), "Filesystem event {} should map to filesystem source", event);
    }
    
    // Verify terminal events are mapped to terminal source
    for event in &terminal_events {
        let source = registry.source_for_event(event);
        pretty_assertions::assert_eq!(source, Some("terminal_kitty"), "Terminal event {} should map to terminal_kitty source", event);
    }
    Ok(())
}
...
async fn test_event_registry_concurrent_access(_ctx: TestContext) -> TestResult {
    ...
    for i in 0..10 {
        ...
        let handle = thread::spawn(move || {
            ...
            assert!(source.is_some(), "Should find source for event type {}", event_type);
    ...
    for handle in handles {
        ...
        assert!(result == "filesystem" || result == "terminal.kitty", "Source should be filesystem or terminal.kitty, got: {}", result);
    }
    Ok(())
}
...
async fn test_event_registry_with_real_events(_ctx: TestContext) -> TestResult {
    ...
    for event_name in filesystem_events {
        ...
        if source.is_some() {
            pretty_assertions::assert_eq!(source.unwrap(), "filesystem", "Filesystem event {} should map to filesystem source", event_name);
        }
    }
    ...
    for event_name in terminal_events {
        let source = registry.source_for_event(event_name);
        if source.is_some() {
            pretty_assertions::assert_eq!(source.unwrap(), "terminal.kitty", "Terminal event {} should map to terminal.kitty source", event_name);
        }
    }
    Ok(())
}