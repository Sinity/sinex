...
async fn test_database_connection(ctx: TestContext) -> TestResult {
    ...
    let result_validation = assert_with_validation(result, "db_test_result")
        .custom(|&val| val == 1, "should equal 1");
...
async fn test_multi_assertion_batch(ctx: TestContext) -> TestResult {
    ...
    let events = vec![
        EventBuilder::filesystem().path("/test/file1.txt").created().build(),
