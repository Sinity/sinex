...
"#;
    ...
    async fn test_practical_significance(_ctx: TestContext) -> TestResult {
        ...
        println!(
            "Can distinguish between {:.20} states",
        ...
        println!(
            "  Ratio: {:.1e}× smaller than thermal noise",
            energy_content / thermal_energy
        );
        ...
        println!("  Our entropy gain: {:.3e} bits", entropy_gain);
        println!(
            "  Security improvement: {:.3e}%",
            (entropy_gain / 128.0) * 100.0
        );
    ...
    async fn test_bit_layout_verification(_ctx: TestContext) -> TestResult {
        ...
        if same_ms_ulids.len() > 1 {
            println!(
                "\nVerifying monotonic ordering within timestamp {}:",
                target_timestamp
            );
