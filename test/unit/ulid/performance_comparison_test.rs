...
fn test_ulid_monotonic_performance_validation() {
    ...
    println!("Batch Generation with Ordering Validation ({} batches of {}):", batches, batch_size);
    ...
    println!("  Ordering violations: {}/{} ({:.4}%)", 
             ordering_violations, total_ulids,
    ...
    println!("  Required sustained: {:.0} events/sec", required_events_per_sec);
    println!("  Required burst:     {:.0} events/sec", required_burst_capacity);
    println!("  Required latency:   < {:.0} ns/op", required_ns_per_op);
    println!();
    
    println!("Actual performance:");
    if ops_per_sec >= required_burst_capacity {
        println!("  ✅ Sustained throughput: {:.0} events/sec (exceeds burst requirements)", ops_per_sec);
    } else if ops_per_sec >= required_events_per_sec {
        println!("  ✅ Sustained throughput: {:.0} events/sec (meets requirements)", ops_per_sec);
    } else {
        println!("  ❌ Sustained throughput: {:.0} events/sec (below requirements)", ops_per_sec);
    }
    
    if ns_per_op <= required_ns_per_op {
        println!("  ✅ Latency: {:.0} ns/op (meets requirements)", ns_per_op);
    } else {
        println!("  ❌ Latency: {:.0} ns/op (exceeds requirements)", ns_per_op);
    }
    ...
    if ordering_violations == 0 {
        ...
        println!("  ⚠️  Ordering: {} violations detected", ordering_violations);
    }
    ...
    assert!(ops_per_sec >= required_events_per_sec, 
    ...
    assert!(ns_per_op <= required_ns_per_op,
