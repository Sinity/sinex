...
async fn test_checkpoint_invalid_ulid_parsing(ctx: TestContext) -> TestResult {
    ...
    let invalid_ulids = vec![
        ...
        ("01234567890123456789012345", "Wrong length (25 chars instead of 26)"),
        ...
        ("0123456789ABCDEFGHIJKLMNOP", "Mixed case (should be uppercase)"),
    ...
    for (invalid, description) in invalid_ulids {
        ...
        assert!(!result.is_valid_format, "Database should reject invalid ULID format");
    }
    ...
    for (timestamp_secs, description) in edge_timestamps {
        ...
        match DateTime::from_timestamp(timestamp_secs, 0) {
            Some(dt) => {
                ...
                // Verify round-trip
                assert_eq!(result.epoch_secs.unwrap_or(0), timestamp_secs,
                    "Timestamp round-trip failed for {}", description);
            }
            None => {
    ...
    for (json_val, description) in edge_cases {
        ...
        match serde_json::to_string(&json_val) {
            Ok(json_str) => {
                ...
                println!("  DB type: {:?}, size: {:?} bytes", 
                    result.json_type, result.size_bytes);
                
                // Special handling for infinity/NaN
                if json_val.get("number").and_then(|v| v.as_f64()).map(|f| f.is_infinite() || f.is_nan()).unwrap_or(false) {
...
async fn test_database_constraint_violations(ctx: TestContext) -> TestResult {
    ...
    assert!(duplicate_result.is_err(), "Duplicate primary key should fail");
...
async fn test_transaction_rollback_scenarios(ctx: TestContext) -> TestResult {
    ...
    assert_eq!(count_in_tx.unwrap_or(0), 1, "Event should exist in transaction");
    ...
    assert_eq!(count_after_rollback.unwrap_or(0), 0, "Event should not exist after rollback");
...
async fn test_query_builder_invalid_operations(ctx: TestContext) -> TestResult {
    ...
    for invalid_col in invalid_columns {
        ...
        let builder = QueryBuilder::select("test_table")
            .columns(&[invalid_col]);
