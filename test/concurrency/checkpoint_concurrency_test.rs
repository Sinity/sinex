...
async fn test_concurrent_checkpoint_updates_basic(ctx: TestContext) -> TestResult {
    ...
    println!("  Rate: {:.0} updates/sec", successful as f64 / duration.as_secs_f64());
    
    // Verify final state
    let final_checkpoint = get_checkpoint(pool, processor_name, consumer_group, consumer_name).await?;
...
async fn test_checkpoint_lost_update_prevention(ctx: TestContext) -> TestResult {
    ...
    println!("  Count: {}, Version: {}", checkpoint1.processed_count, checkpoint1.checkpoint_version);
    ...
    // Verify final state
    let final = get_checkpoint(pool, processor_name, consumer_group, consumer_name).await?;
    assert_eq!(final.processed_count, 110, "Count should be updated by tx1");
    assert_eq!(final.checkpoint_version, 2, "Version should be incremented");
...
async fn test_checkpoint_optimistic_locking(ctx: TestContext) -> TestResult {
    ...
    for worker_id in 0..workers {
        ...
        let handle = tokio::spawn(async move {
            // Read current state
            let current = get_checkpoint(&pool, processor_name, consumer_group, consumer_name).await?;
            
            println!("Worker {} read version {}", worker_id, current.checkpoint_version);
            ...
            match result {
                Some(row) => {
                    println!("Worker {} successfully updated to version {}", worker_id, row.new_version);
                    Ok((worker_id, true))
                }
                None => {
    ...
    assert_eq!(successful_workers.len(), 1, "Only one worker should succeed with optimistic locking");
...
async fn test_checkpoint_high_contention_stress(ctx: TestContext) -> TestResult {
            ...
            tasks.spawn(async move {
                ...
                for update_id in 0..updates_per_worker {
                    ...
                    match update_checkpoint_atomic(
                        ...
                        Err(e) => {
                            if update_id == 0 {
                                eprintln!("Consumer {} worker {} error: {}", 
    ...
    println!("  Expected updates: {}", consumer_count * workers_per_consumer * updates_per_worker);
    println!("  Successful updates: {}", total);
    println!("  Contention events: {}", contentions);
    println!("  Duration: {:?}", duration);
    println!("  Throughput: {:.0} updates/sec", total as f64 / duration.as_secs_f64());
...
async fn test_checkpoint_history_consistency(ctx: TestContext) -> TestResult {
    ...
    for i in 0..update_count {
        ...
        let handle = tokio::spawn(async move {
            update_checkpoint_atomic(
                ...
                Some(event_id.clone()),
                1,
            ).await.map(|success| (i, event_id, success))
        });
    ...
    println!("Successful updates: {}/{}", successful_updates.len(), update_count);
    ...
    assert_eq!(
        latest.processed_count, successful_updates.len() as i64,
