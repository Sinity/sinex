# Source-Worker Drain Protocol

This record defines the shutdown and recovery contract for source-worker units
that emit material-provenance events from live or staged source material. It is
the shared protocol for acquisition jobs, parser lanes, child-process sidecars,
private-mode transitions, and continuity diagnostics.

The runtime implementation starts in
`crate/core/sinex-source-worker/src/drain.rs`. The database material lifecycle is
carried by `raw.source_material_registry.status` and the source-material
repository.
...
`SourceWorkerDrainController` owns a per-source-unit phase machine. It wraps the
...
source-worker drain phase is the lifecycle clock; source-specific code only
...
source identities. The parent source-worker unit owns:
...
Current narrow proof exists in the source-worker drain obligation and the
