...
# Paste contents of .tls/server.pem, save and exit

agenix -e secrets/sinex-gateway-key.age
# Paste contents of .tls/server-key.pem, save and exit

agenix -e secrets/sinex-ca.age
# Paste contents of .tls/ca.pem, save and exit
```

**Important**: Only encrypt **development** certificates. Production certificates should come from a proper CA (Let's Encrypt, internal PKI).
...
2. **Store in NixOS configuration** (not recommended for production):
   ```nix
   {
     services.sinex.core.gateway = {
       ...
       tlsClientCAFile = "${./dev-certs}/ca.pem";
     };
   }
   ```

3. **Or encrypt with agenix** (recommended even for dev):
   ```bash
   agenix -e secrets/dev-gateway-cert.age
   # Better: keeps secrets out of /nix/store
   ```
...
2. **Re-encrypt with agenix**:
   ```bash
   agenix -e secrets/sinex-gateway-cert.age
   # Update with new certificate content
   ```
...
```nix
{
  # Generate dev certs in project: xtask xtr tls generate-dev-certs
  # Encrypt with agenix
