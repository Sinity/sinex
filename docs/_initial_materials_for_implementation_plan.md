...
- Example: /run/sinex/ingest.sock (prod) vs
  ...
  sinex-telemetry exists but isn't integrated into satellites:
...
- **Original Point:** `sinex-telemetry` exists but is not used.
- **The Gap:** This is correct. The `sinex-telemetry` crate (`file-140` to `file-156`) is well-designed but there are no calls to it from the core SDK or satellites.
