...
Shared infrastructure owns registration, invalidation, replay scheduling, trace
shape, and generic object-event indexes. Per-domain code owns domain semantics:
task status transitions, account balance rules, health hypothesis state,
project membership rules, and typed state validation.

## Projection Records

The shared schema target is a reducer registry plus a current-object index:
...
Reducers consume immutable events and produce rebuildable projections. Replay
therefore invalidates affected object keys rather than mutating historical
state in place.
...
3. Late-arriving events follow the reducer's settlement policy from
   `late-arriving-event-temporal-coordination.md`: they can leave state
   provisional, trigger reconciliation, or mark a conflict.
4. High-fan-in outputs use a derivation scope from
   `high-fan-in-lineage.md` when the exact event set is too large for inline
   `source_event_ids[]`.
...
`sinexctl trace` should treat a current object as a projection node:
