...
The **live tool inventory** is owned by `crate/cli/src/mcp.rs` and
validated by `crate/cli/tests/validation_test.rs`. This record is the
...
The server may read from existing gateway/RPC/CLI query surfaces. It
...
- expose runtime privacy, health, node, ingestor, automata, replay,
  task, document, semantic-lane, and ingestd telemetry read models;
...
proposal/judgment or an existing authenticated gateway command with
...
explicit policy work, not a quiet addition to `crate/cli/src/mcp.rs`:
...
- no workbench-inspect tool until its gateway read surface can enforce
...
Open or link follow-up issues when a desired tool lacks a gateway/RPC
read surface. Do not silently fall back to ad-hoc SQL as the stable
backend.

**Related:** `docs/architecture/operator-surfaces.md`,
`docs/architecture/runtime-target-boundaries.md`, issue `#1105`.
