...
Status: design record. Supersedes target-vision/reference/privacy-and-operations.md §5.

This is a deliberately small doctrine. For a single-user self-hosted system,
the realistic threat model (`threat-model.md` T1, T3) collapses
...
- Decryption happens application-side, never inside Postgres. Export
  (`gdpr-rights-surface.md`) consumes the key in-process when
  `--decrypt` is passed.
...
pgsodium (column-level DB-side encryption) is exploratory. The
`Strategy::Encrypt` token format is forward-compatible: tokens can be
decrypted application-side and re-encrypted into pgsodium columns when
pgsodium graduates to the active roadmap. It is not blocking on the
baseline at-rest design.
...
| `sinex-gateway-admin-token` | Gateway admin RPC bearer | agenix | `/run/agenix/sinex-gateway-admin-token` | 0400, sinex user |
...
- Keys are generated once and managed through agenix (`age.secrets.*` in
  the NixOS configuration; see `deployment-topology.md` secret inventory).

### Rotation

For the privacy key:

1. Generate: `xtask privacy key generate --out /tmp/new-privacy.key`.
2. Re-encrypt all `⌜enc:v1:…⌝` tokens in the database using
   `sinexctl privacy rekey --old-key <old> --new-key <new>`. **This command
   is planned, not yet implemented.** Until it lands, key rotation is not
   operationally supported and the design must not assume frequent
   rotation.
3. Update the agenix secret with the new key.
4. Deploy the NixOS config to rotate `/run/agenix/sinex-privacy-key`.
5. Delete the old key file.

For PostgreSQL credentials and gateway admin tokens: regenerate the agenix
secret, redeploy, restart the affected service. Token-suffix RBAC has no
revocation surface today (`runtime-boundaries.md`).

## Operator Audit

`sinexctl doctor` should surface:

- Whether the PostgreSQL data directory is on a LUKS-backed partition.
- Whether the CAS root is on a LUKS-backed partition.
- Whether any sync tool (syncthing, rclone) has the PostgreSQL data dir,
  CAS root, or agenix run dir on its include set.
- Whether configured offsite backups are wrapped with `age` (or
  equivalent) before upload.
- For any future CAS remote: whether an explicit encryption policy is
  declared.

The NixOS module should _assert_ (or at minimum warn loudly) when the
PostgreSQL data directory is not on a LUKS-encrypted filesystem.
...
- **pgsodium configuration**: noted as forward-compatible; design when
  promoted.
...
- `docs/architecture/threat-model.md` (T1, T3, T5)
- `docs/architecture/deployment-topology.md` (agenix secret inventory,
  service-user permissions)
- `docs/architecture/gdpr-rights-surface.md` (`--decrypt` and
  `--encrypt-output` for exports)
