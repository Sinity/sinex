...
`command.rs`: remove `_json: bool` parameter; update the single call site in `lib.rs`.
...
Keep `--all` as the sole override. The default IS affected mode. Background serialization
changes from `if !self.affected { args.push("--affected=false") }` to
`if self.all { args.push("--all") }`.

### B2. Remove the `xtr` namespace entirely
`xtr` disappears. Each child is promoted:
- `xtask xtr ci` → `xtask ci` with `#[clap(hide = true)]`
...
- `xtask db migrate` — preflight runs declarative convergence; no migration concept
...
via lazy migration `ensure_live_stage_column()`.
...
Lazy migration columns on `invocations`: `pre_fix_errors`, `pre_fix_warnings`, `pre_fix_fixable`.
...
**Table definition** (lazy migration via `ensure_trace_events_table()`, same pattern as
`ensure_diagnostic_columns()`):
