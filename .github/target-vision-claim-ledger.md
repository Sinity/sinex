...
| Claim ID | Claim | Status | Evidence source | Authority | Proof / promotion gate | Notes |
|---|---|---|---|---|---|---|
| TV-001 | Bus-first/source-contract staged interpretation is the current ingestion spine; old per-source ingestor prose is historical unless explicitly re-promoted. | verified | `/realm/project/sinex-target-vision/reference/historical-architecture.md`; source execution issues | #1054, #1126, #1223, #1225; `crate/sinexd/src/sources/source_contracts/` | Production-path source tests and source contract registrations stay green. | Supersedes raw/reference wording that assumes one long-lived ingestor crate per source family. |
| TV-002 | Event source/type naming must be treated as semantic taxonomy, not ad hoc strings. | issue_backed | `/realm/project/sinex-target-vision/reference/design-intent.md`; event taxonomy work | #744 | Promote only through schema registry updates and call-site verification. | The old event-taxonomy/source catalog docs were pruned as stale generated-catalog authority. |
...
| TV-012 | Settlement/ErrorClass adoption, source health, `pub mod testing`, COPY routing, cascade trigger, DB credential redaction are open architectural fragilities. | verified | `report/architecture.md` (Architectural Fragilities table); `reference/sdk-assessment.md` (Related Issues) | #1009/#1010/#754/#988/#951/#986/#995 all closed | Maintain regression coverage in source production matrix (#1367). | Fragility table already annotated "closed" but historical prose persists across vision files. |
...
| TV-014 | Bus-First / source-contract host architecture is target but legacy domain ingestors remain. | implementation_ready | `report/architecture.md` (System Shape); `report/work-ahead.md` (Cluster 1-2) | #1054 (open spine) + #1081, #1097, #1098, #1057, #1067, #1100, #1064 all closed | Active migration tracked in #1126 execution plan; per-source production smoke matrix already lands via #1367. | Treat #1054 as the live execution authority. |
...
| TV-022 | Source-runtime drain / in-flight material shutdown protocol. | issue_backed | `report/architecture.md` (Three-Phase Source Startup §); `reference/prescriptive-ideas.md` | #1125 | Land drain semantics + recovered_partial markers + continuity surface. | — |
...
| TV-034 | Declarative SQL derivation engine (SQL-as-Automaton). | issue_backed | `report/intelligence.md` (SQL-as-Automaton); `reference/design-rationale.md` | #1117 (declarative SQL derivation engine) | Ship SQL executor automaton type under existing AutomatonRuntime. | Former global doc dissolved into #1117 comment on 2026-06-04. |
...
| TV-062 | Per-domain ingestor binaries (`sinex-fs-ingestor`, …) are ACTIVE services | superseded | `Cargo.toml`: no per-domain ingestor crates; source contracts hosted by sinexd post-Wave-B. |
...
| TV-072 | "8 existing ingestors/automatons" baseline count | superseded | Post-Wave-B: 13 automata specs + 20+ source contract modules in `crate/sinexd/src/sources/source_contracts/`. |
...
| TV-074 | `sinex-fs-ingestor` watches the vault directory | superseded | Binary dissolved; `fs` source contract hosted in sinexd (`crate/sinexd/src/sources/source_contracts/fs/mod.rs`). |
...
| TV-082 | `cli-tui-design.md` "sinex-terminal-ingestor captures the execution event" | superseded | Terminal source contract hosted in sinexd. Fixed in `1ea30e4`. |
...
| TV-093 | `work-ahead.md` "source-unit host (#1081) will eventually consolidate the 5 ingestors" | superseded | #1081 landed — `sinexd` is the unified source host; per-ingestor crates dissolved (Wave-B). |
...
| TV-095 | `media-document-processing.md` `DocumentIngestorNode (sinex-document-ingestor)` separate crate | superseded | Dissolved; logic at `crate/sinexd/src/sources/source_contracts/document/`. |
...
| TV-102 | `A-system-hardware.md` `**RuntimeActor:** sinex-fs-ingestor` / `sinex-system-ingestor` | superseded | Per-domain binaries dissolved; source contracts at `crate/sinexd/src/sources/source_contracts/`. status_note added (`6ec6d05`). |
| TV-103 | `B-desktop-interaction.md` `**RuntimeActor:** sinex-desktop-ingestor` / `sinex-system-ingestor` | superseded | Source contracts at `…/source_contracts/desktop/` + `system/`. status_note added (`6ec6d05`). |
| TV-104 | `D-web-browser.md` `**RuntimeActor:** sinex-browser-ingestor` | superseded | Browser source contract at `…/source_contracts/browser/`. status_note added (`6ec6d05`). |
| TV-105 | `H-media.md` `**RuntimeActor:** sinex-desktop-ingestor` (H2 MPRIS2) | superseded | Desktop source contract in sinexd. status_note added (`6ec6d05`). |
