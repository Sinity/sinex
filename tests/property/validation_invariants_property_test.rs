            ...
            if let Some(size) = size_constraint {
                if let Value::Object(ref mut map) = event.payload {
                    ...
                    map.insert(
                        ...
                        json!("x".repeat(size as usize % 1000)),
                    );
                }
            }
...
fn arb_validation_scenario() -> impl Strategy<Value = ValidationScenario> {
    prop_oneof![
        ...
        // Security scenarios
        Just(ValidationScenario::SecurityThreat(
...
fn contains_security_threat(event: &Event<JsonValue>, threat: &SecurityThreat) -> bool {
    match threat {
        ...
        SecurityThreat::NullByteInjection => {
            event.source.contains('\0') || event.payload.to_string().contains('\0')
        }
        SecurityThreat::XssAttempt => event.payload.to_string().contains("<script>"),
        SecurityThreat::SqlInjection => event.payload.to_string().contains("DROP TABLE"),
    }
}
...
mod unit_tests {
    ...
    fn test_security_threat_detection() -> color_eyre::eyre::Result<()> {
        ...
        assert!(contains_security_threat(
            &threat_event,
            &SecurityThreat::PathTraversal
        ));
    ...
    fn test_validation_scenario_generators() -> color_eyre::eyre::Result<()> {
        ...
        let scenario = arb_validation_scenario()
            .new_tree(&mut runner)
            .unwrap()
            .current();
