//! Cross-crate validation invariants.
//!
//! These properties bridge `sinex-core` sanitisation with
//! `sinex-satellite-sdk` checkpoint metadata to ensure the two layers remain
//! compatible.
...
fn sanitized_events_roundtrip_through_db() -> color_eyre::eyre::Result<()> {
    proptest!(|(payload in arb_event_payload())| {
        run_async(async move {
            ...
            // Sanitiser may mutate payload, so compare the sanitised version.
