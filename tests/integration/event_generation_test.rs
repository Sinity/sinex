...
impl TestFilesystemSatellite {
    ...
    async fn generate_next_event(&mut self) -> Option<sinex_events::RawEvent> {
        if self.events_sent >= self.events_to_generate {
            return None;
        }

        let event = sinex_events::EventFactory::new(&self.source_name).create_event(
            "file.created",
            serde_json::json!({
...
async fn test_event_factory_basic_generation(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let factory = sinex_events::EventFactory::new("test-source");
...
async fn test_event_generation_payload_varieties(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    let pool = ctx.pool().clone();
    let factory = sinex_events::EventFactory::new("payload-test");
...
async fn test_event_generation_performance(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    let pool = ctx.pool().clone();
    let factory = sinex_events::EventFactory::new("perf-test");
...
async fn test_event_generation_payload_sizes(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    let pool = ctx.pool().clone();
    let factory = sinex_events::EventFactory::new("size-test");
...
async fn test_event_generation_error_handling(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let factory = sinex_events::EventFactory::new("");
    let event = factory.create_event(
        "test.event",
    ...
    let long_factory = sinex_events::EventFactory::new(&long_source);
