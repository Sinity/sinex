...
#[sinex_test]
async fn test_distributed_coordination_instance_registration(ctx: TestContext) -> color_eyre::Result<()> {
    let pool = ctx.db_pool();
...
#[sinex_test]
async fn test_distributed_coordination_version_based_leadership(ctx: TestContext) -> color_eyre::Result<()> {
    let pool = ctx.db_pool();
...
#[sinex_test]
async fn test_distributed_coordination_leadership_handoff(ctx: TestContext) -> color_eyre::Result<()> {
    let pool = ctx.db_pool();
...
#[sinex_test]
async fn test_distributed_coordination_leadership_contention(ctx: TestContext) -> color_eyre::Result<()> {
    let pool = ctx.db_pool();
    ...
    let mut coordinators: Vec<_> = instances.into_iter()
    ...
    let leadership_results = futures::future::join_all(
        coordinators.iter_mut().map(|coord| coord.try_acquire_leadership())
    ).await;
    
    // Exactly one should succeed (deterministic based on instance_id or other factors)
    let successful_leaders = leadership_results.iter()
        .filter_map(|result| result.as_ref().ok())
        .filter(|opt| opt.is_some())
        .count();
    
    assert_eq!(successful_leaders, 1, "Exactly one leader should be elected");
...
async fn test_advisory_lock_stress_test(ctx: TestContext) -> color_eyre::Result<()> {
    ...
    println!("Stress test: {}/{} successful acquisitions", successful, total);
...
async fn test_lock_cleanup_on_connection_drop(ctx: TestContext) -> color_eyre::Result<()> {
    ...
    assert!(recovered_lock.is_some(), "Lock should be released when connection drops");
    
    Ok(())
}

#[sinex_test]
async fn test_distributed_coordination_leader_heartbeat_simulation(ctx: TestContext) -> color_eyre::Result<()> {
    let pool = ctx.db_pool();
