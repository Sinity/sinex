...
async fn test_concurrent_pipeline_processing(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    for stream_id in 0..concurrent_streams {
        let stream_events: Vec<_> = stored_events
            .iter()
            .filter(|e| {
                e.payload.get("stream_id") == Some(&json!(stream_id))
            })
            .collect();
...
async fn test_pipeline_data_transformation(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    for raw_event_id in &raw_event_ids {
        let raw_event = ctx.pool.events().get_by_id(*raw_event_id).await?.expect("Raw event should exist");
        ...
        let transformed_event = ctx.create_test_event(
            &raw_event.source,
...
async fn test_pipeline_error_handling(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    for (source, event_type, payload, should_succeed) in test_scenarios {
        match ctx.create_test_event(source, event_type, payload.clone()).await {
            Ok(event) => {
                if should_succeed {
...
async fn test_pipeline_performance_under_load(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let stored_events = ctx.pool
        .events()
        .get_by_source(&EventSource::from("load_test"), Some(events_to_process as i64), None)
        .await?;
    ...
    let avg_batch_duration = batch_durations
        .iter()
        .map(|d| d.as_millis())
        .sum::<u128>() / batch_durations.len() as u128;
