...
async fn test_subscription_pattern_matching(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let patterns = stored_subscriptions["core.events_feed_all"].as_array().unwrap();
...
async fn test_subscription_filter_validation(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let test_filters = vec![
        ("valid_wildcard", json!({"source_filter": "app.*", "event_type_filter": "*"})),
        ("valid_specific", json!({"source_filter": "terminal.kitty", "event_type_filter": "command_executed"})),
        ("valid_multiple", json!({"source_filter": "fs.watcher|fs.scanner", "event_type_filter": "file_created|file_modified"})),
    ];
...
async fn test_subscription_schema_references(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let schema_ref = note_subscription["schema_id_expected_ref"].as_str().unwrap();
