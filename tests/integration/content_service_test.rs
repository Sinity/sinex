...
impl ContentServiceTest {
    ...
    async fn init_test_annex(path: &Utf8Path) -> color_eyre::eyre::Result<()> {
        ...
        if !output.status.success() {
            return Err(color_eyre::eyre::eyre!("Failed to initialize git-annex repository"));
        }
...
async fn test_content_service_without_git_annex() -> color_eyre::eyre::Result<()> {
    ...
    let pool = sinex_test_utils::create_test_db_pool().await?;
    let temp_dir = temp_dir()?;
