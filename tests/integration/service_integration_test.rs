...
async fn test_cross_service_data_flow(ctx: TestContext) -> Result<()> {
    ...
    let events_by_source = ctx.pool.events()
...
async fn test_service_initialization(ctx: TestContext) -> Result<()> {
    ...
    assert!(counts.is_empty(), "Empty database should return empty counts");
...
async fn test_service_error_handling(ctx: TestContext) -> Result<()> {
    ...
    if let Some(event) = found_event {
        assert!(event.payload["special_characters"].is_string(), "Should handle special characters safely");
    }
...
async fn test_service_performance_integration(ctx: TestContext) -> Result<()> {
    ...
    let by_source_events = ctx.pool.events().get_by_source(&EventSource::from("perf-source-1"), Some(20), None).await?;
    ...
    assert!(!by_source_events.is_empty(), "Should find source-specific events");

    // Performance assertions (reasonable thresholds for CI)
    assert!(analytics_duration.as_millis() < 1000, "Analytics should complete quickly");
    assert!(query_duration.as_millis() < 1000, "Queries should complete quickly");
...
async fn test_service_lifecycle(ctx: TestContext) -> Result<()> {
    ...
    // Test service recreation (simulating restart)
    {
        let analytics = AnalyticsService::new(ctx.pool.clone());
        let counts = analytics.get_event_count_by_source(None, None).await?;
        assert_eq!(counts.get("lifecycle-test"), Some(&5), "Data should persist across service restarts");
    }
    ...
    assert_eq!(counts1, counts2, "Concurrent service instances should return consistent results");
...
async fn test_time_based_service_integration(ctx: TestContext) -> Result<()> {
    ...
    let recent_counts = analytics.get_event_count_by_source(Some(one_hour_ago), Some(now)).await?;
    let all_counts = analytics.get_event_count_by_source(None, None).await?;
    
    assert_eq!(recent_counts.get("time-test"), Some(&1), "Should find only recent event");
    assert_eq!(all_counts.get("time-test"), Some(&2), "Should find both events without time filter");
    ...
    assert_eq!(total_in_series, 1, "Should find recent event in time series");
...
async fn test_service_configuration(ctx: TestContext) -> Result<()> {
    ...
    let events = ctx.pool.events().get_by_source(&EventSource::from("config-test"), Some(10), None).await?;
...
async fn test_cross_service_error_handling(ctx: TestContext) -> Result<()> {
    ...
    let problematic_event = ctx.create_test_event(
        "error-propagation",
        "error.event",
        json!({
    ...
    assert!(analytics_result.is_ok(), "Analytics should handle problematic events");
    ...
    let found_event = events.iter().find(|e| e.source.as_str() == "error-propagation");
