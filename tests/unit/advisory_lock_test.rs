...
async fn test_advisory_lock_basic_acquisition(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let lock1 = sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, "test_lock_basic").await?;
    assert!(lock1.is_some());
    
    // Same lock should not be acquirable again
    let lock2 = sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, "test_lock_basic").await?;
    ...
    let lock3 = sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, "test_lock_basic").await?;
...
async fn test_advisory_lock_raii_cleanup(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    // Test RAII cleanup
    {
        let _lock = sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, "test_lock_raii").await?;
        assert!(_lock.is_some());
        
        // Lock should be held here
        let attempt = sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, "test_lock_raii").await?;
    ...
    let lock_after = sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, "test_lock_raii").await?;
...
async fn test_advisory_lock_different_names(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let lock1 = sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, "lock_alpha").await?;
    let lock2 = sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, "lock_beta").await?;
    let lock3 = sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, "lock_gamma").await?;
    ...
    let lock1_conflict = sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, "lock_alpha").await?;
...
async fn test_distributed_coordination_patterns(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    let pool = &ctx.pool;
    let coordination = sinex_db::distributed_locking::DistributedCoordination::new(pool.clone());
...
async fn test_resource_coordination_with_timeout(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let resource_lock = coordination.acquire_resource_lock("shared_resource", timeout).await?;
    ...
    let coordination2 = sinex_db::distributed_locking::DistributedCoordination::new(pool.clone());
    
    // This should timeout since resource is locked
    let start = std::time::Instant::now();
    let result = coordination2.acquire_resource_lock("shared_resource", timeout).await;
...
#[sinex_test]
async fn test_advisory_lock_concurrent_acquisition(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    let pool = &ctx.pool;
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            match sinex_db::distributed_locking::AdvisoryLock::try_acquire(&pool_clone, lock_name).await {
...
async fn test_lock_status_checking(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let _lock = sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, lock_name).await;
...
async fn test_force_release_functionality(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    // Actual functionality testing is limited due to PostgreSQL hash function issues
    let _result = sinex_db::distributed_locking::AdvisoryLock::force_release(pool, "test").await;
