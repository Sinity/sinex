...
async fn test_advisory_lock_basic_acquisition(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let lock1 =
        sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, "test_lock_basic").await?;
...
async fn test_advisory_lock_raii_cleanup(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    {
        let _lock =
            sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, "test_lock_raii")
                .await?;
    ...
    let lock_after =
        sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, "test_lock_raii").await?;
...
async fn test_advisory_lock_different_names(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let lock1 =
        sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, "lock_alpha").await?;
    let lock2 = sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, "lock_beta").await?;
    let lock3 =
        sinex_db::distributed_locking::AdvisoryLock::try_acquire(pool, "lock_gamma").await?;
...
async fn test_advisory_lock_concurrent_acquisition(
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            match sinex_db::distributed_locking::AdvisoryLock::try_acquire(&pool_clone, lock_name)
                .await
            {
