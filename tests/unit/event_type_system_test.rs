...
use sinex_types::events::payloads::{
    ...
    KittyCommandExecutedPayload, AtuinCommandExecutedPayload,
...
async fn test_source_naming_conventions(_ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    for source in test_sources {
        ...
        source_type.validate().map_err(|e| color_eyre::eyre::eyre!(e))?;
...
async fn test_source_event_type_mapping(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    let test_cases = vec![
        ("fs-watcher", vec!["file.created", "file.modified", "file.deleted", "dir.created", "dir.deleted"]),
...
async fn test_event_id_uniqueness_concurrent(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    for i in 0..10 {
        ...
        let handle = task::spawn(async move {
            ...
            for j in 0..3 {
                let payload = FileCreatedPayload::test_default(&format!("/test/file{}_{}.txt", i, j));
    ...
    assert_eq!(all_ids.len(), 30, "Should have 30 unique event IDs (10 tasks * 3 events each)");
...
async fn test_payload_validation_system(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    assert!(valid_event.payload.is_object(), "Payload should be an object");
    assert!(valid_event.payload["path"].is_string(), "Path should be string");
    assert!(valid_event.payload["size"].is_number(), "Size should be number");
