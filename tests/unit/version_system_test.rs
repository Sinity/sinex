...
async fn test_version_string_parsing_patterns(_ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    for pattern in version_patterns {
        ...
        assert!(is_valid_version_pattern(pattern), "Invalid version pattern: {}", pattern);
...
async fn test_version_comparison_logic(_ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    for ((maj1, min1, pat1), (maj2, min2, pat2), expected) in test_cases {
        let v1 = SimpleVersion { major: maj1, minor: min1, patch: pat1 };
        let v2 = SimpleVersion { major: maj2, minor: min2, patch: pat2 };
        
        assert_eq!(
            v1.cmp(&v2),
            expected,
            "Comparing {}.{}.{} with {}.{}.{} should be {:?}",
            maj1, min1, pat1, maj2, min2, pat2, expected
        );
    }
...
async fn test_dirty_build_detection(_ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    for (version_str, expected_dirty) in test_cases {
        ...
        assert_eq!(
            ...
            "Version '{}' dirty detection failed", version_str
        );
    }
...
async fn test_build_timestamp_parsing(_ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    for timestamp in timestamp_examples {
        ...
        if let Ok(dt) = parsed {
            ...
            let age_seconds = now.signed_duration_since(dt.with_timezone(&chrono::Utc))
...
async fn test_instance_identification_patterns(_ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    for (service_name, instance_id) in instance_patterns {
        ...
        assert!(is_valid_service_name(service_name), "Invalid service name: {}", service_name);
        
        // Validate instance ID pattern  
        assert!(is_valid_instance_id(instance_id), "Invalid instance ID: {}", instance_id);
        
        // Ensure uniqueness
        let key = format!("{}/{}", service_name, instance_id);
        assert!(seen_ids.insert(key.clone()), "Duplicate instance key: {}", key);
    }
...
async fn test_leadership_election_logic(_ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let newer_version = SimpleVersion { major: 1, minor: 1, patch: 0 };
    let older_version = SimpleVersion { major: 1, minor: 0, patch: 100 };
    ...
    let same_version = SimpleVersion { major: 1, minor: 0, patch: 100 };
...
async fn test_tiebreaker_scenarios(_ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let earlier = LeadershipCandidate {
        version: SimpleVersion { major: 1, minor: 0, patch: 100 },
...
#[sinex_test]
async fn test_version_system_integration_concepts(_ctx: TestContext) -> color_eyre::eyre::Result<()> {
...
fn is_preferred_build(candidate1: &VersionCandidate, candidate2: &VersionCandidate) -> bool {
    ...
    if candidate1.version_str.starts_with(&candidate2.version_str.replace(".dirty", "")) ||
       candidate2.version_str.starts_with(&candidate1.version_str.replace(".dirty", "")) {
