...
|-------|---------|-------------|------|
| **sinex-primitives** | Foundation types (UUID IDs, `Timestamp`), validation, error handling | `prelude::*`, `SinexError`, `Event<T>`, `EventBuilder`, `Id<T>`, `EventSource`, `EventType`, `Uuid`, `Timestamp` | `crate/lib/sinex-primitives/docs/` |
