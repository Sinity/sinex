...
impl DirectoryManager {
    ...
    pub async fn directory_exists<P: AsRef<Path>>(&self, path: P) -> Result<bool> {
        let path = path.as_ref();
        let full_path = self.config.base_path.join(path);
    ...
    pub async fn list_directory<P: AsRef<Path>>(&self, path: P) -> Result<Vec<PathBuf>> {
        let path = path.as_ref();
        let full_path = self.config.base_path.join(path);
    ...
    pub async fn ensure_directory<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        let path = path.as_ref();
