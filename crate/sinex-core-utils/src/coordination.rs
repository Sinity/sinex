...
impl CoordinationPrimitive {
    ...
    pub async fn wait_for_threshold(&self, timeout_duration: Duration) -> Result<usize> {
        ...
        loop {
            ...
            tokio::time::timeout(remaining, self.notify.notified()).await
                .map_err(|_| CoreError::Timeout(format!(
        ...
        if self.reset_behavior == ResetBehavior::Automatic {
            ...
            if count == self.threshold {
                ...
                loop {
                    ...
                    tokio::time::timeout(timeout_duration / 10, self.notify.notified()).await
                        .map_err(|_| CoreError::Timeout(format!(
    ...
    pub fn reset(&self) {
        ...
        tracing::debug!("CoordinationPrimitive '{}' reset from {} to 0", self.name, old_state);
    }
    
    /// Reset to specific value
    pub fn reset_to(&self, value: usize) {
        let old_state = self.state.swap(value, Ordering::AcqRel);
        tracing::debug!("CoordinationPrimitive '{}' reset from {} to {}", self.name, old_state, value);
    }
        ...
        if new_state >= self.threshold {
            match self.reset_behavior {
                ResetBehavior::Automatic => {
                    ...
                    tracing::debug!("Barrier '{}' reached threshold {} - auto-reset and generation increment", 
                                  self.name, self.threshold);
                }
                _ => {
                    tracing::debug!("CoordinationPrimitive '{}' reached threshold: {}", 
                                  self.name, new_state);
                }
            }
