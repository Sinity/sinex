...
use sinex_events::{
    Event, SystemdTimerTriggeredPayload, SystemdUnitFailedPayload, SystemdUnitReloadedPayload,
...
impl SystemdWatcher {
    ...
    fn parse_unit_status(&self, line: &str) -> Option<Event> {
        ...
        if line.trim().starts_with("Active: ") {
            ...
            return match status {
                "active" => Some(Event::from(SystemdUnitStartedPayload {
                ...
                "inactive" => Some(Event::from(SystemdUnitStoppedPayload {
                ...
                "failed" => Some(Event::from(SystemdUnitFailedPayload {
                    unit_name: "unknown".to_string(),
                ...
                "activating" => Some(Event::from(SystemdUnitStartingPayload {
                    status: status.to_string(),
                    status_detail: status_part.to_string(),
                    timestamp: chrono::Utc::now().to_rfc3339(),
                })),
                "deactivating" => Some(Event::from(SystemdUnitStoppingPayload {
                    status: status.to_string(),
                    status_detail: status_part.to_string(),
                    timestamp: chrono::Utc::now().to_rfc3339(),
                })),
                _ => Some(Event::from(SystemdUnitStateChangedPayload {
                    status: status.to_string(),
                    status_detail: status_part.to_string(),
                    timestamp: chrono::Utc::now().to_rfc3339(),
                })),
    ...
    async fn get_unit_status(
        &self,
        tx: &mpsc::UnboundedSender<sinex_events::Event>,
    ) -> SatelliteResult<()> {
    ...
    fn parse_systemd_journal_entry(&self, line: &str) -> Option<sinex_events::Event> {
        match serde_json::from_str::<serde_json::Value>(line) {
            Ok(entry) => {
                ...
                if message.contains("Started ") {
                    ...
                    Some(Event::from(SystemdUnitStartedPayload {
                    ...
                    Some(Event::from(SystemdUnitStoppedPayload {
                        unit_name: unit_name.unwrap_or("unknown").to_string(),
                        unit_type: unit_type.to_string(),
                        exit_code: None,
                    ...
                    Some(Event::from(SystemdUnitFailedPayload {
                        unit_name: unit_name.unwrap_or("unknown").to_string(),
                    ...
                    Some(Event::from(SystemdUnitReloadedPayload {
                    ...
                    Some(Event::from(SystemdTimerTriggeredPayload {
                        unit_name: unit_name.map(String::from),
                        message: message.to_string(),
                        cursor: cursor.to_string(),
                        pid: entry["_PID"].as_str().map(String::from),
