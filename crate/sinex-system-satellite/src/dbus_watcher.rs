...
impl DbusWatcher {
    ...
    async fn process_signal(
        ...
        {
            let event = Event::from(DbusPowerStateChangedPayload {
                ...
                || interface == "org.freedesktop.UPower.Device")
        {
            ...
            let event = Event::from(DbusDeviceConnectedPayload {
        ...
        if config.extract_bluetooth && interface.starts_with("org.bluez") {
            let event = Event::from(DbusBluetoothDeviceChangedPayload {
                event_type: member.to_string(),
        ...
        if config.extract_network && interface.starts_with("org.freedesktop.NetworkManager") {
            let event = Event::from(DbusNetworkStateChangedPayload {
                event_type: member.to_string(),
        ...
        if config.extract_mounts && interface == "org.freedesktop.UDisks2.Filesystem" {
            ...
            let event = Event::from(DbusMountEventPayload {
        ...
        let event = Event::from(DbusSignalPayload {
    ...
    async fn process_method_call(
        ...
        let event = Event::from(DbusMethodCalledPayload {
            bus: bus_type.to_string(),
            sender: sender.as_deref().unwrap_or_default().to_string(),
