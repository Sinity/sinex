...
impl HookManager {
    ...
    pub async fn install_all_hooks(&mut self, config: &IntegrationConfig) -> sinex_core::Result<()> {
        if !self.shell_info.capabilities.supports_hooks {
            return Err(sinex_core::CoreError::Configuration(
                format!("Shell {} does not support hooks", self.shell_info.shell_type.name())
            ));
        }
        ...
        info!("Installed {} hooks for {}", self.installed_hooks.len(), self.shell_info.shell_type.name());
        Ok(())
    }
    ...
    pub async fn install_hook(&mut self, hook_type: HookType, config: &IntegrationConfig) -> sinex_core::Result<()> {
        let config_file = self.shell_info.config_path.as_ref()
            .ok_or_else(|| sinex_core::CoreError::Configuration("No config file found for shell".to_string()))?;
        ...
        fs::write(config_file, updated_content).await
            .map_err(|e| sinex_core::CoreError::Io(format!("Failed to write config file: {}", e)))?;
    ...
    pub async fn uninstall_hook(&mut self, hook_type: &HookType) -> sinex_core::Result<()> {
        let installation = self.installed_hooks.remove(hook_type)
            .ok_or_else(|| sinex_core::CoreError::Configuration("Hook not installed".to_string()))?;
        
        // Read current config
        let current_content = fs::read_to_string(&installation.config_file).await
            .map_err(|e| sinex_core::CoreError::Io(format!("Failed to read config file: {}", e)))?;
        ...
        // Write updated config
        fs::write(&installation.config_file, updated_content).await
            .map_err(|e| sinex_core::CoreError::Io(format!("Failed to write config file: {}", e)))?;
    ...
    pub async fn verify_hooks(&self) -> sinex_core::Result<bool> {
        let config_file = self.shell_info.config_path.as_ref()
            .ok_or_else(|| sinex_core::CoreError::Configuration("No config file found".to_string()))?;
    ...
    fn generate_hook_content(&self, hook_type: &HookType, config: &IntegrationConfig) -> sinex_core::Result<String> {
        ...
        match (&self.shell_info.shell_type, hook_type) {
            (ShellType::Bash, HookType::PreCommand) => {
                Ok(format!(r#"
...
trap '_sinex_preexec' DEBUG"#, sinex_notify_cmd))
            }
            
            (ShellType::Bash, HookType::PostCommand) => {
                Ok(format!(r#"
...
PROMPT_COMMAND="_sinex_postexec;$PROMPT_COMMAND""#, sinex_notify_cmd))
            }
            
            (ShellType::Zsh, HookType::PreCommand) => {
                Ok(format!(r#"
# Sinex pre-command hook
preexec() {{
    {} --event precommand --command "$1" --pwd "$PWD" &
            ...
            (ShellType::Zsh, HookType::PostCommand) => {
                Ok(format!(r#"
# Sinex post-command hook
precmd() {{
    local exit_code=$?
    {} --event postcommand --exit-code $exit_code --pwd "$PWD" &
            ...
            (ShellType::Fish, HookType::PreCommand) => {
                Ok(format!(r#"
# Sinex pre-command hook
function _sinex_preexec --on-event fish_preexec
    {} --event precommand --command "$argv[1]" --pwd "$PWD" &
end"#, sinex_notify_cmd))
            }
            
            (ShellType::Fish, HookType::PostCommand) => {
                Ok(format!(r#"
# Sinex post-command hook
function _sinex_postexec --on-event fish_postexec
    {} --event postcommand --exit-code $status --pwd "$PWD" &
            ...
            _ => Err(sinex_core::CoreError::Configuration(
                format!("Hook {:?} not supported for shell {}", hook_type, self.shell_info.shell_type.name())
            )),
        }
    }
    ...
    async fn create_backup(&self, config_file: &PathBuf) -> sinex_core::Result<PathBuf> {
        let backup_file = config_file.with_extension(
            format!("{}.sinex-backup.{}", 
                config_file.extension().and_then(|s| s.to_str()).unwrap_or(""),
