...
impl EnvironmentSetup {
    ...
    pub async fn configure_environment(&mut self, config: &IntegrationConfig) -> sinex_core::Result<()> {
    ...
    fn create_environment_config(&self, config: &IntegrationConfig) -> sinex_core::Result<EnvironmentConfig> {
        ...
        env_config.environment_vars.insert(
            ...
            config.session_id.clone().unwrap_or_else(|| "unknown".to_string()),
        );
        ...
        env_config.environment_vars.insert(
            "SINEX_SHELL_INTEGRATION".to_string(),
            "enabled".to_string(),
        );
        ...
        for (key, value) in &config.environment_vars {
            env_config.environment_vars.insert(key.clone(), value.clone());
        }
    ...
    async fn set_environment_variables(&self, config: &EnvironmentConfig) -> sinex_core::Result<()> {
        for (key, value) in &config.environment_vars {
    ...
    async fn restore_environment_variables(&self, config: &EnvironmentConfig) -> sinex_core::Result<()> {
    ...
    async fn modify_shell_profile(&self, config: &EnvironmentConfig) -> sinex_core::Result<()> {
        ...
        let existing_content = if profile_file.exists() {
            fs::read_to_string(&profile_file).await
                .map_err(|e| sinex_core::CoreError::Io(format!("Failed to read profile file: {}", e)))?
        } else {
            String::new()
        };
        ...
        fs::write(&profile_file, updated_content).await
            .map_err(|e| sinex_core::CoreError::Io(format!("Failed to write profile file: {}", e)))?;
    ...
    async fn cleanup_shell_profile(&self, _config: &EnvironmentConfig) -> sinex_core::Result<()> {
        ...
        let current_content = fs::read_to_string(&profile_file).await
            .map_err(|e| sinex_core::CoreError::Io(format!("Failed to read profile file: {}", e)))?;
        ...
        fs::write(&profile_file, cleaned_content).await
            .map_err(|e| sinex_core::CoreError::Io(format!("Failed to write profile file: {}", e)))?;
        ...
        match &self.shell_info.shell_type {
            ShellType::Bash => {
                ...
                let home = dirs::home_dir()
                    .ok_or_else(|| sinex_core::CoreError::Configuration("Cannot find home directory".to_string()))?;
            ...
            _ => Err(sinex_core::CoreError::Configuration(
                format!("Profile modification not supported for {}", self.shell_info.shell_type.name())
            )),
        }
    }
    
    fn generate_profile_additions(&self, config: &IntegrationConfig) -> sinex_core::Result<Vec<String>> {
