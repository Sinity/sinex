...
use sinex_types::events::{
    AutomatonErrorPayload, ClipboardCopiedPayload, EventPayload, FileCreatedPayload,
    FileDeletedPayload, FileModifiedPayload, HyprlandWindowClosedPayload,
    HyprlandWindowFocusedPayload, HyprlandWindowOpenedPayload, KittyCommandExecutedPayload,
    ProcessHeartbeatPayload, ProcessStartedPayload, SystemdUnitStartedPayload,
...
impl TestContext {
    ...
    // TODO: Remove after Redis migration
    ...
    // Removed obsolete assert_snapshot - use snapshot(), snapshot_event(), etc. below
    ...
    /// Create a snapshot of an event with automatic redactions and custom settings
    pub fn snapshot_event(&self, event: &Event, name: Option<&str>) {
        let snapshot_name = name.unwrap_or_else(|| "event");
        let mut settings = insta::Settings::new();

        // Use test-specific snapshot directory
        settings.set_snapshot_path(format!("test/snapshots/{}", self.test_name));
    ...
    /// Create a JSON snapshot with custom redactions
    pub fn snapshot_json<T: serde::Serialize>(
        &self,
        value: &T,
        name: &str,
        redactions: Vec<(&str, &str)>,
    ) {
        let mut settings = insta::Settings::new();
        settings.set_snapshot_path(format!("test/snapshots/{}", self.test_name));
        ...
        for (path, replacement) in redactions {
            settings.add_redaction(path, replacement);
        }
    ...
    /// Create a YAML snapshot of any serializable value
    ...
    /// Create a debug snapshot (for types that don't implement Serialize)
    pub fn snapshot_debug<T: std::fmt::Debug>(&self, value: &T, name: Option<&str>) {
        let snapshot_name = name.unwrap_or("debug_snapshot");
        let mut settings = insta::Settings::new();
        settings.set_snapshot_path(format!("test/snapshots/{}", self.test_name));
        settings.set_snapshot_suffix(snapshot_name);
        settings.set_prepend_module_to_snapshot(false);
    ...
    /// Create an inline snapshot (useful for small values)
    pub fn assert_inline_snapshot<T: serde::Serialize>(&self, value: &T) {
        insta::assert_yaml_snapshot!(value);
    }

    /// Assert two values are equal with enhanced diff output
        ...
        T: std::fmt::Debug + PartialEq,
    {
        if left != right {
            ...
            similar_asserts::assert_eq!(left, right, "{}", msg);
        }
        Ok(())
    }

    /// Assert JSON values are equal with better diffs
    pub fn assert_json_similar(
        &self,
        ...
        if left != right {
            ...
            similar_asserts::assert_eq!(left_pretty, right_pretty, "{}", msg);
        }
        Ok(())
    }

    // TODO: Fix tracing_test integration - API has changed
    // /// Enable tracing for this test with custom filter
    // pub fn with_tracing(&self, filter: &str) -> tracing_test::TracingTestGuard {
    //     tracing_test::internal::set_test_with_filter(filter);
    //     tracing_test::internal::get_guard()
    // }

    // /// Static method to enable tracing without TestContext
    // pub fn init_tracing(filter: &str) -> tracing_test::TracingTestGuard {
    //     tracing_test::internal::set_test_with_filter(filter);
    //     tracing_test::internal::get_guard()
    // }

    /// Get captured logs from tracing-test
    ...
    /// Assert that no error logs were emitted
    pub fn assert_no_errors_logged(&self) -> Result<()> {
        let logs = self.captured_logs();
        let errors: Vec<_> = logs.iter().filter(|log| log.contains("ERROR")).collect();
        if !errors.is_empty() {
            return Err(SinexError::validation(format!(
...
mod benches {
    ...
    // #[sinex_bench]
    // fn bench_context_creation() -> color_eyre::eyre::Result<()> {
    //     black_box(TestContext::new().await?);
    //     Ok(())
    // }

    // #[sinex_bench]
    // fn bench_context_with_name() -> color_eyre::eyre::Result<()> {
    //     black_box(TestContext::with_name("bench_test").await?);
    //     Ok(())
    // }

    // #[sinex_bench]
    // fn bench_single_event_creation() -> color_eyre::eyre::Result<()> {
    ...
    // #[sinex_bench]
    // fn bench_batch_event_creation_small() -> color_eyre::eyre::Result<()> {
    //     let ctx = TestContext::new().await?;
    ...
    #[sinex_bench]
    fn bench_batch_event_creation_medium() -> color_eyre::eyre::Result<()> {
        let ctx = TestContext::new().await?;
    ...
    // #[sinex_bench]
    // async fn bench_query_count_all() -> color_eyre::eyre::Result<()> {
    ...
    // #[sinex_bench]
    // async fn bench_query_fetch_limited() -> color_eyre::eyre::Result<()> {
    //     ctx.query_bench(crate::static_fixtures::DatasetSize::Small)
    //         .await?;
    //
    //     use sinex_db::repositories::*;
    ...
    // #[sinex_bench]
    // async fn bench_query_filtered() -> color_eyre::eyre::Result<()> {
    //     ctx.query_bench(crate::static_fixtures::DatasetSize::Small)
    //         .await?;
    //
    //     use sinex_db::repositories::*;
    ...
    // #[sinex_bench]
    // fn bench_concurrent_operations(ctx: &mut BenchContext) -> color_eyre::eyre::Result<()> {
    //     let ctx = TestContext::new().await?;
    //     let results = ctx
    ...
    // #[sinex_bench]
    // fn bench_simple_assertions(ctx: &mut BenchContext) -> color_eyre::eyre::Result<()> {
    //     let ctx = TestContext::new().await?;
