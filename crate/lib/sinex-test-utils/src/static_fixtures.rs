    ...
    pub fn new() -> Self {
        Self {
            ...
            config: FixtureConfig {
                base_dir: Utf8PathBuf::from("target/bench-fixtures"),
...
impl FixtureManager {
    ...
    async fn generate_fixture(
        ...
        let manifest = FixtureManifest {
            ...
            files: FixtureFiles {
                sql: Utf8PathBuf::from(format!("{}.sql", size.name())),
                json: Utf8PathBuf::from(format!("{}.json", size.name())),
                manifest: Utf8PathBuf::from("manifest.json"),
            },
        };
...
pub async fn ensure_fixture(
    ...
    // Load into database
    crate::fixture_generator::load_dataset(pool, sql_path.as_path()).await?;
