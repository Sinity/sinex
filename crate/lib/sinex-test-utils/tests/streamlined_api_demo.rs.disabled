...
async fn demo_streamlined_querying(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    ctx.create_test_event("fs-watcher", "file.created", json!({"path": "/demo1.txt"}))
        .await?;
    ctx.create_test_event("fs-watcher", "file.created", json!({"path": "/demo2.txt"}))
        .await?;
...
async fn demo_streamlined_fixtures(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    let perf_data = ctx.performance().large_dataset_with(1000).await?;
    ...
    println!(
        "Error scenarios: {} failed operations",
        errors.failed_operation_ids.len()
    );
...
async fn demo_assertions_and_timing(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    // Create events
    ctx.create_test_event("fs-watcher", "file.created", json!({"path": "/test1.txt"}))
        .await?;
    ctx.create_test_event("fs-watcher", "file.created", json!({"path": "/test2.txt"}))
        .await?;
