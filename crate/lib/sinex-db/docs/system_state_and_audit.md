...
The processor manifest serves as a registry for all nodes (ingestors and automata) that have participated in the cluster.

- **Identification**: Each entry records the `processor_name`, its `version`, and its `node_type`.
- **Manifest vs. Status**: While the manifest records that a processor *exists*, its current runtime health is tracked separately via heartbeats.

## Health Monitoring

The system determines processor health by analyzing `process.heartbeat` events emitted by nodes.

- **Active Count**: Processors that have emitted a heartbeat within the staleness threshold (default 120s).
- **Inactive Count**: Registered processors that have either never sent a heartbeat or have exceeded the staleness threshold.
