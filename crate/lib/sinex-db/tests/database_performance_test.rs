...
use serde_json::json;
use sinex_db::types::events::{event_types, services, EventFactory};
use xtask::sandbox::prelude::*;
...
impl DatabaseMetrics {
    ...
    fn print_summary(&self) {
        ...
        for query_name in self.query_times.keys() {
            ...
            println!(
                "  - Average latency: {:?}",
                self.average_query_time(query_name)
            );
            println!(
                "  - P50 latency: {:?}",
                self.percentile_query_time(query_name, 50.0)
            );
            println!(
                "  - P95 latency: {:?}",
                self.percentile_query_time(query_name, 95.0)
            );
            println!(
                "  - P99 latency: {:?}",
                self.percentile_query_time(query_name, 99.0)
            );
            println!(
                "  - Success rate: {:.2}%",
                self.query_success_rate(query_name)
            );
            ...
            if let Some(times) = self.query_times.get(query_name) {
                ...
                println!(
                    "  - Min latency: {:?}",
                    times.iter().min().unwrap_or(&StdDuration::from_millis(0))
                );
                println!(
                    "  - Max latency: {:?}",
                    times.iter().max().unwrap_or(&StdDuration::from_millis(0))
                );
            }
        }

        if !self.connection_times.is_empty() {
            let avg_conn_time = self.connection_times.iter().sum::<StdDuration>()
        ...
        if !self.transaction_times.is_empty() {
            let avg_tx_time = self.transaction_times.iter().sum::<StdDuration>()
                / self.transaction_times.len() as u32;
...
async fn test_query_performance_patterns(ctx: TestContext) -> TestResult<()> {
    ...
    println!(
        "🔄 Populating database with {} test events for query performance testing",
        event_count
    );

    for event in &test_events {
        sinex_db::db::insert_event_with_validator(pool, event, None).await?;
    }
    ...
    for (test_name, query) in query_tests {
        ...
        for iteration in 0..50 {
            ...
            let result = match test_name {
                ...
                "JSON Payload Contains" => {
                    sqlx::query(query)
                        .bind(json!({"test": "value"}))
                        .fetch_all(&pool)
                        .await
                }
                "JSON Payload Key Exists" => sqlx::query(query).bind("test").fetch_all(&pool).await,
                ...
                "Count Aggregation" => sqlx::query(query).fetch_all(&pool).await,
                "Time Series Aggregation" => sqlx::query(query).fetch_all(&pool).await,
                "Recent Events" => sqlx::query(query).fetch_all(&pool).await,
                "Full Text Search" => sqlx::query(query).bind("%test%").fetch_all(&pool).await,
                "Multi-Join Query" => {
                    let test_type = &test_events[iteration % test_events.len()].event_type;
                    sqlx::query(query).bind(test_type).fetch_all(&pool).await
                }
        ...
        println!(
            "  ⏱️  Average latency: {:?}",
            metrics.average_query_time(test_name)
        );
        println!(
            "  📈 P95 latency: {:?}",
            metrics.percentile_query_time(test_name, 95.0)
        );
    }
    ...
    assert!(
        metrics.average_query_time("Primary Key Lookup") < StdDuration::from_millis(5),
        "Primary key lookups should be < 5ms"
    );
    assert!(
        metrics.average_query_time("Source Index Scan") < StdDuration::from_millis(50),
        "Index scans should be < 50ms"
    );
    assert!(
        metrics.average_query_time("Time Range Query") < StdDuration::from_millis(100),
    ...
    for query_name in metrics.query_times.keys() {
        assert!(
            metrics.query_success_rate(query_name) > 95.0,
            "Query '{}' should have >95% success rate",
            query_name
        );
    }
...
async fn test_concurrent_database_performance(ctx: TestContext) -> TestResult<()> {
    ...
    // Performance assertions
    assert!(
        final_metrics.average_query_time("Concurrent Insert") < StdDuration::from_millis(100),
        "Concurrent inserts should be < 100ms"
    );
    assert!(
        final_metrics.average_query_time("Concurrent Query") < StdDuration::from_millis(50),
        "Concurrent queries should be < 50ms"
    );
    assert!(
        final_metrics.query_success_rate("Concurrent Insert") > 95.0,
        "Concurrent insert success rate should be > 95%"
    );
    assert!(
        final_metrics.query_success_rate("Concurrent Query") > 95.0,
...
async fn test_connection_pool_performance(ctx: TestContext) -> TestResult<()> {
    ...
    let connection_handles = (0..concurrent_connections).map(|conn_id| {
        let pool_clone = pool.clone();
            ...
            match pool_clone.acquire().await {
                Ok(mut conn) => {
                    ...
                    for op in 0..5 {
                        ...
                        let mut metrics_lock = metrics.lock().await;
                        metrics_lock.record_query(
                            "Concurrent Connection Query",
                            query_duration,
                            result.is_ok(),
                        );
                    }
                    ...
                    (
                        true,
                        metrics_lock.connection_times.clone(),
                        metrics_lock.query_times.clone(),
                    )
                }
                Err(e) => {
    ...
    println!(
        "  ✅ Successful concurrent connections: {}/{}",
    ...
    if !total_connection_times.is_empty() {
        ...
        println!(
            "  ⏱️  Average concurrent connection time: {:?}",
            avg_concurrent_conn_time
        );
    }
    ...
    assert!(
        avg_connection_time < StdDuration::from_millis(50),
        "Average connection acquisition should be < 50ms"
    );
    assert!(
        successful_connections as f64 / concurrent_connections as f64 > 0.95,
        "Concurrent connection success rate should be > 95%"
    );
    assert!(
        metrics.query_success_rate("Connection Test Query") > 95.0,
...
async fn test_transaction_performance(ctx: TestContext) -> TestResult<()> {
    ...
    for i in 0..transaction_count {
        ...
        let insert1 = sinex_db::db::insert_event_with_validator(&mut tx, &event1, None).await;
        let insert2 = sinex_db::db::insert_event_with_validator(&mut tx, &event2, None).await;
    ...
    let tx_handles = (0..concurrent_transactions).map(|tx_id| {
        let pool_clone = pool.clone();
        ...
        tokio::spawn(async move {
            ...
            for op in 0..3 {
                ...
                sinex_db::db::insert_event_with_validator(&mut tx, &event, None).await?;
            }
    ...
    for result in tx_results {
        match result {
            Ok(Ok((tx_id, duration))) => {
                ...
                println!(
                    "  ✅ Concurrent transaction {} completed in {:?}",
                    tx_id, duration
                );
            }
            Ok(Err(e)) => {
    ...
    println!(
        "  📊 Successful concurrent transactions: {}/{}",
    ...
    println!(
        "🔍 Transaction consistency check: {} events inserted",
        tx_event_count.count.unwrap_or(0)
    );

    // Performance assertions
    let avg_tx_time = if !metrics.transaction_times.is_empty() {
        metrics.transaction_times.iter().sum::<StdDuration>()
            / metrics.transaction_times.len() as u32
    } else {
        StdDuration::from_millis(0)
    };

    assert!(
        avg_tx_time < StdDuration::from_millis(200),
        "Average transaction time should be < 200ms"
    );
    assert!(
        successful_transactions as f64 / concurrent_transactions as f64 > 0.9,
        "Concurrent transaction success rate should be > 90%"
    );
    assert!(
        metrics.query_success_rate("Transaction") > 95.0,
