...
impl EventRepository<'_> {
    ...
    pub async fn count_by_time_range(&self, start: Timestamp, end: Timestamp) -> DbResult<i64> {
    ...
    pub async fn get_source_activity(
        ...
        let rows = sqlx::query!(
            ...
            SELECT
                ...
                MIN(ts_orig) as "first_event: Timestamp",
                MAX(ts_orig) as "last_event: Timestamp"
            FROM core.events
            WHERE ts_orig >= $1
    ...
    pub async fn get_events_over_time(
        ...
        let rows = sqlx::query_as!(
            ...
            r#"
            SELECT
                time_bucket($1::interval, COALESCE(ts_orig, ts_ingest)) as "bucket!: Timestamp",
                COUNT(*) as "count!"
            FROM core.events
