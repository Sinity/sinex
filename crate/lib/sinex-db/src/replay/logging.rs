...
impl ReplayLogger {
    ...
    pub fn start_replay(
        operation_id: &Id<crate::repositories::state::Operation>,
    ...
    pub fn invariant_violated(violation: &crate::replay::invariants::InvariantViolation) {
        let severity = violation.violation_type.severity();
        match severity {
            crate::replay::invariants::ViolationSeverity::Low => {
                debug!(
                    violation_type = ?violation.violation_type,
                    "Low severity invariant violation"
                );
            }
            crate::replay::invariants::ViolationSeverity::Medium => {
                info!(
                    violation_type = ?violation.violation_type,
                    "Medium severity invariant violation"
                );
            }
            crate::replay::invariants::ViolationSeverity::High => {
                warn!(
                    violation_type = ?violation.violation_type,
            ...
            crate::replay::invariants::ViolationSeverity::Critical => {
                error!(
                    violation_type = ?violation.violation_type,
                    context = %violation.context,
