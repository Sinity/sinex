...
impl EventSanitizer {
    ...
    pub fn sanitize_event_generic<T>(event: &mut Event<T>) -> SinexResult<bool>
    where
        T: serde::Serialize + serde::de::DeserializeOwned,
    {
        ...
        let mut payload_json = serde_json::to_value(&event.payload).map_err(|e| {
            sinex_primitives::error::SinexError::serialization(format!(
        ...
        if Self::sanitize_json_payload(&mut payload_json)? {
            ...
            event.payload = serde_json::from_value(payload_json).map_err(|e| {
                sinex_primitives::error::SinexError::serialization(format!(
                    "Failed to deserialize sanitized payload: {}",
                    e
                ))
            })?;
            was_modified = true;
    ...
    pub fn sanitize_event(event: &mut Event<JsonValue>) -> SinexResult<bool> {
        let mut was_modified = false;
    ...
    fn sanitize_json_payload(payload: &mut Value) -> SinexResult<bool> {
        let mut was_modified = false;
