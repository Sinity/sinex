use sinex_core::db::query_helpers::{db_error, with_retry_transaction, with_transaction, RetryConfig};
...
async fn with_transaction_accepts_async_closures(ctx: TestContext) -> TestResult<()> {
    with_transaction(ctx.pool(), |tx| async move {
        sqlx::query("SELECT 1")
            .execute(&mut **tx)
            .await
...
async fn with_retry_transaction_accepts_async_closures(ctx: TestContext) -> TestResult<()> {
    ...
    with_retry_transaction(ctx.pool(), config, |tx| async move {
        sqlx::query("SELECT 1")
            .execute(&mut **tx)
            .await
