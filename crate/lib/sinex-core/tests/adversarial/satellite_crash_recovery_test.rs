//! Adversarial tests for satellite crash recovery during material acquisition.
//!
//! These scenarios exercise Stage-as-You-Go’s JetStream material pipeline when
//! satellites crash mid-acquisition. Since satellites no longer write directly
...
/// Detect orphaned materials left sensing after multiple satellites crash.
#[sinex_test]
        ...
        SET status = $1,
            metadata = metadata || jsonb_build_object(
                'recovery_info', jsonb_build_object(
                    'recovered_at', NOW(),
                    'recovery_reason', 'satellite_crash',
