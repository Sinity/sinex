...
use serde_json::json;
use sinex_core::db::repositories::{checkpoints::CheckpointInput, state::Operation, DbPoolExt};
...
async fn test_processor_lifecycle_management(ctx: TestContext) -> Result<()> {
    ...
    assert!(processor_operations
        .iter()
        .any(|op| op.operation_type == "service_started"));
    assert!(processor_operations
        .iter()
        .any(|op| op.operation_type == "service_stopped"));
...
async fn test_system_health_monitoring(ctx: TestContext) -> Result<()> {
    ...
    assert!(
        health_report.ulid_extension_works,
        "ULID extension should work"
    );
    assert!(
        health_report.json_schema_extension_works,
        "JSON Schema extension should work"
    );
    assert!(
        health_report.events_table_exists,
        "Events table should exist"
    );
    assert!(
        health_report.checkpoints_table_exists,
    ...
    if let Some(version) = &health_report.timescaledb_version {
        assert!(
            !version.is_empty(),
            "TimescaleDB version should not be empty if present"
        );
    }
