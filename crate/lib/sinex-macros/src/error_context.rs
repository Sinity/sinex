...
pub fn with_context(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    // Generate context building code with enhanced error handling
    let mut context_building = quote! {
        core_err.wrap_err()
            .with_operation(#operation)
            .wrap_err_with("function", #fn_name_str)
            .wrap_err_with("module", module_path!())
            .wrap_err_with("crate", env!("CARGO_PKG_NAME"))
            .wrap_err_with("version", env!("CARGO_PKG_VERSION"))
    };
    ...
    // Note: context_building is now unused since we inline the context building logic
