...
- `ProcessorInitContext::into_runtime()` yields a `ProcessorRuntimeState` with ergonomic accessors for acquisition, job, lifecycle, coordination, heartbeat, and replay helpers. nodes should stop threading raw pools or emitters and instead consume the runtime snapshot directly.
