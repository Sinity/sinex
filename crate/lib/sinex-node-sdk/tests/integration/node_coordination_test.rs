...
async fn test_single_instance_becomes_leader() -> TestResult<()> {
    ...
    let result = timeout(COORDINATION_TIMEOUT, coordination.run_coordination_loop(move || {
...
async fn test_multi_instance_leader_election() -> TestResult<()> {
    ...
    let handle1 = tokio::spawn(async move {
        timeout(COORDINATION_TIMEOUT, coord1.run_coordination_loop(move || {
    ...
    let handle2 = tokio::spawn(async move {
        timeout(COORDINATION_TIMEOUT, coord2.run_coordination_loop(move || {
    ...
    let handle3 = tokio::spawn(async move {
        timeout(COORDINATION_TIMEOUT, coord3.run_coordination_loop(move || {
