...
use sinex_node_sdk::{EventSourceConfig, Node};
...
#[sinex_test]
async fn test_event_factory_basic_generation(ctx: TestContext) -> TestResult<()> {
...
#[sinex_test]
async fn test_event_generation_error_handling(ctx: TestContext) -> TestResult<()> {
    let pool = ctx.pool().clone();
