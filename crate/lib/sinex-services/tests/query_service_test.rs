...
async fn test_query_by_source_filter(ctx: TestContext) -> TestResult<()> {
    ...
    let expected_fs = dataset.expected_sources.get("fs").copied().unwrap_or(0);
    assert!(
        results.len() >= expected_fs.min(2).max(1),
        "Expected at least 2 fs events, saw {}",
        results.len()
    );
...
async fn test_query_by_time_range(ctx: TestContext) -> TestResult<()> {
    ...
    let results = service.search_events(query).await?;
    assert!(!results.is_empty(), "Expected recent results");
    assert!(
        !results
            .iter()
            .any(|r| r.event_type == "file.deleted" && r.snippet.contains("old_file.txt")),
        "Old file deletion should be outside the time window"
    );
    assert!(results
        .iter()
...
async fn test_query_combined_filters(ctx: TestContext) -> TestResult<()> {
    ...
    let query = SearchQuery {
        ...
        sources: vec!["fs".to_string()],
        event_types: vec![],
        start_time: Some(clock.base() - Duration::hours(1)),
        end_time: None,
        limit: 10,
        offset: 0,
    };
...
async fn test_query_pagination(ctx: TestContext) -> TestResult<()> {
    ...
    let pagination_specs: Vec<EventSpec> = (0..10)
        .map(|i| {
            ...
            .after(Duration::minutes(i as i64))
        })
        .collect();
    seed_events_via_scope(&scope, &clock, &pagination_specs).await?;
...
async fn test_query_pagination_stable_during_concurrent_ingestion(
    ...
    let start_time = clock.base() - Duration::minutes(1);
    let end_time = clock.base() + Duration::minutes(10);
    ...
    let ingest_task = {
        ...
        async move {
            for seq in 0..total_events {
                let ts = clock.base() + Duration::seconds(seq as i64);
                let overrides = EventOverrides {
                    ts_orig: Some(ts.to_rfc3339()),
                    ..Default::default()
                };
                scope
