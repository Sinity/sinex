use sinex_satellite_sdk::emit_heartbeat;
use sinex_satellite_sdk::heartbeat::HeartbeatEmitter;
use sinex_test_utils::sinex_test;

#[sinex_test]
async fn heartbeat_emitter_tracks_metadata() -> color_eyre::eyre::Result<()> {
...
#[sinex_test]
async fn counter_handle_updates_metrics() -> color_eyre::eyre::Result<()> {
    let emitter = HeartbeatEmitter::new("test-service".to_string(), 30);
...
#[sinex_test]
fn heartbeat_metrics_include_latest_state() -> color_eyre::eyre::Result<()> {
    let emitter = HeartbeatEmitter::new("test-service".to_string(), 30);
...
#[sinex_test]
fn emit_heartbeat_macro_compiles() -> color_eyre::eyre::Result<()> {
