...
use sinex_satellite_sdk::{
    ...
    version::{SatelliteVersion, SatelliteInstance},
...
async fn test_satellite_complete_lifecycle(ctx: TestContext) -> Result<()> {
    ...
    let instance = SatelliteInstance::new(
        ...
        SatelliteVersion::parse("1.0.0+lifecycle").unwrap()
    );

    let mut coordination = SatelliteCoordination::new(instance.clone(), ctx.pool().clone());
    ...
    assert!(became_leader.load(Ordering::SeqCst), "Satellite should have become leader");
    let initial_processing = processing_count.load(Ordering::SeqCst);
    assert!(initial_processing > 0, "Satellite should have processed work");
    ...
    assert!(final_processing >= initial_processing, "Processing should not decrease");
...
async fn test_satellite_initialization_sequence(ctx: TestContext) -> Result<()> {
    ...
    for (service_name, version) in configs {
        ...
        let instance = SatelliteInstance::new(
            service_name,
            SatelliteVersion::parse(version).unwrap()
        );
        ...
        let registered = sqlx::query!(
            "SELECT instance_id, service_name, version FROM core.satellite_instances WHERE service_name = $1",
            service_name
        )
        .fetch_optional(ctx.pool())
        .await?;
        
        assert!(registered.is_some(), "Instance should be registered");
        let reg = registered.unwrap();
        assert_eq!(reg.service_name, service_name);
...
async fn test_satellite_health_monitoring(ctx: TestContext) -> Result<()> {
    ...
    let instance = SatelliteInstance::new(
        "health_monitor_test",
        SatelliteVersion::parse("1.0.0+health").unwrap()
    );
...
async fn test_satellite_error_recovery(ctx: TestContext) -> Result<()> {
    ...
    let instance = SatelliteInstance::new(
        "error_recovery_test",
        SatelliteVersion::parse("1.0.0+recovery").unwrap()
    );
    ...
    let recovery_handle = tokio::spawn(async move {
        timeout(Duration::from_millis(600), coordination.run_coordination_loop(|| {
    ...
    assert_eq!(final_errors, final_recoveries, "Each error should trigger recovery");
    assert!(final_successes > final_errors, "Should have more successes than errors");
    assert!(final_errors > 0, "Should have encountered some errors");

    info!("✓ Error recovery: {} errors, {} recoveries, {} successful operations", 
...
async fn test_satellite_state_transitions(ctx: TestContext) -> Result<()> {
    ...
    let instance = SatelliteInstance::new(
        ...
        SatelliteVersion::parse("1.0.0+states").unwrap()
    );
    ...
    let transition_handle = tokio::spawn(async move {
        timeout(Duration::from_millis(400), coordination.run_coordination_loop(|| {
    ...
    assert!(became_leader.load(Ordering::SeqCst), "Should have transitioned to leader");
    assert!(state_changes.load(Ordering::SeqCst) > 0, "Should have recorded state changes");
...
async fn test_satellite_configuration_lifecycle(ctx: TestContext) -> Result<()> {
    ...
    for (i, config) in test_configs.iter().enumerate() {
        ...
        let instance = SatelliteInstance::new(&config.base.service_name, version);
        
        let mut coordination = SatelliteCoordination::new(instance.clone(), ctx.pool().clone());
        coordination.initialize().await?;
        
        // Verify instance uses configuration correctly
        let registered = sqlx::query!(
            "SELECT service_name FROM core.satellite_instances WHERE service_name = $1",
            config.base.service_name
        )
        .fetch_one(ctx.pool())
        .await?;
        
        assert_eq!(registered.service_name, config.base.service_name);
...
async fn test_satellite_graceful_shutdown(ctx: TestContext) -> Result<()> {
    ...
    let instance = SatelliteInstance::new(
        "shutdown_test",
        SatelliteVersion::parse("1.0.0+shutdown").unwrap()
    );
    ...
    let shutdown_handle = tokio::spawn(async move {
        ...
        let result = coordination.run_coordination_loop(|| {
            ...
            async move {
                ...
                if start_time.elapsed() > Duration::from_millis(200) && !shutdown.load(Ordering::SeqCst) {
    ...
    assert!(operations_completed.load(Ordering::SeqCst) > 0, "Should have completed some operations");
    assert!(shutdown_initiated.load(Ordering::SeqCst), "Should have initiated shutdown");
    assert!(cleanup_completed.load(Ordering::SeqCst), "Should have completed cleanup");
    ...
    assert_eq!(saved_checkpoint.processed_count, final_checkpoint.processed_count);
...
async fn test_satellite_concurrent_lifecycle(ctx: TestContext) -> Result<()> {
    ...
    for i in 0..satellite_count {
        let instance = SatelliteInstance::new(
            "concurrent_test",
            SatelliteVersion::parse(&format!("1.0.{}", i)).unwrap()
        );
    ...
    for (i, result) in results.iter().enumerate() {
        assert!(result.as_ref().unwrap(), "Satellite {} should complete successfully", i);
    }

    let total_operations = completion_count.load(Ordering::SeqCst);
    assert!(total_operations > 0, "Should have completed operations");
    
    info!("✓ Concurrent satellite lifecycle completed: {} total operations", total_operations);
    Ok(())
}
