...
#[sinex_test]
async fn test_database_high_load_resilience(ctx: TestContext) -> color_eyre::eyre::Result<()> {
        ...
        let task = tokio::spawn(async move {
            ...
            match TestContext::with_name(&format!("conn_test_{}", i)).await {
                Ok(task_ctx) => {
                    ...
                    match task_ctx
                        .create_test_event("conn-test", "exhaustion", json!({"task": i}))
                        .await
                    {
                        Ok(_) => {
...
#[sinex_test]
async fn test_event_creation_extreme_payloads(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    let test_cases = vec![
        // Empty payload
        ("empty", json!({})),
        // Null payload
    ...
    for (name, payload) in test_cases {
        let result = ctx
            .create_test_event("extreme-test", name, payload.clone())
            .await;
...
#[sinex_test]
async fn test_concurrent_event_creation_stress(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    use std::sync::atomic::{AtomicU32, Ordering};
    use std::sync::Arc;
        ...
        match result {
            Ok(test_ctx) => {
                ...
                let event_result = test_ctx
                    .create_test_event(source, event_type, json!({"test": "invalid"}))
                    .await;
...
#[sinex_test]
async fn test_error_recovery_patterns(ctx: TestContext) -> color_eyre::eyre::Result<()> {
    ...
    for i in 0..10 {
        let result = if i % 3 == 0 {
            ...
            ctx.create_test_event("", "invalid", json!({})).await
        } else {
            // Valid operations
    ...
    let final_event = ctx
        .create_test_event("recovery-test", "final", json!({"recovered": true}))
        .await?;
