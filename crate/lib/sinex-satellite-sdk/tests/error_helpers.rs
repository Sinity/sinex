...
#[sinex_test]
fn io_error_with_context_includes_message() -> color_eyre::eyre::Result<()> {
    let cases = [
...
#[sinex_test]
fn io_error_with_empty_context_still_includes_source() -> color_eyre::eyre::Result<()> {
    let err = std::io::Error::new(ErrorKind::NotFound, "test error");
    match io_error_with_context(err, "") {
        SatelliteError::Processing(message) => assert!(message.contains("test error")),
        other => panic!("Expected Processing error, got {other:?}"),
    }
    Ok(())
}
...
#[sinex_test]
fn utf8_error_context_describes_failure() -> color_eyre::eyre::Result<()> {
...
#[sinex_test]
fn json_error_context_preserves_details() -> color_eyre::eyre::Result<()> {
    let invalid_cases = [
...
#[sinex_test]
fn processing_error_helpers_round_trip_messages() -> color_eyre::eyre::Result<()> {
    match processing_error("Something went wrong") {
        SatelliteError::Processing(message) => assert_eq!(message, "Something went wrong"),
        other => panic!("Expected Processing error, got {other:?}"),
    }

    match processing_error_fmt(format_args!("Value {} is invalid", 42)) {
        SatelliteError::Processing(message) => assert_eq!(message, "Value 42 is invalid"),
        other => panic!("Expected Processing error, got {other:?}"),
    }

    let special = "Error: 100% failed with UTF-8 chars: ñ, é, 中文";
    match processing_error(special) {
        SatelliteError::Processing(message) => assert_eq!(message, special),
        other => panic!("Expected Processing error, got {other:?}"),
    }
...
#[sinex_test]
fn error_chain_context_is_preserved() -> color_eyre::eyre::Result<()> {
...
#[sinex_test]
fn error_helpers_handle_empty_json() -> color_eyre::eyre::Result<()> {
...
#[sinex_test]
fn error_display_and_debug_include_context() -> color_eyre::eyre::Result<()> {
