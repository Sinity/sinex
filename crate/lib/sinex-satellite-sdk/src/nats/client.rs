...
impl NatsClient {
    ...
    async fn connect(config: &NatsConfig) -> Result<Client> {
        ...
        if let Some(auth) = &config.auth {
            options = match auth {
                super::config::AuthConfig::UserPassword { username, password } => {
                    options.user_and_password(username.clone(), password.clone())
                }
                super::config::AuthConfig::Token { token } => options.token(token.clone()),
                super::config::AuthConfig::NKey { seed } => {
                    // NATS expects the seed as a String for nkey auth
                    options = options.nkey(seed.clone());
                    options
                }
                super::config::AuthConfig::Jwt { jwt: _, seed: _ } => {
