...
impl DlqRetryHandler {
    ...
    pub async fn retry_all(&self) -> SatelliteResult<usize> {
        ...
        let stream = js
            ...
            .await
            .map_err(|e| SatelliteError::Processing(format!("Failed to get DLQ stream: {}", e)))?;
        ...
        let consumer = stream
            ...
            .await
            .map_err(|e| {
                SatelliteError::Processing(format!("Failed to create DLQ consumer: {}", e))
            })?;

        let mut messages = consumer.messages().await.map_err(|e| {
            SatelliteError::Processing(format!("Failed to get DLQ messages: {}", e))
        })?;
    ...
    pub async fn retry_by_id(&self, event_id: &str) -> SatelliteResult<()> {
        ...
        let consumer = stream
            ...
            .await
            .map_err(|e| {
                SatelliteError::Processing(format!("Failed to create specific DLQ consumer: {}", e))
            })?;
        ...
        if let Some(Ok(msg)) = messages.next().await {
            ...
            msg.ack().await.map_err(|e| {
                SatelliteError::Processing(format!("Failed to ack retried message: {}", e))
            })?;
    ...
    async fn retry_message(
        ...
        let original_subject = msg
            ...
            .ok_or_else(|| {
                SatelliteError::Processing("Missing Original-Subject header".to_string())
            })?;
        ...
        js.publish_with_headers(original_subject.to_string(), headers, msg.payload.clone())
            .await
            .map_err(|e| SatelliteError::Processing(format!("Failed to republish message: {}", e)))?
            .await
            .map_err(|e| {
                SatelliteError::Processing(format!("Failed to await publish ack: {}", e))
            })?;
    ...
    pub async fn get_stats(&self) -> SatelliteResult<DlqStats> {
        ...
        let info = stream
            .info()
            .await
            .map_err(|e| SatelliteError::Processing(format!("Failed to get stream info: {}", e)))?;
