...
impl ProgressTracker {
    ...
    pub async fn update(&mut self, processed: u64) {
        ...
        if guard.total_events > 0 {
            let remaining = guard.total_events.saturating_sub(guard.processed_events);
            guard.eta = Some(Duration::seconds(
                (remaining as f64 / guard.average_rate.max(0.01)).round() as i64,
            ));
        }
        drop(guard);
        self.invoke_callback().await;
