...
impl GitAnnex {
    ...
    pub async fn get_key(&self, file_path: &Utf8Path) -> Result<AnnexKey> {
        let output = AsyncCommand::new("git-annex")
            ...
            .output()
            .await
            .wrap_err("Failed to run git-annex lookupkey")?;
    ...
    pub async fn get_content(&self, key_or_path: &str) -> Result<()> {
        ...
        let output = AsyncCommand::new("git-annex")
            ...
            .output()
            .await
            .wrap_err("Failed to run git-annex get")?;
    ...
    pub async fn drop_content(&self, key_or_path: &str, force: bool) -> Result<()> {
        ...
        let output = cmd
            ...
            .output()
            .await
            .wrap_err("Failed to run git-annex drop")?;
    ...
    pub async fn fsck(&self, fast: bool, incremental: bool) -> Result<String> {
        ...
        let output = cmd
            ...
            .output()
            .await
            .wrap_err("Failed to run git-annex fsck")?;
    ...
    pub async fn status(&self) -> Result<String> {
        let output = AsyncCommand::new("git-annex")
            ...
            .output()
            .await
            .wrap_err("Failed to run git-annex status")?;
    ...
    async fn set_config(&self, key: &str, value: &str) -> Result<()> {
        let output = AsyncCommand::new("git")
            ...
            .output()
            .await
            .wrap_err("Failed to set git config")?;
...
mod tests {
    ...
    async fn test_blake3_hash() -> color_eyre::eyre::Result<()> {
        ...
        let test_file = Utf8Path::from_path(&test_file).unwrap();
