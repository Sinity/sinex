...
impl TableDef for Manifests {
    fn table_name() -> &'static str { "manifests" }
...
impl Manifests {
    ...
    pub fn create_table_statement() -> TableCreateStatement {
        Table::create()
            ...
            .col(ColumnDef::new(Manifests::CreatedAt).timestamp_with_time_zone().not_null()
    ...
    pub fn create_indexes() -> Vec<IndexCreateStatement> {
        vec![
            Index::create().if_not_exists().name("uk_manifest_identity")
                .table(Self::table_iden())
                .col(Manifests::Name).col(Manifests::Version)
                .unique().to_owned(),
            Index::create().if_not_exists().name("ix_manifests_type")
                .table(Self::table_iden())
                .col(Manifests::ManifestType).to_owned(),
            Index::create().if_not_exists().name("ix_manifests_parent")
                .table(Self::table_iden())
                .col(Manifests::ParentManifestId).to_owned(),
        ]
    }
}
