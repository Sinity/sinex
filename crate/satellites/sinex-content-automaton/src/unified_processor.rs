...
use sinex_core::db::models::{RawEvent, RpcContentResponsePayload, RpcError};
...
fn missing_field(field: &str) -> SatelliteError {
    SatelliteError::General(eyre!("Missing {}", field))
}
...
impl ContentServiceProcessor {
    ...
    async fn handle_store_blob(
        ...
        let annex_key = service
            ...
            .await
            .map_err(|e| SatelliteError::General(eyre!("Content storage error: {}", e)))?;
    ...
    async fn handle_retrieve_blob(
        ...
        let (_metadata, content) = service
            ...
            .await
            .map_err(|e| SatelliteError::General(eyre!("Content retrieval error: {}", e)))?;
    ...
    async fn handle_get_metadata(
        ...
        let metadata = service
            ...
            .await
            .map_err(|e| SatelliteError::General(eyre!("Metadata retrieval error: {}", e)))?;
                ...
                match self.handle_content_request(event.payload.clone()).await {
                    Ok(response) => {
                        ...
                        if let Some(ctx) = &self.context {
                            ...
                            let synthesis_event = RawEvent::from_payload(RpcContentResponsePayload {
