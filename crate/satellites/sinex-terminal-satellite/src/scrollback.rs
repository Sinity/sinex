...
pub struct ScrollbackWatcher {
    ...
    git_annex_repo: Option<Utf8PathBuf>,
    auto_annex: bool,
...
impl ScrollbackWatcher {
    ...
    pub async fn new() -> SatelliteResult<Self> {
        ...
        let kitty_socket_path = Utf8PathBuf::from(format!("{}/kitty", tmp_dir));
        ...
        let watcher = Self {
            ...
            git_annex_repo: Some(Utf8PathBuf::from("/realm/sinex-annex")),
            auto_annex: true,
        ...
        info!(
            ...
            watcher.kitty_socket_path.as_str()
        );
        Ok(watcher)
    }
    ...
    async fn get_kitty_windows(&self) -> SatelliteResult<Vec<WindowState>> {
        ...
        let output = tokio::process::Command::new("kitty")
            .arg("@")
            .arg("--to")
            .arg(format!("unix:{}", self.kitty_socket_path.as_str()))
            .arg("ls")
            .output()
            .await
            .map_err(|e| {
