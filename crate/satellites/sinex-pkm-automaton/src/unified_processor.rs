...
impl PkmServiceProcessor {
    ...
    async fn handle_create_note(
        ...
        let content = params
            .get("content")
            .and_then(|v| v.as_str())
            .ok_or_else(|| SatelliteError::General(eyre!("Missing content")))?;
        ...
        let annotation_id = service
            ...
            .await
            .map_err(|e| SatelliteError::General(eyre!("PKM error: {}", e)))?;
    ...
    async fn handle_create_entities(
        ...
        let source_material_id = params
            ...
            .and_then(|v| v.as_str())
            .and_then(|s| s.parse::<Ulid>().ok())
            .ok_or_else(|| SatelliteError::General(eyre!("Invalid or missing source_material_id")))?;
    ...
    async fn handle_link_entities(
        ...
        let from_entity_id = params
            ...
            .and_then(|s| s.parse::<Ulid>().ok())
            .ok_or_else(|| {
                SatelliteError::General(eyre!("Invalid or missing from_entity_id"))
            })?;
        ...
        let to_entity_id = params
            ...
            .and_then(|s| s.parse::<Ulid>().ok())
            .ok_or_else(|| {
                SatelliteError::General(eyre!("Invalid or missing to_entity_id"))
            })?;

        let relationship_type = params
            .get("relationship_type")
            .and_then(|v| v.as_str())
            .ok_or_else(|| SatelliteError::General(eyre!("Missing relationship_type")))?;
    ...
    async fn handle_register_source_material(
        ...
        let material_type = params
            .get("material_type")
            .and_then(|v| v.as_str())
            .ok_or_else(|| SatelliteError::General(eyre!("Missing material_type")))?;
                ...
                match self.handle_pkm_request(&event, event.payload.clone()).await {
                    Ok(response) => {
                        ...
                        if let Some(ctx) = &self.context {
                            ...
                            let synthesis_event: RawEvent = RawEvent::from_payload(RpcPkmResponsePayload {
    ...
    async fn scan(
        ...
        match until {
            TimeHorizon::Continuous => {
                ...
                info!("Starting continuous processing from NATS JetStream");
