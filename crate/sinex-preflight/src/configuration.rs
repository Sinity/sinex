...
pub async fn verify_configuration_generation() -> Result<(VerificationStatus, Value, Vec<String>)> {
    ...
    match verify_service_configuration_compatibility(&mut messages).await {
        ...
        Err(e) => {
            messages.push(format!("⚠ Service configuration compatibility warning: {}", e));
            has_warnings = true;
        }
    }
    ...
    info!("Configuration verification completed with status: {:?}", status);
...
async fn verify_environment_variables(messages: &mut Vec<String>) -> Result<Value> {
    ...
    for (var_name, description, required) in required_vars {
        match std::env::var(var_name) {
            Ok(value) => {
                ...
                env_vars.insert(var_name.to_string(), json!({
                    "value": display_value,
            ...
            Err(_) => {
                ...
                if required {
                    ...
                    messages.push(format!("✗ Required environment variable '{}' is missing", var_name));
                    has_issues = true;
                } else {
                    messages.push(format!("ℹ Optional environment variable '{}' is not set", var_name));
                }
            }
        }
    }
    ...
    for (var_name, description) in optional_vars {
        match std::env::var(var_name) {
            Ok(value) => {
                ...
                messages.push(format!("✓ Optional environment variable '{}' is set", var_name));
            }
            Err(_) => {
                env_vars.insert(var_name.to_string(), json!({
                    "description": description,
    ...
    if has_issues {
        bail!("Missing required environment variables: {}", missing_vars.join(", "));
    }
...
async fn verify_configuration_files(messages: &mut Vec<String>) -> Result<Value> {
    ...
    for (path_str, description) in config_paths {
        ...
        if expanded_path.exists() {
            match validate_toml_file(&expanded_path).await {
                Ok(config_info) => {
                    config_files.insert(path_str.to_string(), json!({
                ...
                Err(e) => {
                    ...
                    messages.push(format!("⚠ Configuration file exists but invalid: {} ({})", path_str, e));
                }
            }
        } else {
            config_files.insert(path_str.to_string(), json!({
    ...
    if let Ok(custom_config) = std::env::var("SINEX_CONFIG") {
        let custom_path = Path::new(&custom_config);
        
        if custom_path.exists() {
            match validate_toml_file(custom_path).await {
                Ok(config_info) => {
                    config_files.insert("SINEX_CONFIG".to_string(), json!({
                    ...
                    messages.push("✓ Custom configuration file (SINEX_CONFIG) is valid".to_string());
                }
                Err(e) => {
                    config_files.insert("SINEX_CONFIG".to_string(), json!({
                        "path": custom_path.to_string_lossy(),
                    ...
                    messages.push(format!("⚠ Custom configuration file (SINEX_CONFIG) is invalid: {}", e));
                }
            }
        } else {
            messages.push(format!("⚠ SINEX_CONFIG points to non-existent file: {}", custom_config));
        }
    }
...
async fn verify_service_configuration_compatibility(messages: &mut Vec<String>) -> Result<Value> {
    ...
    match check_systemd_compatibility().await {
        ...
        Err(e) => {
            ...
            compatibility_info.insert("systemd", json!({
                "compatible": false,
                "error": e.to_string()
            }));
        }
    ...
    match check_nixos_compatibility().await {
        ...
        Err(e) => {
            ...
            compatibility_info.insert("nixos", json!({
                "compatible": false,
                "note": e.to_string()
            }));
        }
...
async fn check_systemd_compatibility() -> Result<Value> {
    ...
    let systemd_version = std::process::Command::new("systemctl")
        .arg("--version")
        .output()
        .context("Failed to check systemd version")?;
...
async fn validate_toml_file(path: &Path) -> Result<Value> {
