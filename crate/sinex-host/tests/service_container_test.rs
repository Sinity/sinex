...
async fn test_service_container_initialization_success() -> Result<()> {
    ...
    assert!(Arc::strong_count(&container.analytics) > 0, "Analytics service should be initialized");
    assert!(Arc::strong_count(&container.content) > 0, "Content service should be initialized");
    assert!(Arc::strong_count(&container.pkm) > 0, "PKM service should be initialized");
    assert!(Arc::strong_count(&container.search) > 0, "Search service should be initialized");
...
async fn test_service_container_no_database_url() {
    ...
    match result {
        Err(error) => {
            assert!(
                error.to_string().contains("Database URL not provided and DATABASE_URL not set"),
...
async fn test_service_container_annex_path_config() -> Result<()> {
    ...
    assert!(Arc::strong_count(&container2.content) > 0, "Content service should be initialized with default path");
