...
pub fn auto_metrics(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        #(#attrs)*
        #vis #sig {
            let _guard = sinex_metrics_lib::track_function_call(#fn_name_str, module_path!());
            #original_block
        }
    };
...
pub fn auto_db_metrics(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        #(#attrs)*
        #vis #sig {
            let _guard = sinex_metrics_lib::track_database_query(#operation);
            #original_block
        }
    };
...
pub fn auto_event_metrics(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        #(#attrs)*
        #vis #sig {
            let _guard = sinex_metrics_lib::track_event_processing(#event_type);
            #original_block
        }
    };
...
pub fn auto_resource_metrics(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        #(#attrs)*
        #vis #sig {
            let _metrics = sinex_metrics_lib::create_system_metrics();
