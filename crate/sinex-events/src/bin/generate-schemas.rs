...
fn main() -> Result<()> {
    ...
    generate_schema::<WorkspaceSwitchedPayload>(schema_dir, "window_manager", "workspace_switched.json")?;
...
fn generate_schema<T: schemars::JsonSchema>(
    ...
    // Convert to JSON
    let mut json_schema = serde_json::to_value(&schema)
    ...
    if let Some(obj) = json_schema.as_object_mut() {
        ...
        obj.insert("$id".to_string(), 
            serde_json::json!(format!("https://sinex.io/schemas/v1/{}/{}", category, filename)));
    }
    ...
    let json_string = serde_json::to_string_pretty(&json_schema)
