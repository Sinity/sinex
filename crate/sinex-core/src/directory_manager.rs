...
impl DirectoryManager {
    ...
    async fn ensure_directory(&self, config: &DirectoryConfig) -> Result<()> {
        ...
        if !path.exists() {
            if config.create_if_missing {
                ...
                self.create_directory_with_permissions(path, config.mode)
                    .await
        ...
        #[cfg(unix)]
        {
            ...
            if current_mode != config.mode {
                ...
                self.set_directory_permissions(path, config.mode)
                    .await
                    .with_context(|| {
        ...
        // Test write access
        self.test_directory_access(path)
            .await
    ...
    async fn test_directory_access(&self, path: &Path) -> Result<()> {
        ...
        fs::write(&test_file, b"access_test")
            .await
        ...
        if content != b"access_test" {
            return Err(anyhow::anyhow!(
                "Test file content mismatch - filesystem corruption?"
            ));
        }
    ...
    pub fn get_standard_directories() -> Vec<DirectoryConfig> {
        ...
        let tmp_dir = std::env::var("SINEX_TMP_DIR").unwrap_or_else(|_| "/tmp/sinex".to_string());
        ...
        vec![
            DirectoryConfig::new(&data_dir, "Main data directory").with_mode(0o755),
            DirectoryConfig::new(format!("{}/events", data_dir), "Event storage").with_mode(0o755),
            DirectoryConfig::new(format!("{}/dlq", data_dir), "Dead letter queue").with_mode(0o755),
            DirectoryConfig::new(&tmp_dir, "Temporary files").with_mode(0o755),
            DirectoryConfig::new(&log_dir, "Log files").with_mode(0o755),
