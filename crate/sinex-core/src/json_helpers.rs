...
pub fn parse_json<T: DeserializeOwned>(
    ...
    serde_json::from_str(json_str)
        .map_err(|e| {
            ErrorContext::new(CoreError::Serialization(format!("Failed to parse {}", context_type)))
...
pub fn parse_json_file<T: DeserializeOwned>(
    ...
    serde_json::from_str(json_str)
        .map_err(|e| {
            ErrorContext::new(CoreError::Serialization("Failed to parse JSON file".to_string()))
                .with_operation(operation)
...
pub fn parse_json_value(
    ...
    serde_json::from_str(json_str)
        .map_err(|e| {
            ErrorContext::new(CoreError::Serialization(format!("Failed to parse {} as JSON", context_type)))
                .with_operation(operation)
...
pub fn extract_field<T: DeserializeOwned>(
    ...
    let field_value = value
        .get(field_name)
        .ok_or_else(|| {
            ErrorContext::new(CoreError::Validation(format!("Missing field: {}", field_name)))
                .with_operation(operation)
                .with_context("available_fields", format!("{:?}", value.as_object().map(|o| o.keys().collect::<Vec<_>>()).unwrap_or_default()))
                .build()
        })?;
    
    serde_json::from_value(field_value.clone())
        .map_err(|e| {
            ErrorContext::new(CoreError::Serialization(format!("Failed to deserialize field: {}", field_name)))
                .with_operation(operation)
                .with_context("error", e.to_string())
                .build()
        })
...
pub fn to_json_value<T: serde::Serialize>(
    ...
    serde_json::to_value(value)
        .map_err(|e| {
            ErrorContext::new(CoreError::Serialization(format!("Failed to serialize {}", context_type)))
                .with_operation(operation)
                .with_context("error", e.to_string())
                .build()
        })
