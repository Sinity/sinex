...
use crate::{
    sources, CoreError, DbPool, EventPipeline, EventReceiver, EventSender, PipelineConfig,
    RawEvent, Result,
};
use tokio::sync::mpsc;
...
impl PipelineCollectorBuilder {
    ...
    pub fn build(self) -> Result<PipelineAwareCollector> {
        let db_pool = self
            .db_pool
            .ok_or_else(|| CoreError::Configuration("Database pool is required".to_string()))?;
