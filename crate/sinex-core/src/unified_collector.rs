...
pub fn create_registry() -> EventRegistry {
    ...
    // Manually add all known event types for now
    // TODO: Replace with automatic registration from event crates
    ...
    builder.add_event_type("file.modified", "fs", || {
        use schemars::schema::*;
        RootSchema {
            meta_schema: None,
            schema: SchemaObject {
    ...
    // For now, let's just add a few key ones to test the concept
    
    builder.add_event_type("file.deleted", "fs", || {
        use schemars::schema::*;
        RootSchema {
            meta_schema: None,
            schema: SchemaObject {
    ...
    builder.add_event_type("command.executed", "shell.kitty", || {
        use schemars::schema::*;
        RootSchema {
            meta_schema: None,
            schema: SchemaObject {
        ...
        RootSchema {
            ...
            schema: SchemaObject {
                ...
                object: Some(Box::new(ObjectValidation {
                    ...
                    required: vec!["content_type".to_string(), "content_size".to_string()]
                        .into_iter()
                        .collect(),
                    ..Default::default()
                })),
...
#[allow(dead_code)]
pub fn create_registry_manual() -> EventRegistry {
    EventRegistry {
        event_types: &[
            // Filesystem events
            "file.created",
...
mod tests {
    ...
    fn test_create_registry() {
        ...
        assert!(registry.schema_for_event("file.created").is_some());
        assert!(registry.schema_for_event("nonexistent.event").is_none());
        
        // Test that events_for_source works
