...
use super::expected_outputs::ExpectedReplayOutputs;
use super::telemetry::ReplayPreviewSummary;
use super::validation::{
    REPLAY_OUTPUT_VISIBILITY_TIMEOUT, replay_scope_drift_error,
...
impl ReplayExecutionEngine {
    ...
    pub(super) async fn execute(&self, operation_id: Uuid, executor_name: String) -> Result<ReplayOperation> {
    ...
    pub(super) async fn submit(&self, operation_id: Uuid, submitter: String) -> Result<ReplayOperation> {
        let Some(_execution_lock) = self.replay.acquire_execution_lock(operation_id).await? else {
            return Err(eyre!(
                "Operation {} is already executing on another node",
                operation_id
