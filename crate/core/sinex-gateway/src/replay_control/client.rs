//! NATS-side client for issuing replay control commands.
//!
//! `ReplayControlClient` validates actor/scope arguments, ships the
//! request as JSON over the `sinex.control.replay` subject, and decodes
//! the response back into typed `ReplayOperation` values. The matching
//! server lives in `super::server`.
...
use super::protocol::{ReplayControlRequest, ReplayControlResponse, ReplayControlStatus};
