...
pub async fn handle_documents_search(pool: &PgPool, params: Value) -> Result<Value> {
    let req: DocumentsSearchRequest = serde_json::from_value(params).map_err(|e| {
        SinexError::serialization("invalid documents.search request").with_std_error(&e)
    })?;
...
pub async fn handle_documents_get(pool: &PgPool, params: Value) -> Result<Value> {
    let req: DocumentsGetRequest = serde_json::from_value(params).map_err(|e| {
        SinexError::serialization("invalid documents.get request").with_std_error(&e)
    })?;
...
pub async fn handle_documents_get_chunks(pool: &PgPool, params: Value) -> Result<Value> {
    let req: DocumentsGetChunksRequest = serde_json::from_value(params).map_err(|e| {
        SinexError::serialization("invalid documents.get_chunks request").with_std_error(&e)
    })?;
