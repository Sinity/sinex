...
pub async fn handle_nodes_list(
    ...
    while let Some(key) = entries.next().await {
        let key = key.map_err(|e| eyre!("Failed to read key: {}", e))?;
...
pub async fn handle_nodes_drain(
    ...
    nats_client
        ...
        .await
        .map_err(|e| eyre!("Failed to publish drain command: {}", e))?;
...
pub async fn handle_nodes_resume(
    ...
    nats_client
        .publish(subject, serde_json::to_vec(&payload)?.into())
        .await
        .map_err(|e| eyre!("Failed to publish resume command: {}", e))?;
...
pub async fn handle_nodes_set_horizon(
    ...
    nats_client
        .publish(subject, serde_json::to_vec(&payload)?.into())
        .await
        .map_err(|e| eyre!("Failed to publish set-horizon command: {}", e))?;
