...
//! Simulates the inotify-driven `FileDropAdapter` path: the source worker
...
pub fn build(data: &[u8]) -> Result<FixtureHandle, String> {
    let dir = TempDir::new()
        .map_err(|e| format!("failed to create file-drop temp dir: {e}"))?;
    ...
    Ok(FixtureHandle::with_resource(FixtureBinding::FilePath(watched_dir), dir))
}
