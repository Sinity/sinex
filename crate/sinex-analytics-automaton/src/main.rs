//! Analytics Service Automaton Binary
//!
//! Standalone binary that runs the analytics service as an automaton

use anyhow::Result;
use clap::Parser;
use sinex_analytics_automaton::AnalyticsServiceAutomaton;
...
#[derive(Parser)]
#[command(name = "sinex-analytics-automaton")]
#[command(about = "Analytics service automaton")]
struct Cli {
    ...
    /// Working directory
    #[arg(long, default_value = "/tmp/sinex-analytics")]
    work_dir: PathBuf,
    ...
    /// Consumer group name
    #[arg(long, default_value = "analytics-service-group")]
    consumer_group: String,
    
    /// Consumer name
    #[arg(long, default_value = "analytics-consumer")]
...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "sinex_analytics_automaton=info".into()),
        )
        .init();

    let cli = Cli::parse();

    // Create analytics automaton
    let analytics_automaton = AnalyticsServiceAutomaton::new();

    // Configure and run the automaton
    let mut runner = HotlogAutomatonRunner::new(analytics_automaton);
    ...
    // Set up event filters for analytics RPC requests
