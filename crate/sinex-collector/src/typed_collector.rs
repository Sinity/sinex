...
use sinex_core::{EventReceiver, EventSender, RawEvent};
use sinex_events::{typed_event_channel, EventEnvelope, TypedEventReceiver, TypedEventSender};
use tokio::sync::mpsc;
...
mod tests {
    ...
    use sinex_events::{FileCreatedPayload, TypedEventBuilder};
    ...
    async fn test_hybrid_collector() {
        ...
        // Send a JSON event
        let json_event =
            RawEventBuilder::new("test", "test.event", serde_json::json!({"data": "test"})).build();
        ...
        typed_tx
            .send(EventEnvelope::FileCreated(typed_event))
            .unwrap();
