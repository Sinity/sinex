//! RPC Dispatcher Binary
//!
//! Standalone binary that provides RPC protocol support and dispatches
//! requests to service automata, replacing the gateway monolith.
...
#[command(about = "RPC dispatcher with HTTP/WebSocket support")]
struct Cli {
    ...
    /// Service name
    #[arg(long, default_value = "rpc-dispatcher")]
    service_name: String,
    ...
    /// Working directory
    #[arg(long, default_value = "/tmp/sinex-rpc")]
    work_dir: PathBuf,
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "sinex_rpc_dispatcher=info".into()),
        )
        .init();

    let cli = Cli::parse();

    // Create RPC dispatcher automaton
    let rpc_dispatcher = RpcDispatcherAutomaton::new();
