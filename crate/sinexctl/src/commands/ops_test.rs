    ...
    SourcePackageCompletenessPackageView {
        ...
        modes: vec![
            sinex_primitives::rpc::sources::SourcePackageCompletenessModeView {
                ...
                replayability_class: Some("live_observation".to_string()),
                catch_up_authority: Some("none".to_string()),
                accepted_loss_policy: Some(serde_json::json!({
                    "kind": "accepted",
                    "rationale": "live-only terminal observation has no finite replay source",
                    "record": "sinex-r6d.8"
                })),
    ...
    SourcesRemediationPlanResponse {
        ...
        items: vec![SourceMaterialRemediationCandidate {
            material: SourceMaterialSummary {
                ...
                source_identifier: "browser.history#material=019f2105-b86b-78d2-a220-94f027e7e81f"
                    .to_string(),
            ...
            suggested_action:
                "mark timed-out material recovered_partial because events were admitted".to_string(),
        }],
    }
}
...
#[sinex_test]
async fn evidence_bundle_preserves_underlying_targets_caveats_and_actions() -> xtask::TestResult<()>
{
    let mut view = EvidenceBundleView::new("sinexctl.ops.evidence.compile");
    let source_ref = SinexObjectRef::new(SinexObjectKind::SourceDriver, "terminal.kitty-osc-live");
...
#[sinex_test]
async fn evidence_bundle_omissions_carry_target_caveats_and_diagnostics() -> xtask::TestResult<()> {
    let mut view = EvidenceBundleView::new("sinexctl.ops.evidence.compile");
...
#[sinex_test]
async fn catchup_status_command_is_registered_as_finite_readiness_view() -> xtask::TestResult<()> {
    let registry = crate::model::format_registry::build();
    let capability = registry
...
#[sinex_test]
async fn catchup_material_remediation_signal_preserves_policy_summary() -> xtask::TestResult<()> {
...
#[sinex_test]
async fn evidence_bundle_package_matching_accepts_package_mode_and_subject() -> xtask::TestResult<()>
{
...
#[sinex_test]
async fn evidence_bundle_compiler_reports_unavailable_requested_sections() -> xtask::TestResult<()>
{
    let spec = build_evidence_bundle_spec(
...
async fn ops_list_ndjson_renders_operation_view_records() -> xtask::TestResult<()> {
    ...
    let output =
        render_envelope(&envelope, &views, OutputFormat::Ndjson)?.expect("ndjson renders records");
...
#[sinex_test]
async fn debt_rows_from_source_coverage_reports_material_without_events() -> xtask::TestResult<()> {
...
#[sinex_test]
async fn debt_rows_from_source_coverage_reports_events_without_material() -> xtask::TestResult<()> {
...
#[sinex_test]
async fn debt_row_from_failed_eventful_material_points_to_remediation() -> xtask::TestResult<()> {
...
#[sinex_test]
async fn debt_row_from_recovered_partial_material_carries_recovery_reason() -> xtask::TestResult<()>
{
    let material = fixture_source_material_detail(
...
#[sinex_test]
async fn debt_rows_from_source_status_reports_unobserved_runtime_bridge() -> xtask::TestResult<()> {
    let mut source = fixture_source_status_coverage(
        SourceCoverageReadiness::MissingMaterial,
        SourceCoverageContinuity::Gapped,
        0,
    ...
    source.actions.push(ActionAvailability {
    ...
    assert!(
        row.actions.iter().any(|action| {
            action.id == "source.status.inspect"
                && action.command_hint.as_deref() == Some("sinexctl sources status --format json")
        }),
...
async fn debt_rows_from_replay_operations_reports_stranded_archive_rows() -> xtask::TestResult<()> {
    ...
    operation.preview_summary.as_mut().unwrap()["archive_recovery"] = serde_json::json!({
        "remaining_archived_events": 4,
    });

    let rows = debt_rows_from_replay_operations(&[operation]);
    assert_eq!(rows.len(), 1);
    let row = &rows[0];
    assert_eq!(row.id, "debt:replay-archive:op-replay-1");
    assert_eq!(row.stage, DebtStage::OperationFailed);
    assert!(row.summary.contains("4 stranded archived event(s)"));
    assert_eq!(row.caveats[0].id, "replay.archive.recovery_required");
    assert!(row.actions.iter().any(|action| {
        action.id == "replay.operation.inspect"
            && action.command_hint.as_deref() == Some("sinexctl ops jobs show op-replay-1")
    }));
    Ok(())
}
