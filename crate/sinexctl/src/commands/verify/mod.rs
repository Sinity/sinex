...
pub struct VerifyCommand {
    ...
    /// empty, seed it with `sinexctl ops demo`; then exercise documented
...
impl VerifyCommand {
    ...
    pub fn command_path(&self) -> &'static str {
        match self.cmd {
            Some(VerifySubcommand::Baseline(_)) => "verify baseline",
            None => "verify",
        }
    }
...
fn finalize_summary(summary: &VerificationSummary, format: OutputFormat) -> Result<()> {
    match format {
        ...
        OutputFormat::Dot => {
            return Err(eyre!(
                "sinexctl ops verify does not support --format dot; use --format json|yaml|table"
            ));
        }
    }
    Ok(())
...
// ─────────────────────────────────────────────────────────────
// `sinexctl ops verify --demo`  (#1172 AC-10)
// ─────────────────────────────────────────────────────────────
...
async fn run_demo_walkthrough(client: &GatewayClient, format: OutputFormat) -> Result<()> {
    ...
    // 1. Snapshot the current event-store size; if empty, run `sinexctl ops demo`.
    let initial_count = sample_events(client).await?;
    let seeded_now = if initial_count == 0 {
        if table_mode {
            println!("Database empty — invoking `sinexctl ops demo` to seed deterministic data.");
        }
        ...
        if !status.success() {
            return Err(eyre!(
                "sinexctl ops demo exited with status {status}; cannot run --demo walkthrough"
            ));
        }
        true
    } else {
...
fn render_demo_markdown(steps: &[DemoWalkthroughStep], seeded_now: bool) -> String {
    ...
    out.push_str(
        "Auto-generated by `sinexctl ops verify --demo`. \
This file is regenerated on every run; do not edit by hand.\n\n",
    );
    if seeded_now {
        out.push_str("The database was empty when this walkthrough started, so `sinexctl ops demo` was invoked to seed deterministic events before exercising queries.\n\n");
    } else {
