...
/// A caller-supplied byte offset inside a multibyte character must be floored
/// to a valid UTF-8 boundary throughout both query and DLQ completion paths.
#[sinex_test]
async fn completion_cursor_mid_multibyte_char_does_not_panic() -> TestResult<()> {
    for line in [
        "sinexctl query events where source = w\u{00F6}rd",
        "sinexctl ops dlq p\u{00F6}rd",
    ] {
        let char_start = line.find('\u{00F6}').expect("fixture contains ö");
        let cursor = char_start + 1;
        assert!(
            !line.is_char_boundary(cursor),
            "fixture cursor must land inside ö"
        );
        ...
        let response = cmd.complete(None).await;
        assert_eq!(response.cursor, char_start);
        assert!(line.is_char_boundary(response.cursor));
        assert!(response.candidates.iter().all(|candidate| {
            line.is_char_boundary(candidate.replace_start)
                && line.is_char_boundary(candidate.replace_end)
        }));
    }
    Ok(())
}
...
#[test]
fn cursor_normalization_floors_and_clamps() {
    let line = "w\u{00F6}rd";
    assert_eq!(normalize_cursor(line, 2), 1);
    assert_eq!(normalize_cursor(line, usize::MAX), line.len());
}
