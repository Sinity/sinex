...
fn gateway_unavailable_envelope(tool: &str, query: &Value, target_url: &str) -> Result<Value> {
    let payload = json!({
        ...
        "suggested_next_probe": "Check the runtime target with `sinexctl runtime gateway ping` or inspect the AgentCTL lease with `agentctl job list` before relying on runtime-status evidence.",
        "result": null,
    });
