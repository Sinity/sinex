...
fn config_init(force: bool) -> Result<()> {
    ...
    let editor = Text::new("Preferred editor:")
        .with_default(
            &std::env::var("EDITOR").unwrap_or_else(|_| std::env::var("VISUAL").unwrap_or_else(|_| "vim".to_string())),
        )
    ...
    config_content.push_str("# Runtime connection/auth/TLS settings come from CLI flags or env vars.\n");
