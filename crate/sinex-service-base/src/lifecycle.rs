...
use crate::{Service, ServiceContext, ServiceError, ServiceId, ServiceName, ServiceResult};
...
impl ServiceLifecycle {
    ...
    pub async fn wait_for_state(&self, target_state: LifecycleState) -> ServiceResult<()> {
        loop {
            ...
            if matches!(current_state, LifecycleState::Error(_))
                && !matches!(target_state, LifecycleState::Error(_))
            {
                return Err(ServiceError::Runtime(format!(
                    "Service entered error state: {:?}",
                    current_state
                )));
            }
    ...
    pub async fn initialize(&mut self, context: ServiceContext) -> ServiceResult<()> {
        ...
        if !matches!(current_state, LifecycleState::Created) {
            return Err(ServiceError::Initialization(format!(
                "Cannot initialize service in state: {:?}",
                current_state
            )));
        }
        ...
        match self.service.initialize(context.clone()).await {
            ...
            Err(e) => {
                ...
                self.change_state(LifecycleState::Error(error_msg.clone()))
                    .await;
...
impl LifecycleManager {
    ...
    pub fn add_service(
        ...
        self.dependencies
            .insert(service_id, dependencies.into_iter().collect());
    ...
    pub async fn wait_for_service_state(
        ...
        match self.services.get(service_id) {
            ...
            None => Err(ServiceError::Configuration(format!(
                "Service {} not found",
                service_id
            ))),
        }
