...
use crate::{ServiceResult, ServiceError, ServiceName};
...
impl ServiceConfig {
        ...
        match self.get(key) {
            Some(value) => {
                let typed_value = serde_json::from_value(value.clone())
                    .map_err(|e| ServiceError::Configuration(format!(
                        "Failed to parse config key '{}': {}", key, e
                    )))?;
                Ok(Some(typed_value))
            }
            None => Ok(None),
    ...
    {
        self.get_typed(key)?
            .ok_or_else(|| ServiceError::Configuration(format!("Required config key '{}' not found", key)))
    }
    ...
    pub fn validate(&self) -> ServiceResult<()> {
        if let Some(_schema) = &self.schema {
            let config_value = serde_json::to_value(self.as_map())
                .map_err(|e| ServiceError::Configuration(format!("Failed to serialize config: {}", e)))?;
            
            // In practice, you'd use a JSON Schema validation library here
            // For now, we'll just do basic validation
            if !config_value.is_object() {
                return Err(ServiceError::Configuration("Configuration must be an object".to_string()));
            }
...
impl ConfigManager {
    ...
    pub async fn load_from_file(&mut self, service_name: impl Into<ServiceName>, file_path: impl AsRef<Path>) -> ServiceResult<()> {
        let service_name = service_name.into();
        let file_path = file_path.as_ref();
        
        let content = tokio::fs::read_to_string(file_path).await
            .map_err(|e| ServiceError::Configuration(format!("Failed to read config file {}: {}", file_path.display(), e)))?;
        
        let toml_value: toml::Value = content.parse()
            .map_err(|e| ServiceError::Configuration(format!("Failed to parse TOML config: {}", e)))?;
        
        let json_value = toml_to_json(toml_value);
        
        let config = self.configs.entry(service_name.clone()).or_insert_with(|| ServiceConfig::new(&service_name));
    ...
    pub fn load_from_env(&mut self, service_name: impl Into<ServiceName>, prefix: &str) -> ServiceResult<()> {
        let service_name = service_name.into();
        let config = self.configs.entry(service_name.clone()).or_insert_with(|| ServiceConfig::new(&service_name));
    ...
    pub fn set_config(&mut self, service_name: impl Into<ServiceName>, key: impl Into<String>, value: serde_json::Value) {
        let service_name = service_name.into();
        let config = self.configs.entry(service_name.clone()).or_insert_with(|| ServiceConfig::new(&service_name));
    ...
    pub fn validate_all(&self) -> ServiceResult<()> {
        for (service_name, config) in &self.configs {
            config.validate()
                .map_err(|e| ServiceError::Configuration(format!("Validation failed for service {}: {}", service_name, e)))?;
        }
        
        self.global_config.validate()
            .map_err(|e| ServiceError::Configuration(format!("Global config validation failed: {}", e)))?;
    ...
    pub async fn hot_reload(&mut self, service_name: &str) -> ServiceResult<Vec<String>> {
        let config = self.configs.get(service_name)
            .ok_or_else(|| ServiceError::Configuration(format!("Service {} not found", service_name)))?;
        
        let mut reloaded_keys = Vec::new();
        
        // Find file sources to reload
        let file_sources: Vec<String> = config.entries
            .values()
            .filter_map(|entry| {
