...
use crate::{ServiceName, ComponentName, ServiceResult, ServiceError};
...
impl ServiceMetrics {
    ...
    pub fn summary(&self) -> MetricsSummary {
        MetricsSummary {
            ...
            operations_per_second: self.gauges.get("operations_per_second").copied().unwrap_or(0.0),
            avg_operation_duration_ms: self.gauges.get("avg_operation_duration_ms").copied().unwrap_or(0.0),
            error_rate_percent: self.gauges.get("error_rate_percent").copied().unwrap_or(0.0),
            memory_usage_mb: self.gauges.get("memory_usage_mb").map(|v| *v as u64).unwrap_or(0),
            cpu_usage_percent: self.gauges.get("cpu_usage_percent").copied().unwrap_or(0.0),
            active_connections: self.gauges.get("active_connections").map(|v| *v as u32).unwrap_or(0),
        }
    }
}
...
impl StatusManager {
    ...
    pub fn register_component(&self, reporter: Box<dyn StatusReporter>) -> ServiceResult<()> {
        ...
        if components.contains_key(&component_name) {
            return Err(ServiceError::Configuration(format!(
                "Component '{}' already registered", component_name
            )));
        }
    ...
    fn generate_metrics_summary(&self, component_statuses: &HashMap<ComponentName, ComponentStatus>) -> ServiceResult<MetricsSummary> {
...
impl SimpleStatusReporter {
    ...
    pub fn set_health(&self, health: HealthStatus) -> ServiceResult<()> {
        let mut current_health = self.health.write()
            .map_err(|e| ServiceError::Runtime(format!("Failed to acquire write lock: {}", e)))?;
    ...
    pub fn set_message(&self, message: impl Into<String>) -> ServiceResult<()> {
        let mut current_message = self.message.write()
            .map_err(|e| ServiceError::Runtime(format!("Failed to acquire write lock: {}", e)))?;
...
impl StatusReporter for SimpleStatusReporter {
    ...
    fn status(&self) -> ComponentStatus {
        let state = self.state.read().unwrap_or_else(|_| panic!("Poisoned lock")).clone();
        let health = self.health.read().unwrap_or_else(|_| panic!("Poisoned lock")).clone();
        let message = self.message.read().unwrap_or_else(|_| panic!("Poisoned lock")).clone();
        let metrics = self.metrics.read().unwrap_or_else(|_| panic!("Poisoned lock")).clone();
    ...
    fn metrics(&self) -> HashMap<String, serde_json::Value> {
        self.metrics.read().unwrap_or_else(|_| panic!("Poisoned lock")).clone()
    }
}