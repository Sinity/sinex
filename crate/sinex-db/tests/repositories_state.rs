...
async fn register_module_is_idempotent_per_manifest_version(ctx: TestContext) -> TestResult<()> {
    ...
    assert_eq!(
        first.id, second.id,
        "duplicate registration should reuse the manifest row"
    );
    ...
    assert_eq!(
        matching.len(),
        1,
        "duplicate registration should not create extra rows"
    );
...
async fn run_system_health_checks_preserves_clean_probe_state(ctx: TestContext) -> TestResult<()> {
    ...
    assert!(health.module_health.is_some());
    assert_eq!(health.module_health_error, None);
    Ok(())
}
