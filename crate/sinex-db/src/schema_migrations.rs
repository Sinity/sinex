...
pub fn create_all_indexes() -> Vec<String> {
    ...
    indexes.extend(Checkpoints::create_indexes());
    indexes.extend(Schemas::create_indexes());
...
mod tests {
    ...
    async fn test_create_all_tables(_ctx: TestContext) -> Result<()> {
        ...
        assert!(tables.iter().any(|sql| sql.contains("core.checkpoints")));
        assert!(tables.iter().any(|sql| sql.contains("core.payload_schemas")));
        assert!(tables.iter().any(|sql| sql.contains("raw.source_material_registry")));
        assert!(tables.iter().any(|sql| sql.contains("core.operations_log")));
        assert!(tables.iter().any(|sql| sql.contains("audit.archived_events")));
        assert!(tables.iter().any(|sql| sql.contains("core.entities")));
        assert!(tables.iter().any(|sql| sql.contains("core.entity_relations")));
        Ok(())
    }
    ...
    async fn test_generate_complete_migration(_ctx: TestContext) -> Result<()> {
        ...
        assert!(migration.contains("core.checkpoints"));
