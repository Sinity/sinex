        ...
        for stage in &self.stages {
            ...
            match stage.process(staged_event.event.clone()).await {
                ...
                Err(e) => {
                    ...
                    error!("Stage {} failed after {:?}: {}", stage_name, duration, error_msg);
...
impl PipelineMetrics {
    ...
    fn record_stage_success(&self, stage_name: &str, duration: Duration) {
        // In a real implementation, this would use atomic operations
        // For now, this is a placeholder
    }

    fn record_stage_failure(&self, stage_name: &str, duration: Duration) {
        // In a real implementation, this would use atomic operations
        // For now, this is a placeholder
    }
...
impl EventTiming {
    ...
    pub fn total_time(&self) -> Option<Duration> {
        self.processing_completed.map(|completed| completed.duration_since(self.received_at))
    }
}
...
impl PipelineStage for EnrichmentStage {
    async fn process(&self, mut event: RawEvent) -> Result<RawEvent> {
        ...
        if let Some(payload) = event.payload.as_object_mut() {
            payload.insert("enriched_at".to_string(), serde_json::Value::String(chrono::Utc::now().to_rfc3339()));
            payload.insert("enriched_by".to_string(), serde_json::Value::String("sinex-pipeline".to_string()));
        }
...
impl PipelineStage for DistributionStage {
    async fn process(&self, event: RawEvent) -> Result<RawEvent> {
        ...
        debug!("Distributing event: {} - {}", event.source, event.event_type);
        Ok(event)
    }
