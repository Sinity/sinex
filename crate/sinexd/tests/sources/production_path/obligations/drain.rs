...
//!
//! - `SourceDrainController::request_drain()` advances state from `Idle`
//!   to `StoppingAccept`.
...
use sinexd::sources::drain::{DrainPhase, SourceDrainController};
use std::sync::Arc;
use std::time::Duration;

/// Run the drain obligation for a source.
///
/// # Errors
///
...
pub async fn run(
    ...
    let controller = Arc::new(SourceDrainController::new());

    // Simulate one unit of in-flight work.
    controller.enter_work();

    // Signal drain; should transition to StoppingAccept.
    let initiated = controller.request_drain(source_id).await;
    if !initiated {
        return Err(format!(
            "drain for '{source_id}': request_drain() returned false on first call"
        ));
    }
    ...
    if phase != DrainPhase::StoppingAccept {
        return Err(format!(
            "drain for '{source_id}': expected StoppingAccept after request, got {phase:?}"
        ));
    }
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        controller.finish_active_work(source_id).await;
        controller.flush_intents(source_id).await;
        controller
            .wait_confirmations(source_id, Duration::from_millis(100))
            .await;
        controller.finalize_materials(source_id).await;
        controller.save_checkpoint(source_id).await;
    ...
    if final_phase != DrainPhase::Drained {
        return Err(format!(
            "drain for '{source_id}': expected Drained after full phase sequence, got {final_phase:?}"
        ));
    }
