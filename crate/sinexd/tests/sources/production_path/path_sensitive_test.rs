...
const KNOWLEDGEBASE_CRLF_NOTE: &[u8] =
    b"---\r\nid: crlf.production\r\ncreated: 2025-03-15\r\n---\r\n\
Body survives the CRLF front-matter boundary with a [[body-link]] and #body-tag.\r\n";
...
#[sinex_test]
async fn knowledgebase_vault_crlf_front_matter_production_route() -> TestResult<()> {
    let failures = crate::_run_case_with_logical_path(
        "knowledgebase-vault",
        crate::AdapterKind::StaticFile,
        KNOWLEDGEBASE_CRLF_NOTE,
        "notes/crlf.production.md",
        &["note.observed"],
        crate::ALL_OBLIGATIONS,
    )
    .await;
    assert!(
        failures.is_empty(),
        "knowledgebase-vault CRLF production route failed: {failures:#?}"
    );
    Ok(())
}
