...
async fn gateway_tls_accepts_handshake(ctx: TestContext) -> Result<()> {
    ...
    let content_store_path = temp.path().join("content-store");
    let content_store_path = content_store_path.to_string_lossy().to_string();

    let mut env = EnvGuard::new();
    env.set("SINEX_API_TOKEN", "test-token");
    ...
    let config = sinexd::api::config::GatewayConfig::load()?.with_cli_overrides(
