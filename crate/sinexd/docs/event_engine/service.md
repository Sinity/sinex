# Ingest Service

`service.rs` orchestrates event_engine startup and the `JetStream` consumers that
receive events and source material from sources. It applies validation,
delegates persistence to repositories, and drives material assembly.

- Runs `JetStream` consumers for events and material slices.
- Batches writes to reduce contention.
- Emits structured logs with `UUIDv7` IDs for provenance tracking.

Refer to `README.md#architecture` for the event flow diagram and
queue interactions.
