...
| Log/error | Log-bound malformed event diagnostics report length plus BLAKE3 fingerprints for invalid operator-supplied or upstream bytes, not raw field values. Structured logs may still include persisted event UUID refs, source IDs, event types, counts, sizes, and timings. The retired confirmation-watermark retry path no longer has a separate disclosure surface. | `payload_fingerprint_does_not_disclose_raw_log_bytes`; event-card/SSE disclosure tests cover rendered payload exposure. |
