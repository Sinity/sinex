# Source Drain Protocol

This record defines the shutdown and recovery contract for `sinexd::sources`
source contracts that emit material-provenance events from live or staged source
...
The drain controller owns a per-source phase machine. It wraps the
...
| `SavingCheckpoint` | Durable source state is being saved. | Persist checkpoint/state after material finalization decisions are visible. |
...
source drain phase is the lifecycle clock; source-specific code only
...
source identities. The parent source owns:
...
- full seven-phase drain through a source obligation;
- restart after mid-drain crash produces gap evidence;
- per-unit drain isolation when multiple source contracts share a host;
...
- child-process crash degrades only the owning source;
- private-mode and load-shed gaps appear in continuity reports.

Current narrow proof exists in source drain obligations and the
