...
| `{ENV}_SINEX_RAW_EVENTS_CONFIRMED_EVENTS` / `{env}.events.confirmed.{provenance}.{source}.{event_type}` | **Confirmation (JetStream full-payload bus) → Confirmation** | delivery bus only; DB is archive | at-least-once to durable consumers, bounded Limits+discard:Old | publish after commit; raw ACK only after confirmed publish succeeds | `Nats-Msg-Id` = event id | fatal durability-gap leaves raw message unacked for redelivery | consumers catch up from DB when stream tail is gone | n/a (signal *from* admission to automata/SSE) | JetStream |
