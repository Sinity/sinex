...
| `{ENV}_SINEX_RAW_EVENTS_DLQ` / `{env}.events.dlq.>` (publish: `events.dlq.event_engine`) | Critical (DLQ) → **Critical (DLQ)** | operator-durable, 7d, `dupe_window=1h` | at-least-once; publish confirmation gates raw ACK | operator-reviewed; bulk requeue is an operation with durable progress and startup recovery | route IDs include full intent identity; requeue IDs use DLQ sequence + generation | terminal surface (this *is* the DLQ) | requeue publishes exact bytes, then deletes the settled DLQ entry | re-enters admission on requeue | JetStream |
