...
//! staging. The source contract and binding live in
...
impl DocumentSourceDriver {
    ...
    fn record_scan_report(&self, report: &ScanReport) -> RuntimeResult<()> {
    ...
    fn last_scan_report(&self) -> RuntimeResult<Option<ScanReport>> {
        self.exploration
            .lock()
    ...
    ) -> RuntimeResult<ResolvedDocumentTargets> {
    ...
    fn expand_target(
        ...
        warnings: &mut Vec<String>,
    ) -> RuntimeResult<Vec<ResolvedDocumentTarget>> {
    ...
    ) -> RuntimeResult<DocumentInspection> {
        ...
        for target in &resolved_targets.targets {
            ...
            match self.inspect_target(target, state).await {
                    ...
                    match self.ingest_document(&document).await {
                        ...
                        Err(err) => {
                            error!(
                                ...
                                metric = "source.document_ingest_failures_total",
                                path = %document.path,
                                error = %err,
                ...
                Err(err) => {
                    error!(
                        target: "sinex_metrics",
                        metric = "source.document_inspect_failures_total",
                        path = %target.path,
                        error = %err,
    ...
    async fn ingest_document(&self, document: &DocumentInput) -> RuntimeResult<Option<Uuid>> {
