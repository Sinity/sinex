//!
//! See `crate::runtime::docs::coordination` for architectural details on leadership election and handoff.
//!
...
pub struct HandoffRequest {
    ...
    pub requester_version: RuntimeVersion,
...
impl From<&RuntimeInstance> for InstanceMetadata {
    fn from(instance: &RuntimeInstance) -> Self {
        instance_metadata_at(instance, None)
    }
}

fn instance_started_at(instance: &RuntimeInstance) -> i64 {
...
fn version_identity(version: &RuntimeVersion) -> &str {
...
impl RuntimeCoordination {
    ...
    pub fn new(
        ...
        instance_id: String,
        nats_client: async_nats::Client,
        _runtime_state: &RuntimeContext,
    ) -> crate::runtime::RuntimeResult<Self> {
        let service_name = service_name.into();
        let instance = RuntimeInstance::new(instance_id.clone(), service_name.clone())?;
    ...
    /// # Example
    /// ```ignore
    /// let coordinator = RuntimeCoordination::from_runtime(&runtime, instance_id)?;
    ...
    pub async fn maybe_initiate_handoff(&self) -> Result<bool> {
        ...
        let Ok(leader_version) = leader_metadata.version.parse::<RuntimeVersion>() else {
            warn!(
