...
fn verify_environment_variables(messages: &mut Vec<String>) -> RuntimeResult<Value> {
...
fn verify_event_source_configuration(messages: &mut Vec<String>) -> RuntimeResult<Value> {
...
pub fn validate_readable_file(path: &Path) -> RuntimeResult<()> {
...
fn validate_sqlite_tables(path: &Path, label: &str, tables: &[&str]) -> RuntimeResult<()> {
...
pub fn validate_atuin_history_db(path: &Path) -> RuntimeResult<()> {
    validate_sqlite_tables(path, "Atuin history", &["history"])
}

pub fn validate_fish_history_db(path: &Path) -> RuntimeResult<()> {
    validate_sqlite_tables(path, "Fish history", &["history"])
}

pub fn validate_activitywatch_db(path: &Path) -> RuntimeResult<()> {
    validate_sqlite_tables(path, "ActivityWatch history", &["events", "buckets"])
}

pub fn validate_terminal_history_source(shell: &str, path: &Path) -> RuntimeResult<()> {
    match shell {
...
fn validate_document_root(path: &Path) -> RuntimeResult<()> {
...
async fn verify_service_environment(messages: &mut Vec<String>) -> RuntimeResult<Value> {
...
async fn check_systemd_environment() -> RuntimeResult<Value> {
...
fn parse_systemd_version_line(stdout: &[u8]) -> RuntimeResult<String> {
...
async fn check_nixos_environment() -> RuntimeResult<Value> {
