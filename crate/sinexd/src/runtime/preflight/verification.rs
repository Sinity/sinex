...
async fn verify_database_integration(messages: &mut Vec<String>) -> RuntimeResult<Value> {
...
async fn test_schema_access(pool: &PgPool, _messages: &mut Vec<String>) -> RuntimeResult<Value> {
...
async fn relation_exists(pool: &PgPool, schema: &str, table: &str) -> RuntimeResult<bool> {
...
async fn test_transactions(pool: &PgPool, _messages: &mut [String]) -> RuntimeResult<Value> {
...
/// Test database extensions
async fn test_database_extensions(pool: &PgPool, _messages: &mut [String]) -> RuntimeResult<Value> {
...
async fn verify_service_integration(_messages: &mut [String]) -> RuntimeResult<Value> {
    ...
    // test namespace. Derived stream names are based on the already-namespaced
    // raw-events base so the namespace lands in the middle.
    let namespace = crate::runtime::error_helpers::env_nonempty_string_optional(
        "SINEX_NAMESPACE",
    ...
    let required_streams = [
        ...
        format!("{raw_events_stream}_CONFIRMED"),
        env.nats_stream_name_with_namespace(
            namespace.as_deref(),
            crate::runtime::SOURCE_MATERIAL_STREAM,
        ),
    ];
...
async fn get_test_pool() -> RuntimeResult<PgPool> {
