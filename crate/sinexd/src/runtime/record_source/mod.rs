...
use crate::runtime::{
    AppendOnlyFileChange, AppendOnlyFileState, RuntimeResult, SourceRecordAnchor, TailError,
    acquisition_manager::{
...
use crate::runtime::sqlite_source::{
    ...
    Sink: RecordMaterialSink,
{
    ...
    pub async fn append_json_line<T>(&self, record: &T) -> RuntimeResult<SourceRecordAnchor>
    where
        T: Serialize + ?Sized,
    {
    ...
    pub async fn append_stable_bytes(&self, bytes: Vec<u8>) -> RuntimeResult<SourceRecordAnchor> {
    ...
    Sink: RecordMaterialSink,
{
    ...
    ) -> RuntimeResult<RecordProcessReport<Source::Checkpoint, Warning>>
    where
...
pub trait RecordMaterialSink: Clone + Send + Sync + 'static {
    fn append_record(
        &self,
        bytes: Vec<u8>,
    ) -> impl Future<Output = RuntimeResult<SourceRecordAnchor>> + Send + '_;
...
impl RecordMaterialSink for BufferedRecordSink {
    async fn append_record(&self, bytes: Vec<u8>) -> RuntimeResult<SourceRecordAnchor> {
        self.writer.append(bytes).await
    }

    async fn flush<'a>(&'a self, reason: &'a str) -> RuntimeResult<()> {
        self.writer.flush(reason).await
    }

    async fn finalize<'a>(&'a self, reason: &'a str) -> RuntimeResult<()> {
    ...
    ReadError: Error + Send + Sync + 'static,
{
    pub async fn read_process_lenient_with_snapshot<Warning, Process, ProcessFuture, Warn>(
        ...
        mut process: Process,
        warning_disposition: Warn,
    ) -> RuntimeResult<RecordProcessReport<SqliteRowCheckpoint, Warning>>
    where
...
pub fn stable_json_line<T>(record: &T) -> RuntimeResult<Vec<u8>>
where
    T: Serialize + ?Sized,
{
    ...
    async fn sqlite_harness_records_snapshot_success_and_failure_evidence(
        ...
        let source = RecordSources::sqlite(
            ...
            |path, from_row_id, _end_time| {
                crate::runtime::sqlite_source::read_rows_after(
                    path,
        ...
        assert_eq!(
            snapshot.trigger,
            crate::runtime::sqlite_source::SqliteSnapshotTrigger::FirstObservation
        );
