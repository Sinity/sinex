...
/// - `TransducerAdapter<T>` = `AutomatonRuntime<TransducerWrapper<T>>`
/// - `WindowedAdapter<T>` = `AutomatonRuntime<WindowedWrapper<T>>`
/// - `ScopeReconcilerAdapter<T>` = `AutomatonRuntime<ScopeReconcilerWrapper<T>>`
...
where
    N: Automaton,
{
    type Config = AutomatonAdapterConfig;
    ...
    async fn scan(
        ...
        match until {
            ...
            TimeHorizon::Snapshot => Err(SinexError::unknown(
                "Automaton does not support snapshot mode",
            )),
        }
    }
    ...
    async fn periodic_flush(&mut self, now: Timestamp) -> RuntimeResult<u64> {
...
impl<N> crate::runtime::exploration::ExplorationProvider for AutomatonRuntime<N>
where
    N: Automaton,
{
...
pub type TransducerAdapter<N> = AutomatonRuntime<super::traits::TransducerWrapper<N>>;

/// Adapter for a `Windowed` implementation.
pub type WindowedAdapter<N> = AutomatonRuntime<super::traits::WindowedWrapper<N>>;

/// Adapter for a `ScopeReconciler` implementation.
pub type ScopeReconcilerAdapter<N> = AutomatonRuntime<super::traits::ScopeReconcilerWrapper<N>>;

/// Adapter for a `MultiOutputTransducer` implementation.
pub type MultiOutputTransducerAdapter<N> =
