...
//! let config = RuntimeConfig::load_from_env("my-service")?;
//! # Ok::<(), crate::runtime::config::ConfigError>(())
//! ```
//!
...
impl RuntimeConfig {
    ...
    /// let config = RuntimeConfig::load_from_env("debug-service")?;
...
impl EventSourceConfig {
    fn defaults(service_name: &str) -> Self {
        Self {
            base: RuntimeConfig::defaults(service_name),
    ...
    pub fn load_from_env(service_name: &str) -> Result<Self, ConfigError> {
        ...
        let config = Self {
            base: RuntimeConfig::load_from_env(service_name)?,
