...
impl ConfigAccessor for RuntimeContext {
    fn config_map(&self) -> &HashMap<String, serde_json::Value> {
        self.raw_config()
    }
}
...
/// `RuntimeResult` with proper error context, eliminating the verbose
...
impl<T, E: std::fmt::Display> RuntimeErrorExt<T> for Result<T, E> {
    ...
    fn processing_err(self, context: &str) -> Result<T, crate::runtime::SinexError> {
        self.map_err(|e| crate::runtime::SinexError::processing(format!("{context}: {e}")))
    }
}

#[cfg(test)]
#[path = "error_helpers_test.rs"]
mod tests;
