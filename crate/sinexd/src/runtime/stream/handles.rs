...
pub struct EventEmitter {
    ...
    emit_tracker:
        Arc<parking_lot::RwLock<Option<Arc<crate::runtime::health_reporter::EmitTracker>>>>,
}
...
impl EventEmitter {
    ...
    pub fn register_emit_tracker(
        &self,
        tracker: Arc<crate::runtime::health_reporter::EmitTracker>,
    ) {
...
impl RuntimeHandles {
    ...
    /// Create `RuntimeHandles` for Edge Mode (no database)
    #[allow(clippy::too_many_arguments)]
    #[must_use]
    pub fn new_edge(
        checkpoint_manager: Arc<CheckpointManager>,
...
impl<C> RuntimeInitContext<C> {
    ...
    pub fn runtime_state(&self) -> RuntimeContext {
        RuntimeContext::new(
            self.service.clone(),
            self.handles.clone(),
    ...
    /// Consume the context, yielding runtime config and runtime state.
    pub fn into_runtime(self) -> (C, RuntimeContext) {
