...
impl RuntimeRunner {
    ...
    pub(super) async fn execute_dispatched_scan(
        ...
        let replay_service_info = ServiceInfo::new_with_runtime_identity(
            ...
            base_service_info.module_name().to_string(),
    ...
    pub(super) async fn build_replay_worker_handles(
        ...
        let replay_handles = match base_handles.db_pool().cloned() {
            Some(db_pool) => RuntimeHandles::new(
                db_pool,
                checkpoint_manager,
                replay_emitter,
        ...
        #[cfg(not(feature = "db"))]
        let replay_handles = RuntimeHandles::new_edge(
            checkpoint_manager,
            replay_emitter,
            base_handles.transport().clone(),
            schema_cache,
        ...
        let replay_handles =
            replay_handles.with_settlement_registry(base_handles.settlement_registry());
    ...
    pub(super) async fn finish_replay_forwarder(
        forwarder_handle: tokio::task::JoinHandle<RuntimeResult<()>>,
