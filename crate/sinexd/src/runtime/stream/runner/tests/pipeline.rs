...
mod lk67_invalidation_reachability {
    ...
    use crate::runtime::{
        ...
        DerivedScopeInvalidation, INVALIDATION_SUBJECT, ScopeReconciler, ScopeReconcilerWrapper,
    };
    ...
    async fn bridge_never_consumes_a_published_scope_invalidation(
        ...
        // Publish a real scope invalidation on the real subject, exactly as
        // `ReplayArchiveExecutor::publish_scope_invalidations` does after an
        // archive cascade (execution/collect.rs).
...
async fn encode_control_message_serializes_scan_ack() -> TestResult<()> {
    ...
    let encoded =
        encode_control_message("scan acknowledgement", operation_id, &ack.module_name, &ack)?;
    let decoded: SourceScanAck = serde_json::from_slice(&encoded)?;
...
async fn finish_replay_forwarder_surfaces_join_error() -> TestResult<()> {
    ...
    let handle: tokio::task::JoinHandle<RuntimeResult<()>> = tokio::spawn(async move {
...
async fn load_bridge_checkpoint_state_surfaces_corrupt_kv(ctx: TestContext) -> TestResult<()> {
    ...
    manager
        .save_checkpoint(&crate::runtime::checkpoint::CheckpointState::default())
        .await?;
