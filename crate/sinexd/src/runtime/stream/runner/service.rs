...
impl<T: RuntimeActor + 'static> RuntimeRunner<T> {
        ...
        T: Default,
    {
        ...
        let heartbeat = crate::runtime::heartbeat::HeartbeatEmitter::from_runtime(
            &runtime,
        ...
        systemd_notify::stop_watchdog(watchdog_handle, "sinex-runtime").await;
        ...
        let drain_complete_result =
            if drain_controller.is_requested() && service_result.is_ok() && shutdown_result.is_ok()
            {
                ...
                let payload = RuntimeDrainComplete {
                    module_name: runtime.control_identity().to_string(),
