...
impl FilesystemSource {
    ...
    async fn take_snapshot(&mut self) -> RuntimeResult<FilesystemState> {
    ...
    async fn count_files_simple(&self, path: &Utf8Path) -> RuntimeResult<u64> {
        let mut count = 0;
        let mut entries = fs::read_dir(path).await?;
...
impl RuntimeModule for FilesystemSource {
    ...
    async fn initialize(&mut self, init: RuntimeInitContext<Self::Config>) -> RuntimeResult<()> {
        ...
        let runtime = RuntimeContext::new(service_info, handles, raw_config, work_dir_utf8);
...
use crate::runtime::exploration::{
    ExplorationProvider, ExportFormat, IngestionHistoryEntry, SourceState,
};

impl ExplorationProvider for FilesystemSource {
...
#[cfg(test)]
#[path = "filesystem_source_test.rs"]
mod tests;
