...
impl StageAsYouGoContext {
    ...
    pub async fn reconcile_inflight(&self) -> RuntimeResult<StageReconciliationSummary> {
        let ttl = self
            .cleanup_config
    ...
    pub async fn reconcile_inflight_older_than(
        &self,
        stale_ttl: Duration,
    ) -> RuntimeResult<StageReconciliationSummary> {
    ...
    pub async fn emit_derived_event(&self, event: Event<JsonValue>) -> RuntimeResult<()> {
        self.event_emitter.emit(event).await
    }
...
fn resumed_prefix_len(handle: &SourceMaterialHandle, content_len: usize) -> RuntimeResult<usize> {
