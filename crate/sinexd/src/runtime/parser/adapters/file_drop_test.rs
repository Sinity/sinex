...
async fn content_drop_materializes_regular_created_file(
    ...
    let record = SourceRecord {
        ...
        source_ts_hint: None,
        metadata: FileDropRecordMetadata::new(FileDropEventKind::Created, &utf8_path).into_json(),
    };
...
async fn content_drop_reuses_material_for_duplicate_path_content(
    ...
    let second = SourceRecord {
        ...
        logical_path: Some(utf8_path.clone()),
        source_ts_hint: None,
        metadata: FileDropRecordMetadata::new(FileDropEventKind::Modified, &utf8_path).into_json(),
    };
    ...
    assert_eq!(
        second_materialized.material_id,
        first_materialized.material_id
    );
    assert_eq!(
...
async fn content_drop_refuses_to_materialize_symlink_target_content(
    ...
    let utf8_symlink_path = Utf8PathBuf::from_path_buf(symlink_path.clone()).map_err(|path| {
        SinexError::validation("test path is not valid UTF-8")
            .with_context("path", path.display().to_string())
    })?;
    let record = SourceRecord {
        ...
        source_ts_hint: None,
        metadata: FileDropRecordMetadata::new(FileDropEventKind::Created, &utf8_symlink_path)
            .into_json(),
    };
    ...
    let materialized =
        materialize_file_content_record(record, acquisition, 1024 * 1024, &[watch_root]).await?;
...
async fn test_file_drop_metadata_contains_event_kind() -> xtask::sandbox::TestResult<()> {
    ...
    if let Ok(Some(Ok(record))) = tokio::time::timeout(Duration::from_secs(3), stream.next()).await
    {
...
#[sinex_test]
async fn file_drop_stream_keeps_watcher_alive_until_stream_drop() -> xtask::sandbox::TestResult<()>
{
    ...
    assert_eq!(
        record.logical_path.as_deref().map(camino::Utf8Path::as_str),
        path.to_str()
    );
    assert!(
        !dropped.load(Ordering::SeqCst),
...
async fn file_drop_watch_survey_counts_nested_directories() -> xtask::sandbox::TestResult<()> {
    ...
    let survey = survey_file_drop_watch_tree(temp_root.path(), 0, None, false, &HashSet::new())?;
    assert_eq!(
...
#[cfg(unix)]
#[sinex_test]
async fn file_drop_watch_survey_skips_unreadable_subdirectories() -> xtask::sandbox::TestResult<()>
{
...
#[sinex_test]
async fn file_drop_watch_targets_use_recursive_when_plan_allows() -> xtask::sandbox::TestResult<()>
{
    let temp_root = TempDir::new()?;
    std::fs::create_dir_all(temp_root.path().join("notes/daily"))?;
...
#[sinex_test]
async fn file_drop_watch_targets_subsume_nested_recursive_roots() -> xtask::sandbox::TestResult<()>
{
    let temp_root = TempDir::new()?;
...
#[sinex_test]
async fn file_drop_watch_targets_apply_budget_across_all_roots() -> xtask::sandbox::TestResult<()> {
    let temp_root = TempDir::new()?;
...
#[sinex_test]
async fn file_drop_watch_plan_uses_recursive_when_budget_suffices() -> xtask::sandbox::TestResult<()>
{
