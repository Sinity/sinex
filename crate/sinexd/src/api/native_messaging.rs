#![doc = include_str!("../../docs/api/native_messaging.md")]
...
pub struct NativeMessagingConfig {
    ...
    extension_roles: std::collections::HashMap<String, crate::api::auth::Role>,
...
impl NativeMessagingConfig {
    ...
    fn resolve_extension_role(&self, extension_id: Option<&str>) -> Result<crate::api::auth::Role> {
        ...
        Ok(extension_id
            ...
            .copied()
            .unwrap_or(crate::api::auth::Role::ReadOnly))
    }
}
...
fn parse_extension_roles(raw: Option<&str>) -> ParsedConfigMap<crate::api::auth::Role> {
    let Some(raw) = raw else {
        return ParsedConfigMap::default();
    };

    match serde_json::from_str::<std::collections::HashMap<String, crate::api::auth::Role>>(raw) {
        Ok(roles) => {
            info!(
...
async fn dispatch_method(
    ...
    let auth = match extension_id {
        Some(id) => crate::api::rpc_server::RpcAuthContext::extension(id, role),
        None => crate::api::rpc_server::RpcAuthContext::system(),
    };

    // Use shared dispatch table from rpc_server
    crate::api::rpc_server::dispatch_rpc_method("native", services, method, params, &auth).await
}
