...
use crate::node_sdk::runtime::stream::SchemaBroadcastEntry;
...
impl NodeSchemaValidator {
    ...
    pub async fn validate(
        ...
        let schema_id = if let Some(id) = schema_id_opt {
            ...
            if self.is_edge_mode() {
                ...
                return Err(crate::node_sdk::SinexError::validation(format!(
        ...
        let validator = {
            ...
            if let Some(v) = schemas.get(&schema_id) {
                ...
                return Err(crate::node_sdk::SinexError::processing(format!(
    ...
    async fn fetch_schema_from_db(&self, source: &str, event_type: &str) -> NodeResult<Uuid> {
        ...
        .await
        .map_err(crate::node_sdk::SinexError::from)?;

        let row = result.ok_or_else(|| {
            crate::node_sdk::SinexError::validation(format!(
                "No active schema registered for {source}.{event_type}"
            ))
        })?;

        let schema_id_str = row.schema_id.as_ref().ok_or_else(|| {
            crate::node_sdk::SinexError::processing("schema_id is NULL in database".to_string())
        })?;

        let schema_id: Uuid = schema_id_str.parse().map_err(|e| {
            crate::node_sdk::SinexError::processing(format!("Invalid schema_id from DB: {e}"))
        })?;

        let validator = fetch_and_compile_from_kv(kv_store, schema_id_str)
            .await
            .ok_or_else(|| {
                crate::node_sdk::SinexError::processing(format!(
