...
impl<T: Node + 'static> NodeRunner<T> {
    ...
    pub async fn initialize_with_transport(
        ...
        let version = crate::node_sdk::version::node_version().map_or_else(
        ...
        let (schema_cache, schema_validator, schema_listener_shutdown, schema_listener_handle) = (
            Option::<Arc<crate::node_sdk::runtime::stream::SchemaBroadcastCache>>::None,
            Option::<()>::None,
        ...
        let cleanup_enabled = {
            #[cfg(feature = "messaging")]
            {
                crate::node_sdk::checkpoint::CheckpointCleanupConfig::from_env().enabled
            }
            #[cfg(not(feature = "messaging"))]
            {
                false
        ...
        let confirmation_buffer_opt = if matches!(self.node.node_type(), NodeType::Automaton) {
            ...
            Some(Arc::new(crate::node_sdk::ConfirmationBuffer::new(
