...
use crate::node_sdk::derived_node::{
    AutomatonContext, DerivedNodeConfig, DerivedOutput, InputProvenanceFilter,
    ScopeReconcilerWrapper, TransducerWrapper,
};
use crate::node_sdk::exploration::{ExplorationProvider, ExportFormat};
#[cfg(feature = "messaging")]
...
async fn make_runtime_state_with_validator(
    ...
    let (event_sender, event_receiver) = mpsc::channel::<Event<JsonValue>>(32);
    let validator = Arc::new(crate::node_sdk::schema_validator::NodeSchemaValidator::new());
...
async fn derived_health_check_reflects_failed_health_reporter(ctx: TestContext) -> TestResult<()> {
    ...
    assert!(
        !crate::node_sdk::runtime::stream::Node::health_check(&adapter).await?,
