...
use super::traits::{Automaton, DerivedNodeConfig};

use crate::node_sdk::checkpoint::CheckpointManager;
use crate::node_sdk::processing::PersistedState;
...
fn log_self_observation_failure(
    node_name: &str,
    metric_name: &str,
    error: &crate::node_sdk::self_observation::SelfObservationError,
) {
    warn!(
        node = node_name,
        metric = metric_name,
    ...
    N: Automaton,
{
    ...
    #[cfg(feature = "messaging")]
    health_reporter: Option<Arc<crate::node_sdk::health_reporter::HealthReporter>>,
    #[cfg(feature = "messaging")]
...
impl<N> crate::node_sdk::runtime::stream::Node for AutomatonRuntime<N>
where
    N: Automaton,
{
    ...
    async fn initialize(&mut self, init: NodeInitContext<Self::Config>) -> NodeResult<()> {
        ...
        #[cfg(feature = "messaging")]
        {
            if let Some(nats_client) = runtime.nats_client() {
                use crate::node_sdk::health_reporter::{HealthReporter, HealthThresholds};
                use crate::node_sdk::self_observation::{SelfObserver, SelfObserverConfig};
...
impl<N> crate::node_sdk::exploration::ExplorationProvider for AutomatonRuntime<N>
where
    N: Automaton,
{
    fn get_source_state(&self) -> NodeResult<crate::node_sdk::exploration::SourceState> {
        let runtime_initialized = self.runtime.is_some();
        ...
        Ok(crate::node_sdk::exploration::SourceState {
    ...
    fn get_ingestion_history(
        &self,
        _limit: u64,
    ) -> NodeResult<Vec<crate::node_sdk::exploration::IngestionHistoryEntry>> {
    ...
    fn export_data(
        ...
        _format: crate::node_sdk::exploration::ExportFormat,
    ) -> NodeResult<()> {
        Err(SinexError::invalid_state(
...
pub type ScopeReconcilerNodeAdapter<N> = AutomatonRuntime<super::traits::ScopeReconcilerWrapper<N>>;
