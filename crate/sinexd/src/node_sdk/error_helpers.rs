...
use crate::node_sdk::{SinexError, runtime::stream::NodeRuntimeState};
...
/// ```
pub fn general_error<E: std::fmt::Display>(error: E, context: &str) -> crate::node_sdk::SinexError {
    crate::node_sdk::SinexError::processing(format!("{context}: {error}"))
}
...
pub trait NodeErrorExt<T> {
    ...
    fn node_err(self, context: &str) -> Result<T, crate::node_sdk::SinexError>;

    /// Convert error to `SinexError::processing` with context
    fn processing_err(self, context: &str) -> Result<T, crate::node_sdk::SinexError>;
}
...
impl<T, E: std::fmt::Display> NodeErrorExt<T> for Result<T, E> {
    ...
    fn processing_err(self, context: &str) -> Result<T, crate::node_sdk::SinexError> {
        self.map_err(|e| crate::node_sdk::SinexError::processing(format!("{context}: {e}")))
    }
}
