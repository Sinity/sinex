...
//!
//! This module implements `sx generate node "spec..."` which uses an LLM
...
impl GeneratorConfig {
    ...
    pub fn from_env(workspace_root: Utf8PathBuf) -> Result<Self> {
        let llm_api_key = std::env::var("ANTHROPIC_API_KEY")
            ...
            .wrap_err("ANTHROPIC_API_KEY or OPENAI_API_KEY must be set for node generation")?;
...
impl NodeGenerator {
    ...
    pub async fn generate(&self, spec: &NodeSpec) -> Result<GeneratedNode> {
        info!(
            name = %spec.name,
            "Generating SimpleProcessor from spec"
        );
    ...
    fn build_prompt(&self, spec: &NodeSpec) -> String {
        let mut context = HashMap::new();
        context.insert("spec", spec.description.as_str());
    ...
    async fn call_anthropic(&self, prompt: &str) -> Result<String> {
        ...
        let response = self
            ...
            .send()
            .await
            .wrap_err("Failed to call Anthropic API")?;

        let status = response.status();
        if !status.is_success() {
            let body = response.text().await.unwrap_or_default();
            return Err(eyre!("Anthropic API error {}: {}", status, body));
        }
        ...
        response
            ...
            .first()
            .map(|c| c.text.clone())
            .ok_or_else(|| eyre!("Empty response from Anthropic"))
    }
    ...
    async fn call_openai(&self, prompt: &str) -> Result<String> {
        ...
        let response = self
            ...
            .send()
            .await
            .wrap_err("Failed to call OpenAI API")?;
        ...
        response
            ...
            .ok_or_else(|| eyre!("Empty response from OpenAI"))
    }
    ...
    fn create_crate(&self, spec: &NodeSpec, code: &str) -> Result<Utf8PathBuf> {
        ...
        std::fs::create_dir_all(crate_path.join("src"))
            .wrap_err_with(|| format!("Failed to create crate directory: {}", crate_path))?;
...
"#,
        ...
        // Write lib.rs
        std::fs::write(crate_path.join("src/lib.rs"), code).wrap_err("Failed to write lib.rs")?;

        info!(
            crate_path = %crate_path,
            "Created generated node crate"
        );
...
mod tests {
    ...
    fn test_extract_rust_code() {
        let config = GeneratorConfig {
            ...
            output_dir: "crate/nodes".to_string(),
            llm_endpoint: "https://api.anthropic.com".to_string(),
            llm_api_key: "test".to_string(),
            model: "test".to_string(),
        };
        let generator = NodeGenerator::new(config).unwrap();
...
That's all!
"#;
        let code = generator.extract_rust_code(response).unwrap();
