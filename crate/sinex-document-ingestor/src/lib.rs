...
impl DocumentProcessor {
    ...
    async fn process_file(&self, file_path: &Path) -> SatelliteResult<()> {
        ...
        if let Some(ref stage_context) = self.stage_context {
            ...
            let source_uri = format!("file://{}", file_path.display());

            // Step 1: Register in-flight source material
            let initial_metadata = json!({
                "original_path": file_path.to_string_lossy(),
                "file_extension": file_path.extension().and_then(|e| e.to_str()),
                "parent_directory": file_path.parent().and_then(|p| p.to_str()),
