...
impl JournalWatcher {
    ...
    pub async fn new(config: JournalConfig) -> NodeResult<Self> {
    ...
    pub(crate) async fn start_streaming(
        ...
        if self.config.import_on_startup {
            if let Err(e) = self.import_historical(&tx, &material).await {
        ...
        if self.config.follow {
            self.follow_journal(tx, &material).await?;
        }
    ...
    pub(crate) async fn import_historical(
                    ...
                    if let Some(event) = self.parse_journal_entry(&entry, material)? {
                        ...
                        if batch.len() >= self.config.batch_size {
                            for event in batch.drain(..) {
                                Self::send_event(tx, event, "journal_batch", material).await?;
                            }
                        }
                    }
                }
        ...
        for event in batch {
            Self::send_event(tx, event, "journal_final_batch", material).await?;
        }
        ...
        if entries_count > 0 {
            ...
            Self::send_event(tx, sync_event, "journal_sync_event", material).await?;
        }
    ...
    async fn follow_journal(
        ...
        self.follow_journal_inner(&tx, material).await?;
                    ...
                    match serde_json::from_str::<serde_json::Value>(&line) {
                        Ok(entry) => {
                            if let Some(event) = self.parse_journal_entry(&entry, material)? {
                                ...
                                Self::send_event(tx, event, "journal_follow_event", material)
                                    .await?;
                            }
                        }
                        Err(e) => {
