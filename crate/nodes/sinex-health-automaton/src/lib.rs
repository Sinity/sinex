...
use sinex_core::{JsonValue, environment, types::Id, Ulid};
use sinex_core::events::EventId;
...
pub enum HealthStatus { Healthy, Warning, Critical, Unknown }
...
impl SimpleNode for HealthAggregator {
    ...
    fn name(&self) -> &str { "health-aggregator" }
    fn input_event_type(&self) -> &str { "system.health" } 
    fn output_event_type(&self) -> &str { "health.aggregated_report" }
    ...
    async fn process(
        ...
        let component_name = input.get("component").and_then(|v| v.as_str()).unwrap_or("unknown").to_string();
        
        let health = state.component_health.entry(component_name.clone()).or_insert_with(|| ComponentHealth {
        ...
        if health.recent_events.len() > 10 { health.recent_events.remove(0); }
