...
mod common {
    ...
    pub use sinex_node_sdk::{
        ...
        stream_processor::{
            Checkpoint, EventSender, Node, NodeCapabilities, NodeInitContext, NodeType, ScanArgs,
...
pub struct HealthAggregatorConfig {
    ...
    pub component_check_intervals: HashMap<String, Seconds>,
    /// Health aggregation window in seconds
    pub aggregation_window_seconds: Seconds,
...
impl Default for HealthAggregatorConfig {
    fn default() -> Self {
        ...
        component_intervals.insert("database".to_string(), Seconds::from_secs(60)); // 1 minute
        component_intervals.insert("filesystem".to_string(), Seconds::from_secs(300)); // 5 minutes
        component_intervals.insert("network".to_string(), Seconds::from_secs(120)); // 2 minutes
        component_intervals.insert("services".to_string(), Seconds::from_secs(180)); // 3 minutes

        Self {
            component_check_intervals: component_intervals,
            aggregation_window_seconds: Seconds::from_secs(3600), // 1 hour
...
/// Health Aggregator using unified Node architecture
///
...
impl HealthAggregator {
    ...
    async fn emit_reports(&self, event_sender: &EventSender) -> NodeResult<u64> {
        ...
        for alert in alerts {
            if let Err(err) = event_sender.send(alert).await {
    ...
    async fn process_confirmed_event(&mut self, provisional: ProvisionalEvent) -> NodeResult<u64> {
        ...
        let db_pool =
            self.fields.db_pool.as_ref().ok_or_else(|| {
    ...
    async fn query_health_events(
        ...
        let window_start = Utc::now()
            - chrono::Duration::seconds(
                self.fields.config.aggregation_window_seconds.as_secs() as i64
            );
    ...
    async fn update_component_health(&mut self, events: &[Event<JsonValue>]) {
        ...
        let stale_threshold = Utc::now()
            - chrono::Duration::minutes(self.fields.config.unhealthy_threshold_minutes as i64);
    ...
    async fn generate_system_health_status(&self) -> NodeResult<Event<JsonValue>> {
