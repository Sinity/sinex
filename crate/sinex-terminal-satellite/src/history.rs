...
impl HistoryWatcher {
        ...
        for file_path in &self.files {
            if file_path.exists() {
                match fs::metadata(file_path) {
                    Ok(metadata) => {
                        self.file_positions.insert(file_path.clone(), metadata.len());
                        info!("Tracking history file: {} (starting at byte {})", 
                ...
                info!("History file does not exist (will watch if created): {}", file_path.display());
    ...
    fn read_new_lines(&mut self, file_path: &PathBuf) -> SatelliteResult<Vec<String>> {
        ...
        // Update position
        self.file_positions.insert(file_path.clone(), content_bytes.len() as u64);

        // Parse lines based on shell type
        let lines = self.parse_history_content(&new_text, file_path);
        
        if !lines.is_empty() {
            debug!("Found {} new history entries in {}", lines.len(), file_path.display());
        }
    ...
    fn command_to_event(&self, command: String, source_file: &PathBuf) -> RawEvent {
        let (shell_type, source_name) = if source_file.to_string_lossy().contains("fish") {
            ("fish", sinex_core::sources::SHELL_FISH_HISTORY)
        } else if source_file.to_string_lossy().contains("zsh") {
            ("zsh", sinex_core::sources::SHELL_ZSH_HISTFILE)
        } else {
            ("bash", sinex_core::sources::SHELL_BASH_HISTFILE)
        };
    ...
    pub async fn start_streaming(&mut self, tx: mpsc::UnboundedSender<RawEvent>) -> SatelliteResult<()> {
        info!("Starting shell history event streaming for {} files", self.files.len());
        ...
        for file_path in self.files.clone() {
            match self.read_new_lines(&file_path) {
                ...
                Err(e) => {
                    warn!("Failed to read initial content from {}: {}", file_path.display(), e);
                }
            }
        }
                    ...
                    match event.kind {
                        EventKind::Modify(_) | EventKind::Create(_) => {
                            ...
                            match self.read_new_lines(&path) {
                                ...
                                Err(e) => {
                                    warn!("Failed to read new content from {}: {}", path.display(), e);
                                }
                            }
                        }
                        _ => {
