...
async fn create_tables(pool: &PgPool) -> Result<(), ApplyError> {
    let table_sql = vec![
        ...
        render_table(&SourceSessionState::create_table_statement()),
...
async fn create_indexes(pool: &PgPool) -> Result<(), ApplyError> {
    ...
    index_sql.extend(render_indexes(SourceSessionState::create_indexes()));
...
DECLARE
    v_operation_id UUID;
BEGIN
    IF p_operation_type NOT IN ('replay', 'archive', 'restore', 'purge', 'tombstone', 'dlq.requeue', 'projection-rebuild') THEN
