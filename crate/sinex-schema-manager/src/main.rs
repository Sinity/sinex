...
macro_rules! register_payloads {
    ($($module:ident :: $payload:ident),* $(,)?) => {{
        ...
        $(
            ...
            let source = <$module::$payload as EventPayload>::SOURCE.as_str();
            let event_type = <$module::$payload as EventPayload>::EVENT_TYPE.as_str();
...
async fn validate_compatibility(pool: &PgPool, from: &str, to: &str) -> Result<()> {
