...
pub trait ValidatedIdentifier: Identifier {
        ...
        Self: Sized,
    {
        ...
        Err(IdentifierError::Other(
            "new_validated not implemented".to_string(),
        ))
    }
}
...
pub mod utils {
    ...
    pub fn convert_batch<From: Identifier, To: ValidatedIdentifier>(
        ...
        from_ids.into_iter().map(convert_identifier).collect()
    }
    ...
    pub fn sort_by_timestamp<T: TemporalIdentifier>(
        mut ids: Vec<T>,
    ) -> Result<Vec<T>, IdentifierError> {
        ids.sort_by(|a, b| match (a.timestamp(), b.timestamp()) {
