...
pub mod validators {
    ...
    pub fn alphanumeric_with_separators(separators: &str) -> impl Fn(&str) -> Result<(), String> + '_ {
        move |value: &str| {
            if value.chars().all(|c| c.is_alphanumeric() || separators.contains(c)) {
                Ok(())
            } else {
                Err(format!("must contain only alphanumeric characters and separators: {}", separators))
            }
        }
    }
    ...
    pub fn matches_regex(pattern: &str) -> impl Fn(&str) -> Result<(), String> + '_ {
        move |value: &str| {
            ...
            match pattern {
                ...
                r"^[a-z][a-z0-9-]*$" => {
                    if value.chars().next().map(|c| c.is_lowercase() && c.is_alphabetic()).unwrap_or(false) &&
                       value.chars().all(|c| c.is_lowercase() || c.is_numeric() || c == '-') {
                        Ok(())
                    } else {
    ...
    pub fn no_forbidden_chars(forbidden: &str) -> impl Fn(&str) -> Result<(), String> + '_ {
        move |value: &str| {
            if let Some(forbidden_char) = value.chars().find(|c| forbidden.contains(*c)) {
                Err(format!("contains forbidden character: '{}'", forbidden_char))
            } else {
                Ok(())
            }
        }
    ...
    pub fn email_format(value: &str) -> Result<(), String> {
        if value.contains('@') && value.len() > 3 && value.chars().filter(|&c| c == '@').count() == 1 {
            let parts: Vec<&str> = value.split('@').collect();
