...
//!
//! Standalone binary that runs the content service as an automaton

use anyhow::Result;
use clap::Parser;
use sinex_content_automaton::ContentServiceAutomaton;
...
#[derive(Parser)]
#[command(name = "sinex-content-automaton")]
#[command(about = "Content service automaton")]
struct Cli {
    ...
    /// Working directory
    #[arg(long, default_value = "/tmp/sinex-content")]
    work_dir: PathBuf,
    ...
    /// Consumer group name
    #[arg(long, default_value = "content-service-group")]
    consumer_group: String,
    
    /// Consumer name
    #[arg(long, default_value = "content-consumer")]
    consumer_name: String,

    /// Annex path for content storage
...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "sinex_content_automaton=info".into()),
        )
        .init();

    let cli = Cli::parse();

    // Set annex path environment variable
    std::env::set_var("SINEX_ANNEX_PATH", cli.annex_path);

    // Create content automaton
    let content_automaton = ContentServiceAutomaton::new();

    // Configure and run the automaton
    let mut runner = HotlogAutomatonRunner::new(content_automaton);
    ...
    // Set up event filters for content RPC requests
